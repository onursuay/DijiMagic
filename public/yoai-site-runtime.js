...
(function () {
  ...
  function initReveal() {
    ...
    for (var k = 0; k < els.length; k++) {
      ...
      // Store params on element for use in observer callback
      el._yoaiDuration  = duration;
      el._yoaiDelay     = delay;
      el._yoaiThreshold = threshold;
      ...
      (function (group) {
        var observer = new IntersectionObserver(function (entries) {
          for (var e = 0; e < entries.length; e++) {
            ...
            var dur = target._yoaiDuration || 600;
            var del = target._yoaiDelay || 0;
            target.style.transition =
              'opacity ' + dur + 'ms ease ' + del + 'ms, ' +
              'transform ' + dur + 'ms ease ' + del + 'ms';
