...
/* SEVİYE 2 — Ad set / ad group kartı (Faz 3). Popup (drill-down) içinde YATAY.
   Onayla/Reddet BURADA var (kampanya kartında değil). "Reklamları Gör" → reklamlar.
   Tüm detaylar AÇIK. */
  ...
  return (
    <div className="relative text-left w-full rounded-2xl overflow-hidden border bg-[#0f172a] border-[#23314d] shadow-md flex flex-col h-full transition-all duration-200 hover:border-emerald-400/40">
      ...
      {adCount > 0 && (
        <button
          onClick={onDrillDown}
          className="mx-4 mb-3 flex items-center justify-center gap-2 rounded-lg bg-slate-800/60 hover:bg-slate-700/60 border border-slate-700/50 px-3.5 py-2.5 text-[12px] text-emerald-300 font-semibold transition-colors relative"
        >
