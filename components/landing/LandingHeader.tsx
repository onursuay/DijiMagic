                ...
                <div className="grid grid-cols-4 gap-x-4 gap-y-1">
                  {productGroups.map((g, gi) => (
                    <div key={gi}>
                      ...
                      {g.items.map((item, i) => (
                        <Link key={i} href={`/ozellikler#${item.slug}`} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/[0.05] transition-colors group" onClick={() => setOpenMenu(null)}>
