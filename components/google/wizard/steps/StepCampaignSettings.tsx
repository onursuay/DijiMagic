    ...
    <div className="space-y-4">
          ...
          <>
            <div className="space-y-1 max-h-56 overflow-y-auto">
              {conversionActions.map((goal: ConversionActionForWizard) => {
                ...
                return (
                  <label key={goal.resourceName} className={`flex items-center gap-2.5 py-2 px-2.5 rounded border cursor-pointer transition-colors ${isSelected ? 'border-primary/20 bg-primary/5/40' : 'border-gray-100 hover:border-gray-200 hover:bg-gray-50/30'}`}>
                    <input type="checkbox" checked={isSelected} onChange={() => toggleGoal(goal.resourceName)} className="rounded border-gray-300 text-primary focus:ring-primary/20 w-4 h-4 shrink-0" />
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        ...
                        {!isEnabled && (<><span className="text-gray-300 text-[13px]">·</span><span className="text-[13px] text-gray-600">{t('conversion.statusUnenabled')}</span></>)}
                      </div>
                    </div>
                    <div className="shrink-0">
                      {isSelected ? (
                        <button type="button" onClick={e => { e.preventDefault(); setPrimary(goal.resourceName) }}
                          className={`flex items-center gap-1 px-2 py-0.5 rounded text-[13px] font-medium transition-colors ${isPrimary ? 'bg-gray-50 text-gray-700 border border-gray-200' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 border border-gray-200'}`}
      ...
      <section className="border border-gray-100 rounded-md bg-white p-4">
        ...
        <div className="space-y-1">
          <label className={`flex items-center gap-3 py-2.5 px-3 rounded border cursor-pointer transition-colors ${state.euPoliticalAdsDeclaration === 'NOT_POLITICAL' ? 'border-primary/30 bg-primary/5/50' : 'border-gray-100 hover:border-gray-200 bg-gray-50/30'}`}>
            <input type="radio" name="euPoliticalAdsDeclaration" value="NOT_POLITICAL" checked={state.euPoliticalAdsDeclaration === 'NOT_POLITICAL'} onChange={() => update({ euPoliticalAdsDeclaration: 'NOT_POLITICAL' })} className="text-primary focus:ring-primary/20" />
          ...
          <label className={`flex items-center gap-3 py-2.5 px-3 rounded border cursor-pointer transition-colors ${state.euPoliticalAdsDeclaration === 'POLITICAL' ? 'border-primary/30 bg-primary/5/50' : 'border-gray-100 hover:border-gray-200 bg-gray-50/30'}`}>
            <input type="radio" name="euPoliticalAdsDeclaration" value="POLITICAL" checked={state.euPoliticalAdsDeclaration === 'POLITICAL'} onChange={() => update({ euPoliticalAdsDeclaration: 'POLITICAL' })} className="text-primary focus:ring-primary/20" />
          ...
          {state.euPoliticalAdsDeclaration === 'POLITICAL' && (
            <div className="flex items-start gap-2 p-3 mt-1 rounded border border-gray-200 bg-gray-50/60 text-[13px] text-gray-700">
              ...
              <div>
                ...
                <a href={euPolicyUrl} target="_blank" rel="noopener noreferrer" className="mt-1.5 inline-block text-primary hover:text-primary underline">
