...
import type { SectorMainItem } from '@/lib/dijimagic/sectorCatalog'
import { isValidCompetitorReference, MIN_COMPETITORS_REQUIRED } from '@/lib/dijimagic/businessProfileValidation'
...
export default function BusinessProfileOnboarding({ onComplete, onClose, isEditMode = false }: Props) {
  const t = useTranslations('dashboard.dijimagic.businessProfile')
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const sectorsRes = await fetch('/api/dijimagic/business-profile/sectors')
        ...
        const profileRes = await fetch('/api/dijimagic/business-profile')
