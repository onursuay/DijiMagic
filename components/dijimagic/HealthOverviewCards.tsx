...
import type { DeepHealthOverview } from '@/lib/dijimagic/analysisTypes'
...
export default function HealthOverviewCards({ health, loading }: Props) {
  ...
  const cards = [
    ...
    { label: 'İyileştirme Fırsatları', value: health.opportunities, subtitle: health.opportunities > 0 ? 'Optimize edilebilir alan var' : 'Kritik fırsat yok', icon: Lightbulb, iconColor: 'text-gray-600', iconBg: 'bg-gray-50' },
    { label: 'Bekleyen Onaylar', value: health.pendingApprovals, subtitle: health.pendingApprovals > 0 ? 'İncelemenizi bekliyor' : 'Bekleyen yok', icon: ClipboardCheck, iconColor: 'text-primary', iconBg: 'bg-emerald-50' },
    { label: 'Önerilen Aksiyonlar', value: health.draftActions, subtitle: health.draftActions > 0 ? 'AI tarafından önerildi' : 'Öneri oluşturulmadı', icon: Layers, iconColor: 'text-indigo-600', iconBg: 'bg-indigo-50' },
  ]
