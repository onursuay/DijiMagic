...
/* DijiAlgoritma Geliştirme Kartları — SEVİYE 1 grid + popup drill-down (Faz 3).
...
import type { ImprovementHierarchy, HierLevel } from '@/lib/dijimagic/ai/hierarchicalStore'
...
export default function HierarchicalImprovements({ onApprovePublish, refreshKey, activeCampaigns }: Props) {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch('/api/dijimagic/improvements/hierarchy', { credentials: 'include' })
  ...
  useEffect(() => {
    ...
    const markerKey = 'dijimagic_kart_bootstrap_at'
    try {
    ...
    void (async () => {
      setBootstrapping(true)
      try {
        const res = await fetch('/api/dijimagic/improvements/bootstrap', { method: 'POST', credentials: 'include' })
        const json = await res.json()
  ...
  const decide = useCallback(async (level: HierLevel, id: string, action: 'approve' | 'reject' | 'unreject' | 'applied', reason?: string) => {
    ...
    setActionError(null)
    try {
      const res = await fetch('/api/dijimagic/improvements/hierarchy/decision', {
        method: 'POST',
        credentials: 'include',
  ...
  return (
    <div data-testid="dijimagic-hierarchy">
      ...
      {(loading || pending || showPreparing) ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
          ...
          <p className="text-sm text-gray-500">{showPreparing ? t('preparing') : t('loading')}</p>
        </div>
