...
import type { AccountAlertRow } from '@/lib/dijimagic/ai/hierarchicalStore'
...
export default function AccountAlertsBanner({ alerts }: { alerts: AccountAlertRow[] }) {
  ...
  return (
    <div>
      <style>{`
        @keyframes dijimagicShimmerSpin { to { transform: rotate(360deg); } }
        @keyframes dijimagicTap { 0%, 55%, 100% { transform: translateY(0) scale(1); } 70% { transform: translateY(4px) scale(.8); } 85% { transform: translateY(0) scale(1); } }
        .dijimagic-flip { perspective: 1300px; position: relative; }
        .dijimagic-flip-inner { display: grid; height: 100%; transition: transform .6s cubic-bezier(.2,.7,.2,1); transform-style: preserve-3d; }
        .dijimagic-flip:hover .dijimagic-flip-inner { transform: rotateY(180deg); }
        .dijimagic-flip.is-flipped .dijimagic-flip-inner { transform: rotateY(180deg); }
        .dijimagic-flip { cursor: pointer; }
        .dijimagic-flip:focus-visible { outline: 2px solid #34d399; outline-offset: 2px; border-radius: 1rem; }
        .dijimagic-face { grid-area: 1 / 1; backface-visibility: hidden; -webkit-backface-visibility: hidden; border-radius: 1rem; overflow: hidden; }
        .dijimagic-back { transform: rotateY(180deg); }
        .dijimagic-tap { animation: dijimagicTap 1.5s ease-in-out infinite; transform-origin: center; }
        .dijimagic-shimmer { position: absolute; inset: -2px; border-radius: 1.1rem; overflow: hidden; }
        .dijimagic-shimmer::before {
          ...
          animation: dijimagicShimmerSpin 3.4s linear infinite;
        }
        @media (prefers-reduced-motion: reduce) {
          .dijimagic-shimmer::before, .dijimagic-tap { animation: none; }
          .dijimagic-flip-inner { transition: none; }
        }
      `}</style>
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {sorted.map((a) => {
          ...
          return (
            <div
              key={a.id}
              className={`dijimagic-flip min-h-[13rem] ${isFlipped ? 'is-flipped' : ''}`}
              role="button"
              tabIndex={0}
            ...
            >
              <div className="dijimagic-shimmer" aria-hidden="true" />
              <div className="dijimagic-flip-inner">
                {/* ÖN YÜZ — ikon üst · başlık merkez · animasyonlu tap ikonu alt */}
                <div className="dijimagic-face border border-[#23314d] p-4 flex flex-col items-center text-center" style={FACE_STYLE}>
                  ...
                  <Pointer className="dijimagic-tap w-7 h-7 text-emerald-400 shrink-0" aria-label={t('flipHint')} />
                </div>
                {/* ARKA YÜZ — detay (hover) */}
                <div className="dijimagic-face dijimagic-back border border-[#23314d] p-5 flex flex-col" style={FACE_STYLE}>
