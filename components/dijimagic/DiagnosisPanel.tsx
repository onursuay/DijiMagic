...
/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Diagnosis Panel

   Standalone bileşen. Mevcut kampanya insights'larını alır,
   /api/dijimagic/diagnose'a gönderir, root cause + önerilen
...
import type { Decision, DecisionActionType } from '@/lib/dijimagic/meta/decision'
...
export default function DiagnosisPanel({ campaigns, onApplyAction }: Props) {
  const t = useTranslations('dashboard.dijimagic.diagnosis')
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        setLoading(true)
        setErr(null)
        const res = await fetch('/api/dijimagic/diagnose', {
          method: 'POST',
