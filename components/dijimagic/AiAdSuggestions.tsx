...
import { sanitizeProposalForDisplay, isGenericProposalContent } from '@/lib/dijimagic/competitorDisplay'
import type { Platform } from '@/lib/dijimagic/analysisTypes'
import type { DiagnosisResult, RootCauseId } from '@/lib/dijimagic/meta/diagnosis'
import type { Decision } from '@/lib/dijimagic/meta/decision'
...
const PROPOSAL_CACHE_KEY = 'dijimagic_proposals_cache_v4'
// Eski key'leri temizle — stale data gösterme
const LEGACY_CACHE_KEYS = ['dijimagic_proposals_cache_v1', 'dijimagic_proposals_cache_v2', 'dijimagic_proposals_cache_v3']
...
export default function AiAdSuggestions({ connectedPlatforms, onOpenWizard, onApprovalChanged }: Props) {
  const t = useTranslations('dashboard.dijimagic.aiSuggestions')
  ...
  const refreshApprovals = useCallback(async () => {
    try {
      const res = await fetch('/api/dijimagic/approvals?limit=200', { credentials: 'include' })
      if (!res.ok) return
  ...
  const fetchProposals = useCallback(
    async (forceGenerate = false) => {
      ...
      try {
        const res = await fetch('/api/dijimagic/generate-ad', {
          method: 'POST',
  ...
  const patchApproval = async (
    ...
    try {
      const res = await fetch(`/api/dijimagic/approvals/${approvalId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div data-testid="dijimagic-proposal-grid">
