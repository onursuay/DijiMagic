...
/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Approval History Panel (Faz 0D)

   Kullanıcının son approval kararlarını modern kart grid'inde gösterir.
   Veri kaynağı: /api/dijimagic/approvals?limit=20 (dijimagic_pending_approvals).
...
import { translateEnum, type Locale } from '@/lib/dijimagic/translations'
...
export default function ApprovalHistoryPanel({ refreshKey }: Props) {
  const t = useTranslations('dashboard.dijimagic.approvalHistory')
  ...
  const fetchOutcomeForApproval = useCallback(async (rec: ApprovalRecord) => {
    ...
    try {
      const res = await fetch(
        `/api/dijimagic/results?limit=1${rec.source_campaign_id ? `&sourceCampaignId=${encodeURIComponent(rec.source_campaign_id)}` : ''}`,
        { credentials: 'include' },
      )
      if (!res.ok) return
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const res = await fetch('/api/dijimagic/approvals?limit=20', {
        credentials: 'include',
      })
      if (!res.ok) {
        setRecords([])
