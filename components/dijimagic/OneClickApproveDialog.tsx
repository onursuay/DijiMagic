...
/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — One-Click Approve Dialog

   Proposal alır, /api/dijimagic/one-click-approve çağırır.
...
interface Props {
  ...
  onClose: () => void
  /** Faz 0C: dijimagic_pending_approvals row id — publish başarılı olursa published olarak işaretlenir */
...
function humanizeErrorMsg(
  ...
  if (
    lower.includes('dijimagic_direct_publish_enabled') ||
...
export default function OneClickApproveDialog({ proposal, onClose, approvalId, onPublished }: Props) {
  const t = useTranslations('dashboard.dijimagic.oneClickApprove')
  ...
  async function submit(overrideChoices?: typeof choices) {
    ...
    try {
      const res = await fetch('/api/dijimagic/one-click-approve', {
        method: 'POST',
