...
/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Approval Version Panel (Faz 5)
...
export default function ApprovalVersionPanel({ approvalId }: Props) {
  const t = useTranslations('dashboard.dijimagic.approvalVersions')
  ...
  const fetchVersions = useCallback(async () => {
    try {
      const res = await fetch(`/api/dijimagic/approvals/${approvalId}/versions`, {
        credentials: 'include',
      })
      if (!res.ok) return
