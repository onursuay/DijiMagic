...
export default function AdImageGenerator({ prompt, aspectRatio = '1:1', className = '' }: Props) {
  ...
  const generateImage = async () => {
    ...
    try {
      ...
      const enhanceRes = await fetch('/api/tasarim/enhance-prompt', {
        ...
        body: JSON.stringify({ prompt: `Dijital reklam görseli: ${prompt}. Profesyonel, modern, temiz arka plan. NO TEXT on image, no words, no letters, no typography, purely visual content only.`, locale: 'tr' }),
      })
      ...
      if (res.ok) {
        ...
        if (data.url) {
          ...
          try { sessionStorage.setItem(`dijimagic_adimg_${btoa(prompt).slice(0, 20)}`, data.url) } catch {}
        } else { setError(true) }
      } else { setError(true) }
    } catch { setError(true) }
  ...
  useEffect(() => {
    ...
    try {
      const cached = sessionStorage.getItem(`dijimagic_adimg_${btoa(prompt).slice(0, 20)}`)
