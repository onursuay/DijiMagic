...
/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Meta Preflight Panel

   Kullanım: AdCreationWizard içinde preview sonrası gösterilir.
   - /api/dijimagic/preflight çağırır
...
export default function MetaPreflightPanel({
  ...
  const t = useTranslations('dashboard.dijimagic.preflight')
  ...
  async function runPreflight(overrides?: Partial<PreflightConfirmPayload>) {
    ...
    setErr(null)
    try {
      const res = await fetch('/api/dijimagic/preflight', {
        method: 'POST',
