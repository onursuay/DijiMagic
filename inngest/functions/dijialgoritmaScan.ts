/* ──────────────────────────────────────────────────────────
   Inngest Function: dijialgoritma/scan.user
...
import { buildBatchRequestParams, parseBatchResult } from '@/lib/dijimagic/ai/agent'
import { officialKnowledgeBlock } from '@/lib/dijimagic/ai/docs/officialKnowledgeBlock'
import {
  gatherUserScanInputs,
  scanContextFromFetched,
  persistAccountAndDailyRun,
  writeFailedRun,
} from '@/lib/dijimagic/ai/scanUser'
import { scrapeDeclaredCompetitors } from '@/lib/dijimagic/ai/competitorScanStep'
import type { AiEngineResult, AiPlatform } from '@/lib/dijimagic/ai/types'
...
export const dijialgoritmaScanUser = inngest.createFunction(
  {
    id: 'dijialgoritma-scan-user',
    name: 'DijiAlgoritma — Per-User AI Scan (Batch)',
    concurrency: { limit: 5 },
    retries: 2,
    triggers: [{ event: 'dijialgoritma/scan.user' }],
  },
  async ({ event, step, logger }) => {
    ...
    // 0) Rakip scrape (A4) — DIJIALGORITMA_SCRAPE_COMPETITORS=true ise beyan
