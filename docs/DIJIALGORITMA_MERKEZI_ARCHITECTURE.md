# DijiAlgoritma Merkezi — Living Architecture Document
...
DijiAlgoritma Merkezi; reklam verilerini analiz eden, rakip intel toplayan, AI destekli öneriler üreten, insan onayı bekleyen, opsiyonel olarak Meta'ya PAUSED kampanya olarak basan ve sonuçları feedback loop'a geri besleyen bütünleşik bir sistemdir.
...
│  dijimagic_pending_approvals · dijimagic_approval_versions        │
│  dijimagic_model_decisions · dijimagic_competitor_ads             │
│  dijimagic_competitor_insights · dijimagic_platform_doctrine      │
│  dijimagic_publish_audit_log · dijimagic_action_outcomes          │
...
**Dosyalar:** `lib/dijimagic/approvalStore.ts`, `supabase/migrations/20260510003000_create_dijimagic_pending_approvals.sql`

- `dijimagic_pending_approvals` tablosu: status enum (pending/approved/rejected/hold/editing/published/failed/expired)
...
**Dosyalar:** `lib/dijimagic/publishAuditStore.ts`, `supabase/migrations/20260510001000_create_dijimagic_publish_audit_log.sql`

- `dijimagic_publish_audit_log`: pending → success/blocked/failed/orphaned yaşam döngüsü
...
**Dosyalar:** `app/api/dijimagic/one-click-approve/route.ts`, `lib/dijimagic/meta/orchestrator.ts`
...
5. Budget guard (`evaluateBudgetGuard`) — `DIJIMAGIC_MAX_DAILY_BUDGET_TRY`
...
- Son 20 approval kaydını listeler (`GET /api/dijimagic/approvals?limit=20`)
...
**Dosyalar:** `lib/dijimagic/campaignDoctrine.ts`, `supabase/migrations/20260510004000_create_dijimagic_platform_doctrine.sql`

- `dijimagic_platform_doctrine`: platform/campaign_type bazında stratejik doktrin
...
**Dosyalar:** `lib/dijimagic/competitorAdStore.ts`, `lib/dijimagic/competitorInsightStore.ts`

- `dijimagic_competitor_ads`: platform+source bazında normalize reklam kayıtları, fingerprint-based upsert, dedup
- `dijimagic_competitor_insights`: platform+query başına özet insight (CTR/budget/tema/format ağırlıkları)
...
**Dosyalar:** `lib/dijimagic/googleTransparencyConnector.ts`, `lib/dijimagic/competitorScanner.ts`, `app/api/dijimagic/competitors/google-auction/route.ts`, `components/dijimagic/CompetitorDashboard.tsx`
...
**Dosyalar:** `lib/dijimagic/synthesisEngine.ts`
...
**Dosyalar:** `lib/dijimagic/security/*.ts`, `.env.example`
...
**Dosyalar:** `lib/dijimagic/multiAiEngine.ts`, `lib/dijimagic/modelDecisionStore.ts`, `supabase/migrations/20260510006000_create_dijimagic_model_decisions.sql`

- `dijimagic_model_decisions`: role (strategist/creative/risk_policy/technical_validator/judge) + provider + output_json
- `DIJIMAGIC_MULTI_AI_ENABLED=false` — default kapalı (shadow mode)
...
**Dosyalar:** `lib/dijimagic/approvalStore.ts` (eklenti), `lib/dijimagic/modelDecisionStore.ts` (eklenti), `supabase/migrations/20260510007000_create_dijimagic_approval_versions.sql`, `components/dijimagic/DecisionDeskSummary.tsx`, `components/dijimagic/ApprovalVersionPanel.tsx`, `components/dijimagic/AiAdSuggestions.tsx` (güncellendi)

- **dijimagic_approval_versions**: UNIQUE(approval_id, version_number); source CHECK (original/edited/regenerated/manual); NO UPDATE/DELETE policy (immutable audit)
...
**Dosyalar:** `lib/dijimagic/publishSafety.ts`, `lib/dijimagic/publishPayloadValidator.ts`, `lib/dijimagic/policyGuard.ts`

- **publishSafety.ts**: `isDirectPublishEnabled()` (DIJIMAGIC_DIRECT_PUBLISH_ENABLED), `isActivePublishEnabled()` (DIJIMAGIC_ACTIVE_PUBLISH_ENABLED), `assertPausedOnly()` (ACTIVE açıksa throw), `getMaxDailyBudgetTry()`, `validatePublishFeatureFlags()`
...
**Dosyalar:** `lib/dijimagic/resultTrackingStore.ts`, `app/api/dijimagic/results/route.ts`, `supabase/migrations/20260510008000_create_dijimagic_recommendation_results.sql`

- `dijimagic_recommendation_results`: before_snapshot + after_snapshot (MetricSnapshot JSONB) + metric_delta + outcome enum + outcome_summary
...
|-------|------|-----------|
| `dijimagic_pending_approvals` | Approval lifecycle | 20260510003000 |
| `dijimagic_approval_versions` | Versiyonlama (immutable) | 20260510007000 |
| `dijimagic_model_decisions` | Multi-AI karar kayıtları | 20260510006000 |
| `dijimagic_publish_audit_log` | Publish girişim audit | 20260510001000 |
| `dijimagic_action_outcomes` | Learning loop geçmişi | 20260510000000 |
| `dijimagic_competitor_ads` | Normalize rakip reklamlar | 20260510005000 |
| `dijimagic_competitor_insights` | Platform başına insight özeti | 20260510005100 |
| `dijimagic_platform_doctrine` | Kampanya tipi doktrin | 20260510004000 |
| `dijimagic_recommendation_results` | Before/after metric tracking | 20260510008000 |
...
|----------|------|---------|
| `DIJIMAGIC_MAX_DAILY_BUDGET_TRY` | Bütçe üst sınırı | 1000 |
| `DIJIMAGIC_MULTI_AI_ENABLED` | Multi-AI Decision Desk | false |
| `DIJIMAGIC_MULTI_AI_TIMEOUT_MS` | Multi-AI timeout | 45000 |
| `DIJIMAGIC_DIRECT_PUBLISH_ENABLED` | One-click-approve aktif | true (tanımsızsa) |
| `DIJIMAGIC_ACTIVE_PUBLISH_ENABLED` | ACTIVE kampanya izni | false |
| `DIJIMAGIC_COMPETITOR_INTEL_ENABLED` | Rakip tarama | true |
| `DIJIMAGIC_DAILY_RUN_ENABLED` | Günlük otomatik analiz | true |
...
Kullanıcı verisi (Meta/Google kampanyaları)
  ...
  ↓  → dijimagic_pending_approvals (status: pending)
  ↓
  ↓ [Multi-AI: shadow mode]
  ↓ multiAiEngine → dijimagic_model_decisions
  ...
  ↓      → status: published
  ↓      → dijimagic_publish_audit_log: success
  ↓
  ↓ [Result Tracking — Faz 7]
  ↓ recordBeforeSnapshot() → dijimagic_recommendation_results
...
2. Yeni `lib/dijimagic/tiktok/` orchestrator
3. `one-click-approve/route.ts` → platform switch case
4. `capabilityMatrix.ts` → TikTok objective/destination mapping
5. `dijimagic_pending_approvals.platform` CHECK constraint güncelle (migration)

**Yeni Multi-AI rolü eklemek:**
1. `multiAiEngine.ts` → role tanımı + prompt
2. `dijimagic_model_decisions` → role CHECK constraint güncelle (migration)
