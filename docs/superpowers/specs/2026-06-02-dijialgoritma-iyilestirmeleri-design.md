# DijiAlgoritma İyileştirmeleri — Tasarım Dokümanı

**Tarih:** 2026-06-02
**Kapsam:** DijiAlgoritma sayfası (`app/dijimagic/`) ve ilişkili bileşenler için 5 maddelik iyileştirme seti.
...
| DijiAlgoritma EN slug | `dijialgorithm` (TR `/dijialgoritma`, EN `/en/dijialgorithm`) |
...
### Sorun
- `app/dijimagic/` klasörü → URL `/dijimagic`, fakat sayfa başlığı **"DijiAlgoritma"**. Tek gerçek uyumsuzluk.
...
|---|---|---|---|---|
| `dijimagic` | DijiAlgoritma | dijimagic (same) | **dijialgorithm** | **Klasör → `dijialgoritma`**; TR `/dijialgoritma`, EN `/en/dijialgorithm`. Redirect GEREKMEZ — sistem henüz canlıya açılmadı, eski `/dijimagic` linkini kullanan yok |
...
### Etkilenen dosyalar
- `app/dijimagic/` → `app/dijialgoritma/` (git mv — tüm alt dosyalar taşınır)
- [middleware.ts](middleware.ts) — `EN_TO_TR`, `TR_TO_EN`, `APP_SLUGS` güncellenir
- [lib/nav.ts](lib/nav.ts):74-76 — `id: 'dijialgoritma'`, `href: '/dijialgoritma'`
- `lib/routes.ts` (ROUTES sabiti) — `DIJIMAGIC` varsa `/dijialgoritma`'ya güncellenir
- [components/SidebarNav.tsx](components/SidebarNav.tsx) — `getTranslationKey` eşlemesi (`dijimagic`→`dijialgoritma`)
- [components/seo/SeoSitesPanel.tsx](components/seo/SeoSitesPanel.tsx):253 — `href="/dijimagic"` → `/dijialgoritma`
- `locales/tr.json` + `locales/en.json` — sidebar nav key `dijimagic`→`dijialgoritma` ("DijiAlgoritma" / "DijiAlgorithm")

> **Redirect yok:** Sistem henüz kullanıcıya açılmadığı için eski `/dijimagic` URL'sine giden trafik/SEO/bookmark yok → `next.config` redirect'i GEREKMEZ. Yalnız iç linkler (yukarıdaki 2 yer) güncellenir.
- Grep ile kalan `'/dijimagic'` route literal'leri (api/dijimagic HARİÇ) taranır
...
**Sorun:** [DijiAlgoritmaHeader.tsx](components/dijimagic/DijiAlgoritmaHeader.tsx):42-60 — `flex justify-between` ile ticker sağda, hesap seçici dropdown'a sıkışıp truncate oluyor ("…dropdown altında kalıyor").
...
**Sorun:** [CommandCenterHeader.tsx](components/dijimagic/CommandCenterHeader.tsx):70-83 — "AI Analiz" rozeti renkli pill; "Son: …" düz gri yazı.
**Çözüm:**
...
**Kök neden:** [AdCreationWizard.tsx](components/dijimagic/AdCreationWizard.tsx) platform seçince `/api/dijimagic/generate-ad`'i `forceGenerate` olmadan çağırıyor. Tarama hiç çalışmadığı için route persisted veri bulamıyor ve [generate-ad/route.ts](app/api/dijimagic/generate-ad/route.ts):360-369 `{ ok:true, proposals:[], empty:true }` (boş, hata değil) dönüyor. Wizard `proposals.length === 0`'ı hata sanıyor.
...
**Sorun:** [AdCreationWizard.tsx](components/dijimagic/AdCreationWizard.tsx):192,204 — `<span>M</span>` / `<span>G</span>` hardcoded.
...
### Sorun
[HierarchicalImprovements.tsx](components/dijimagic/hierarchy/HierarchicalImprovements.tsx):115-119 — kartlar yalnızca haftalık cron'dan (Pazar 03:00 UTC) gelir. Yeni kullanıcı/owner için ilk tarama tetikleyicisi YOK → "Henüz geliştirme kartı yok. Haftalık analiz Pazar gece otomatik çalışır." uyarısı çıkıyor.

### Çözüm — mevcut Command Center bootstrap deseninin kartlara uyarlanması
Referans desen: [page.tsx](app/dijimagic/page.tsx):116-162 — CC boşsa `triggerBackgroundBootstrap()` → `POST /api/dijimagic/daily-run` otomatik tetikleniyor. Aynısı kartlar için kurulur:

1. **Yeni endpoint** `POST /api/dijimagic/improvements/bootstrap`
   - Oturum kullanıcısı (cookie `user_id`) için `dijialgoritma/campaign-improvements.user` Inngest event'ini tetikler (gerekirse `dijialgoritma/scan.user` ile birlikte).
...
2. **UI** [HierarchicalImprovements.tsx](components/dijimagic/hierarchy/HierarchicalImprovements.tsx)
...
### Etkilenen dosyalar
- `app/api/dijimagic/improvements/bootstrap/route.ts` (YENİ) — event tetikleyici + cooldown
- [components/dijimagic/hierarchy/HierarchicalImprovements.tsx](components/dijimagic/hierarchy/HierarchicalImprovements.tsx) — bootstrap çağrısı + durumlar + periyodik refresh
- [app/dijimagic/page.tsx](app/dijimagic/page.tsx) — `activeCampaigns` prop geçişi
...
### Doğrulama adımları
1. `/dijialgoritma` açılır; `/en/dijialgorithm` çalışır; EN'de `/en/subscription`, `/en/account`, `/en/invoices` çözülür. (Eski `/dijimagic` artık tanımlı değil — redirect yok, gerek de yok.)
...
## Açık Riskler / Notlar
- **Klasör rename (dijimagic→dijialgoritma):** Next.js route segment değişimi; build cache temizliği + deploy sonrası CDN cache gözlenmeli. Sistem henüz canlıya açılmadığı için eski link/SEO riski YOK — redirect gerekmez, yalnız 2 iç link güncellenir.
- **Batch API gecikmesi (Madde 4):** İlk tarama anlık değil; "hazırlanıyor" beklentisi UI'da net verilir.
- **EN slug ekleme (subscription/account/invoices):** middleware rewrite ile çözülür (redirect değil); TR slug klasör adı değişmez, mevcut TR linkler bozulmaz.
