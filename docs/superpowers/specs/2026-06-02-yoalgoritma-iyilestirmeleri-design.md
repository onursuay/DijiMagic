...
|---|---|---|---|---|
| `yoai` | YoAlgoritma | yoai (same) | **yoalgorithm** | **Klasör → `yoalgoritma`**; TR `/yoalgoritma`, EN `/en/yoalgorithm`. Redirect GEREKMEZ — sistem henüz canlıya açılmadı, eski `/yoai` linkini kullanan yok |
...
> **Redirect yok:** Sistem henüz kullanıcıya açılmadığı için eski `/yoai` URL'sine giden trafik/SEO/bookmark yok → `next.config` redirect'i GEREKMEZ. Yalnız iç linkler (yukarıdaki 2 yer) güncellenir.
...
### Doğrulama adımları
1. `/yoalgoritma` açılır; `/en/yoalgorithm` çalışır; EN'de `/en/subscription`, `/en/account`, `/en/invoices` çözülür. (Eski `/yoai` artık tanımlı değil — redirect yok, gerek de yok.)
...
## Açık Riskler / Notlar
- **Klasör rename (yoai→yoalgoritma):** Next.js route segment değişimi; build cache temizliği + deploy sonrası CDN cache gözlenmeli. Sistem henüz canlıya açılmadığı için eski link/SEO riski YOK — redirect gerekmez, yalnız 2 iç link güncellenir.
