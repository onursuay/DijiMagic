# DijiAlgoritma AI Engine — Test Planı
...
| `vercel.json` cron `/api/cron/dijialgoritma-scan` schedule `0 5 * * *` | ✅ doğrulandı | UTC 05:00 = Istanbul 08:00 |
...
- Preview URL'e gir → login ol → `/dijimagic` sayfasına git → `triggerBackgroundBootstrap` otomatik POST atar `/api/dijimagic/daily-run`'a.
- Flag açıksa daily-run AI engine'e delege eder ([app/api/dijimagic/daily-run/route.ts:228-243](../app/api/dijimagic/daily-run/route.ts#L228-L243)).
- 30-90sn içinde sonuç gelmeli (15 iter * ~5-10sn).

**Yöntem B — Tüm kullanıcılar (cron simülasyonu, CRON_SECRET'lı):**
```
curl -i "https://<preview-url>/api/cron/dijialgoritma-scan" \
...
```
curl -i -X POST "https://<preview-url>/api/cron/dijialgoritma-scan" \
...
| **Aynı kampanya için tutarsız öneri** | İki ardışık run'da aynı kampanya hakkında zıt öneriler | Stateless analiz tutarsız. Geçmiş outcome'ları (`dijimagic_action_outcomes`) tool'a vermek lazım. |
...
- [ ] Frontend smoke test: `/dijimagic` sayfası açıldığında "AI Analiz" rozeti gözüküyor
...
- `/api/cron/dijialgoritma-scan` → erken `skipped: true` response (no Claude call).
- `/api/dijimagic/daily-run` cron + manual POST → eski rule engine + adCreator flow'u tam çalışır.
- `dijimagic_daily_runs.command_center_data` eski şekilde yazılır (`aiGenerated: false`).
