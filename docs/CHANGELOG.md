# DijiMagic Project — Değişiklik Günlüğü
...
- **Güvenlik/bütünlük:** Kredi tek kez düşülür, hata/timeout→`refundCreditsServer`, başarı→iade yok; bozuk site (gate fail) ASLA persist edilmez; persist Inngest `step.run` ile event-başına-bir-kez. Bayrak **OFF** → prod davranışı değişmedi (dark deploy).
- **Dosyalar:** lib/website/{genJobStore,persistGeneratedSite,sandboxHmac.mjs}, inngest/functions/websiteAgenticGenerate.ts, app/api/website/[id]/{generate,job,jobs/[jobId]/progress,jobs/[jobId]/complete}/route.ts, app/api/cron/website-jobs-reconcile/route.ts, app/web-site-yoneticisi/[id]/page.tsx, components/website/WizardBuildingAnimation.tsx, supabase/migrations/20260627000000_website_gen_jobs.sql, vercel.json, locales/{tr,en}.json
...
## 2026-06-27 — Web Site Yöneticisi GERÇEK kök neden: lambda'da eksik modül (MODULE_NOT_FOUND → 500)
- **Sorun:** Tüm Web Site Yöneticisi özelliği prod'da kırıktı — site üretimi "Site oluşturulamadı." + serve/önizleme/thumb route'ları da 500. **Production runtime log'u kesin gösterdi:** `Error: Cannot find module 'sanitize-html'` — require stack `lib/website/codegen/renderGate.mjs`. Kök neden: codegen `.mjs` dosyaları (`renderGate`, `assembleDocument`, `tailwindCompile`) CommonJS paketlerini **`createRequire(import.meta.url)` ile dinamik** `require('sanitize-html')` / `require('tailwindcss')`/postcss/autoprefixer şeklinde yüklüyordu. Next.js file-tracing dinamik require'ı **göremediği için** bu paketleri serverless lambda'ya **dahil etmiyordu** → modül init'te `MODULE_NOT_FOUND` → route 500 HTML → istemci `res.json()` throw → genel "Site oluşturulamadı." Lokalde tüm node_modules mevcut olduğundan görünmüyordu; yalnız prod lambda'da (yalnız izlenen dosyalar var). (Önceki "60sn timeout" teşhisi YANLIŞTI — semptom aynıydı, sebep değil; maxDuration iyileştirmesi yine de korundu.)
- **Çözüm:** (1) Üç `.mjs` dosyasında dinamik `require(...)` → **statik `import ... from '...'`** (tracer görür, lambda'ya dahil eder). (2) `next.config.mjs` → `experimental.serverComponentsExternalPackages: ['sanitize-html','tailwindcss','postcss','autoprefixer']` (Next bunları bundle ETMEZ, node_modules'tan dahil eder; büyük/dinamik tailwind güvenle external). Teşhis: auth'suz prob `generate`=500 HTML ama kardeş route'lar `401 JSON` → crash route'a özel; `vercel logs` ile gerçek stack trace.
- **Dosyalar:** lib/website/codegen/{renderGate,assembleDocument,tailwindCompile}.mjs, next.config.mjs

## 2026-06-27 — (İkincil/ön-teşhis) generate route 60sn→300sn timeout payı
- **Sorun:** Web Site Yöneticisi'nde AI ile site üretimi **sürekli** "Site oluşturulamadı." hatası veriyordu. İlk teşhiste `POST /api/website/[id]/generate` route'unun `maxDuration = 60`'ı, tek istekte yaptığı işin (kurulum + Claude içerik çağrısı `timeoutMs:60_000` + Pexels stok görsel çözümleme + DB yazımı) **toplam süresinden kısa** sanıldı. (GERÇEK kök neden yukarıdaki MODULE_NOT_FOUND çıktı; bu değişiklik yine de geçerli bir iyileştirme olarak korundu.) Üretim 60sn'yi aşınca Vercel fonksiyonu öldürüyor (504, JSON değil) → istemcinin `res.json()`'u throw ediyor → genel `buildError` = "Site oluşturulamadı." fallback'i gösteriliyordu. Bütçe yapısal olarak yetersiz olduğu için hata **tutarlıydı**. (Not: prod, `WEBSITE_CODEGEN_V2` set OLMADIĞI için legacy `generateSitePages` yolunu kullanıyor.)
...
- **Dosyalar:** app/api/website/[id]/generate/route.ts, lib/website/ai/generate.ts
...
- **Dosyalar:** app/icon.png, app/favicon.ico, app/apple-icon.png, public/icons/icon-192.png, public/icons/icon-512.png, public/icons/icon-maskable-512.png
...
## 2026-06-20 — Hero: cursor-following glow TRAIL (tam-alan aurora kaldırıldı) + DijiAlgoritma adset modalı Portal ile tam-ortalı
...
- **Dosyalar:** `components/landing/HeroGlowTrail.tsx` (yeni), `components/landing/HeroAuroraBackground.tsx` (silindi), `app/page.tsx`, `components/dijimagic/hierarchy/DrilldownModal.tsx`
...
- **Dosyalar:** `app/layout.tsx`, `app/web-site-yoneticisi/[id]/onizleme/page.tsx`

## 2026-06-20 — Web Site Yöneticisi: marka rengi taşması (nötr arka plan + %10 aksan disiplini)
- **Sorun:** Kurumsal renk "deniz mavisi" (navy) seçilince üretilen site HER ŞEYİ navy boyuyordu — marka rengi zevkli bir AKSAN olacağına tüm arka planları (sayfa + bölümler) dolduruyordu. Kök neden prompt seviyesindeydi: (1) `designSystem.ts` "tüm paleti marka renginden TÜRET" diyordu → `surface` (sayfa arka planı) navy oluyordu; (2) `htmlGenerateShared.mjs` COLOR ethos'u "--accent BASKIN olsun… renk sayfaya AKSIN" diyordu; (3) "modern" stil gradyanı hero + CTA + bölümlere `--gradient-brand` dayatıyordu.
- **Çözüm:** Token'lar (`--surface`/`--accent-soft`/`--accent`) zaten ayrıydı — prompt'lar yanlış kullanıyordu. (A) `designSystem.ts`: rol ayrımı + 60-30-10 disiplini (surface NÖTR; accent yalnız CTA/link/ikon ~%10; accentSoft yalnız küçük rozet). (B) `htmlGenerateShared.mjs`: COLOR ethos 60-30-10 ile yeniden yazıldı (bölüm/sayfa arka planı `bg-[var(--surface)]`; accent tam-bölüm arka planı YASAK; en çok BİR aksan bandı), modern gradyan tek banda sınırlandı, `blockContractRules()` aynı disiplini yansıtır. (C) Savunma: `designSystemValidate.mjs`'e HSL ayrıştırıcı + `ensureNeutralSurface` guard — model marka-tonlu surface üretirse zorla nötre çeker (hue-agnostik, herhangi bir marka rengi). `tsc` 0; `verify-website-codegen.mjs` tüm bölümler OK (yeni D5 surface-guard + H2-COLOR/H2-BLOCK testleri). `lib/meta/*`,`lib/google/*` dokunulmadı.
- **Dosyalar:** `lib/website/codegen/designSystem.ts`, `lib/website/codegen/htmlGenerateShared.mjs`, `lib/website/codegen/designSystemValidate.mjs`, `scripts/verify-website-codegen.mjs`

## 2026-06-20 — Anasayfa: başlık akan gradyan animasyonu (voice_agent tagline tekniği)
- **Sorun:** Hero başlığındaki ("Hepsi Bir Arada Pazarlama Yönetimi") renkler sabit duruyordu; sahibi bir referans sitedeki "Satış ekibinizin yapay zeka gücü." tagline'ının akan-renk animasyonunu istedi (local kaynak: `voice_agent/src/components/layout/topbar.tsx` + `globals.css`).
...
## 2026-06-20 — Web Site Yöneticisi: "Modern" stil = animasyonlu/dinamik (kinetik runtime hook'ları)
- **Sorun:** Üretim sihirbazında "Modern" seçilince STATİK, düz, sade bir site çıkıyordu — sahibin asıl şikâyeti buydu. Kök neden: `ctx.style` her Opus prompt'una çıplak "modern" kelimesi olarak gidiyordu; `styleDirective(style)` (`theme.ts`) hiç çağrılmıyordu ve "modern" preset direktifi literal olarak "sade, gereksiz süsten kaçın" diyordu — hedefin TAM TERSİ. İstenen: wask.co (koyu zemin + neon enerji) / antso ferry (navy + pill + renkli yumuşak gölge) gibi HAREKETLİ / KİNETİK / DİNAMİK siteler (hareket eden başlıklar, değişen renkler, scroll ile beliriş, glow).
- **Çözüm:** (A) `modern` preset direktifi animasyonlu/kinetik/dinamik tanımıyla yeniden yazıldı (parlayan vurgu, animasyonlu gradyan, kinetik başlıklar, scroll-beliriş, hover mikro-etkileşim, cam/glow katmanlar; reduced-motion'da sakin); diğer presetler hareket tonuyla zenginleştirildi ama ayrıştı (minimal sakin kalır). `fontHint` modern→`space` (Space Grotesk + Inter). `buildCodegenContext` artık `styleDirective`'i hesaplayıp `ctx.styleDirective`'e koyuyor; dört enjeksiyon noktası (designSystem + plan + html single/block) çıplak kelime yerine zengin direktifi basıyor. (B) `buildHtmlSystemPrompt`'a stil-koşullu MODERN MOTION bloğu: `style==='modern'`'de hook'ları cömertçe kullan (yoğun staggered `data-dijimagic-reveal`, kinetik başlık, animasyonlu gradyan, count-up, hover lift/scale/glow, cam/glow); modern-dışı stiller sakin kalır. İzinli hook listesi yeni hook'larla genişletildi. (C) `public/dijimagic-site-runtime.js`'e üç self-contained, reduced-motion-korumalı, CSP-güvenli (eval yok), rAF/throttle/cap'li fonksiyon: `initTextRotate` (`data-dijimagic-text-rotate="A|B|C"` + `data-dijimagic-rotate-interval`), `initGradientMotion` (`data-dijimagic-gradient-anim` valueless — offscreen pause), `initCountUp` (`data-dijimagic-count-up` + `data-dijimagic-count-suffix`, IO one-shot). (D) `toDesignVars` artık `--dur-fast/--dur/--dur-slow` markalı zamanlama token'larını `:root`'a yazıyor (motion.durations önceden ölü). Güvenlik: sanitizer çekirdeği DEĞİŞMEDİ — yalnız valueless `data-dijimagic-gradient-anim` `allowedEmptyAttributes`'a eklendi (valued hook'lar zaten `data-*` glob'undan geçer); `<style>`/`<script>`/`on*` hâlâ imkânsız; AI hiç JS yazmaz. Verify'a yeni hook'ların sanitize'ı geçtiği (K1) + modern motion bloğu (H2c) + zengin direktif enjeksiyonu (H2d) + dur token'ları (H2e) eklendi. `lib/meta/*`,`lib/google/*` dokunulmadı.
- **Dosyalar:** `lib/website/render/theme.ts`, `lib/website/codegen/buildCodegenContext.ts`, `lib/website/codegen/types.ts`, `lib/website/codegen/designSystem.ts`, `lib/website/codegen/multipagePlanShared.mjs`, `lib/website/codegen/htmlGenerateShared.mjs`, `lib/website/codegen/sanitizeAllowlist.mjs`, `public/dijimagic-site-runtime.js`, `scripts/verify-website-codegen.mjs`

## 2026-06-20 — Web Site Yöneticisi: Düzenle keşfedilebilirliği + nav Türkçe imla + yeni-sekmede önizle
- **Sorun:** (a) Tıkla-seç manuel düzenleme yalnız `/onizleme` kontrol satırında küçük bir toggle'daydı; üretimden sonra sahibi DETAY sayfasında kalıyordu (orada sadece salt-okunur iframe önizleme var, düzenleme toggle'ı yok) → "Detaylı Önizle"ye tıklamadıkça düzenleme özelliğini hiç görmüyordu. (b) Üretilen sitenin nav etiketleri ASCII çıkabiliyordu ("Hakkimizda", "Iletisim") — plan prompt'u ascii kısıtını yalnız SLUG için koyuyordu ama navLabel/title için doğru Türkçe imlayı ZORUNLU kılmıyordu, kısıt navLabel'a sızıyordu. (c) Detaylı önizleme yalnız satır içi iframe'di; yeni sekmede açma seçeneği yoktu.
- **Çözüm:** (a) Detay sayfasına ("Detaylı Önizle" yanına) belirgin **"Bölümleri Düzenle"** (primary, `MousePointerClick`) butonu → `/web-site-yoneticisi/[id]/onizleme?edit=1`. Onizleme sayfası `useSearchParams` ile `?edit=1` okur ve `editMode`'u AÇIK başlatır → buton sahibi doğrudan düzenleme modu açık önizlemeye düşürür. Onizleme'deki mevcut toggle daha belirgin (daha büyük `font-semibold` + `ring`/`shadow`, kapalıyken `bg-primary/10 border-primary/40`) ve daha net etiketli ("Bölümleri Düzenle"). (b) Plan prompt'unda (`buildPlanSystemPrompt`) title+navLabel kuralı: site dilinde TAM, doğru imla zorunlu — Türkçe için ç,ğ,ı,İ,ö,ş,ü eksiksiz; ascii eşdeğeri ("Hakkimizda"/"Iletisim") YASAK; ascii-only kısıtı YALNIZ slug. `htmlGenerateShared.mjs` nav "VERBATIM" satırına da imla notu. Deterministik garanti: `validatePagePlan` BİLİNEN roller için (`about`→Hakkımızda, `services`→Hizmetler, `contact`→İletişim, … + en/de/fr/es) navLabel'ı kanonik etikete normalize eder; title yalnız boş/ascii-degrade ise normalize edilir; BİLİNMEYEN ('custom') roller AI etiketini korur. (c) Detay + onizleme kontrol satırına **"Yeni sekmede aç"** (`ExternalLink`) → `/website-preview/[id]?locale=…&slug=…` `target="_blank"` (düz önizleme, `?edit=1` yok). Yeni i18n: `editSections` + `openInNewTab` (tr+en). Verify'a MP5b (ascii→kanonik normalize) eklendi; üretim/önizleme/revize/tıkla-seç korunur; amber yok; `lib/meta/*`,`lib/google/*` dokunulmadı.
- **Dosyalar:** `app/web-site-yoneticisi/[id]/page.tsx`, `app/web-site-yoneticisi/[id]/onizleme/page.tsx`, `lib/website/codegen/multipagePlanShared.mjs`, `lib/website/codegen/htmlGenerateShared.mjs`, `scripts/verify-website-codegen.mjs`, `locales/tr.json`, `locales/en.json`

## 2026-06-20 — DijiAlgoritma: Hesap Sağlık yalnız aktif hesaplar + adset kartı sadeleştirme
- **Sorun:** (a) "Hesap Sağlık Durumu" kartları pasif/silinmiş reklam hesaplarının uyarılarını da gösteriyordu — tarama tarafı doğru (yalnız kayıtlı hesaplar) ama okuma tarafı flag'e (`DIJIMAGIC_PER_ACCOUNT_SCOPE`, default KAPALI) bağlıydı; kapalıyken hiçbir hesap filtresi uygulanmıyordu. (b) Ad set kartında çift tam-genişlik ~48px uppercase buton satırı (Onayla/Reddet + Geri/İleri) orantısız ve kötü görünüyordu. (c) Ad set popup'ı sayfanın üstüne yapışık açılıyordu (uzun sayfada ilk açılışta görünmüyor) + body scroll-lock yoktu. (d) FK zinciri için eklenen boş ad set kartları "%0 güven" rozetiyle render oluyordu.
- **Çözüm:** (a) `hierarchy/route.ts`'e KOŞULSUZ (flag'den bağımsız) hesap-uyarısı filtresi: `listRegisteredAccounts` (+ canlı seçili Meta/Google bağlantısı backfill olmayanlar için) ile O AN kayıtlı/aktif hesaplar çözülür; `account_id` dolu uyarı yalnız aktif kümede ise, legacy `account_id NULL` uyarı yalnız o platformun aktif hesabı varsa tutulur (`normalizeMetaAccountId`/`normalizeGoogleCustomerId`). Scope-ON dalı bu filtrelenmiş tabanı ayrıca seçili işletmeye daraltır. Ek (low-risk): `removeRegisteredAccount` artık çıkarılan hesabın pending uyarılarını `supersedePendingAccountAlerts` ile temizler (best-effort). (b) `HierCardActions`'a `compact` prop (py-2, normal-case, text-[12px]); `AdsetCard` footer kompakt — emerald Onayla / red Reddet karar satırı + sessiz slate Geri/emerald İleri navigasyon satırı (uppercase kaldırıldı). (c) `DrilldownModal` overlay `items-start`→`items-center`, panel `max-h-[92vh] overflow-y-auto` + body scroll-lock `useEffect`. (d) `AdsetCard`'da `confidence===0 && suggestions===0` → güven rozeti ve boş öneri bloğu gizlenir (kart kimliği/durumu korunur). Amber yok; mevcut i18n anahtarları yeniden kullanıldı (yeni key yok); Meta/Google çekirdeği (`lib/meta/*`,`lib/google/*`) dokunulmadı.
- **Dosyalar:** `app/api/dijimagic/improvements/hierarchy/route.ts`, `components/dijimagic/hierarchy/AdsetCard.tsx`, `components/dijimagic/hierarchy/HierCardActions.tsx`, `components/dijimagic/hierarchy/DrilldownModal.tsx`, `lib/account/registeredAccounts.ts`

## 2026-06-20 — Web Site Yöneticisi: tıkla-seç düzenlemede görsel değiştirme (yükle veya açıklama ile stok görsel)
- **Sorun:** Tıkla-seç düzenleme metin/AI-değiştir/sil yapıyordu ama bir GÖRSELİ değiştiremiyordu. İstenen: görsele tıkla → "Görseli değiştir" → ya dosya yükle ya da açıklamayla yeni bir stok görsel bul → o görseli değiştir. Basit kalmalı (sahibi aşırı-karmaşık builder'ı geri almıştı).
- **Çözüm:** Mevcut tıkla-seç + block-patch motoru genişletildi, yeni görsel altyapısı yok: (1) `public/dijimagic-select.js` artık tıklanan blokta `<img>` varsa `hasImage:true` + tıklanan görselin `image:{index,src}`'ini (blok içindeki img sırası) postMessage'a ekler; ara relay (`website-preview`) ve ebeveyn (onizleme) bu alanları aynı şekil doğrulamasıyla iletir/doğrular. (2) Onizleme paneline, seçimde görsel varsa "Görseli değiştir" aksiyonu + kompakt alt-panel: "Yükle" (gizli dosya inputu → `POST /api/website/[id]/assets`) veya "Açıklama ile bul" (kısa metin → `GET /api/website/[id]/stock-image?q=`); sonuç URL'i `replace_image` patch'ine gider, başarıda önizleme tazelenir, 402→AccessRequiredModal. (3) Yeni `assets` route logo desenini yeniden kullanır — owner-gated, yalnız raster MIME (png/jpeg/webp/gif; SVG reddedilir), ≤8MB + magic-byte kontrolü, Supabase Storage `website-assets`. (4) Yeni `stock-image` route mevcut `pickStockImage` çözümleyicisini sarar (owner-gated, tek https URL). (5) `patch` route'a `op:'replace_image'` ({targetId,imageIndex,newUrl}) eklendi; `applyImageReplacePatch` TAM deterministik (model yok): bloğu bul → `replaceBlockImageSrc` ile `imageIndex`'teki `<img>`'in src'sini byte-splice ile değiştir (alt + diğer her şey korunur) → merge → re-sanitize + re-gate. `newUrl` ÜÇ katmanda doğrulanır (route regex → `isSafeReplaceImageUrl` → sanitizer img allowlist); yalnız mutlak https kabul (javascript:/data:/relative reddedilir). Ücret = mevcut revizyon politikası (ilk 3 ücretsiz, owner bypass, tek ücret); gate-fail → iade + 422, persist yok. Verify'a RI1..RI5 eklendi (`replace-image OK`). tr+en; amber yok; iframe izolasyonu korunur (sandbox, postMessage doğrulamalı); `lib/meta/*`/`lib/google/*` dokunulmadı; yeni bağımlılık yok.
- **Dosyalar:** `public/dijimagic-select.js`, `app/website-preview/[id]/page.tsx`, `app/web-site-yoneticisi/[id]/onizleme/page.tsx`, `app/api/website/[id]/assets/route.ts`, `app/api/website/[id]/stock-image/route.ts`, `app/api/website/[id]/patch/route.ts`, `lib/website/codegen/applyBlockPatch.ts`, `lib/website/codegen/blockMap.mjs`, `scripts/verify-website-codegen.mjs`, `locales/tr.json`, `locales/en.json`

## 2026-06-20 — Web Site Yöneticisi: önizlemede tıkla-seç manuel düzenleme (metin / AI-değiştir / sil)
- **Sorun:** Önizlemede serbest revize (Reddet/Düzenle) vardı ama kullanıcı belirli bir bölümü işaret edip doğrudan düzenleyemiyordu. İstenen: mevcut sade önizlemenin üstüne HAFİF bir tıkla-seç + düzenle katmanı (workspace/komponent kütüphanesi DEĞİL).
- **Çözüm:** Üç parça, hepsi mevcut block-patch motorunu yeniden kullanır: (1) Yeni `public/dijimagic-select.js` — yalnız EDIT modunda (yeni `assembleDocument({ editMode:true })`, önizleme sayfası `?edit=1`) inline edilen küçük seçim overlay'i; üst-seviye `[data-dijimagic-block][data-dijimagic-id]` bloğa hover → ince outline, tık → `postMessage({type:'dijimagic:select',...})`. Serve (`/s/`) ve normal önizleme overlay'siz kalır. (2) Önizleme sayfasına (`onizleme/page.tsx`) "Düzenle" toggle'ı + `message` dinleyici (`e.source === iframe.contentWindow` + şekil doğrulaması) + seçili bloğun kompakt aksiyon kartı: "Metni düzenle" (textarea), "AI ile değiştir" (talimat), "Bölümü sil" (onay). (3) Yeni `POST /api/website/[id]/patch` — owner-gated, `op∈{edit,ai_rewrite,delete}` + `targetId ^b\d+$` doğrular; yeni `applyTargetedBlockPatch` ile planner'ı atlayıp seçili bloğu yeniden üretir/siler → birleştirilmiş gövde re-sanitize + re-gate; gate-fail → iade + 422 (persist yok). Ücret = mevcut revizyon politikası (ilk 3 ücretsiz, owner bypass, tek ücret). İç srcDoc iframe sandboxed kalır (allow-scripts allow-forms, no allow-same-origin); iki-seviye iframe için `website-preview` sayfası edit modunda yalnız doğrulanmış `dijimagic:select` mesajını üst kareye relay eder. tr+en eklendi; amber/sarı yok; `lib/meta/*` ve `lib/google/*` dokunulmadı.
- **Dosyalar:** `public/dijimagic-select.js`, `lib/website/codegen/assembleDocument.mjs`, `lib/website/codegen/assembleDocument.ts`, `lib/website/codegen/applyBlockPatch.ts`, `app/website-preview/[id]/page.tsx`, `app/web-site-yoneticisi/[id]/onizleme/page.tsx`, `app/api/website/[id]/patch/route.ts`, `scripts/verify-website-codegen.mjs`, `locales/tr.json`, `locales/en.json`

## 2026-06-18 — Web Site Yöneticisi (CRITICAL #3): iletişim formu action'ı sunucu sahiplenir — veri sızdırma açığı kapatıldı
- **Sorun:** İletişim formunun POST hedefi `data-dijimagic-form-action`'tan okunuyordu ve bu attribute sanitizer `form` allowlist'indeydi; `rewriteFormAction`'ın temizleme regex'i YALNIZ çift tırnaklı değeri eşliyordu (`/\sdata-dijimagic-form-action="[^"]*"/gi`). AI `data-dijimagic-form-action='https://evil'` (tek tırnak) veya `=https://evil` (tırnaksız) yazarsa: temizleme ıskalıyor, sunucu kendi güvenli action'ını İKİNCİ bir aynı-isimli attribute olarak ekliyor, sanitize-html İLKİNİ (AI'ın kötücülünü) tutuyordu → runtime ziyaretçinin ad/e-posta/telefon/mesajını saldırgan URL'sine `fetch`liyordu. Tek savunma CSP `connect-src 'self'` idi (tek nokta arıza). Ek kök neden: attribute'u sadece `form` allowlist'inden çıkarmak YETMİYOR — global `data-*` glob onu her etikette yeniden serbest bırakıyor.
- **Çözüm:** Sunucu action'ı TAM sahiplenir, AI ASLA yazamaz (kuşak+pantolon): (1) `data-dijimagic-form-action` `form` allowlist'inden çıkarıldı + global `*` `transformTags` handler'ına koşulsuz strip eklendi → her tırnaklama (tek/çift/tırnaksız), her etikette sanitize tarafından silinir (`data-dijimagic-form` işaretçisi ve diğer `data-dijimagic-*` hook'ları korunur). (2) `assembleDocument`'te pre-sanitize `rewriteFormAction` kaldırıldı; yerine POST-sanitize `injectFormAction(cleanHtml, actionPath)` geldi — temizlenmiş (güvenilir) gövdede cheerio ile her `[data-dijimagic-form]`'a sunucunun aynı-köken yolunu (ör. `/s/<sub>/lead`) tam olarak BİR kez ekler (idempotent; sıkı same-origin guard `//evil`/`/\evil`/mutlak URL reddeder); SERVE'de enjekte eder, PREVIEW'de etmez (runtime iyimser başarı, fetch yok); formsuz gövde aynen döner. (3) `renderGate` backstop: ham girişte same-origin olmayan herhangi bir `data-dijimagic-form-action` değeri (her tırnaklama) → `suspicious_form`; ayrıca post-sanitize'da non-`/` değer hayatta kalırsa (gelecekteki allowlist regresyonu) reddedilir. Verify'a CF-X1..X5 sızdırma iddiaları eklendi (tek tırnak/tırnaksız/duplicate hepsi nötralize; SERVE'de tek same-origin action; PREVIEW'de action yok; gate non-same-origin'i reddeder). Runtime/endpoint/prompt/CSP/Meta/Google dokunulmadı; yeni bağımlılık yok.
- **Dosyalar:** `lib/website/codegen/sanitizeAllowlist.mjs`, `lib/website/codegen/assembleDocument.mjs`, `lib/website/codegen/renderGate.mjs`, `scripts/verify-website-codegen.mjs`

## 2026-06-18 — Web Site Yöneticisi: üretilen sitelerde ÇALIŞAN iletişim formu → site sahibine e-posta
- **Sorun:** Üretilen sitelerin iletişim "formu" yalnız ETİKETLERDEN ibaretti — sanitizer `<input>`/`<textarea>` etiketlerini striplerken (yalnız `label`/`button` izinliydi; prompt "form yok" diyordu), ziyaretçi hiçbir şey yazamıyordu. Owner kararı: form gönderimleri **site sahibinin e-postasına** iletilsin (Resend; helper `notifySiteOwnerOfContact` zaten vardı).
- **Çözüm:** Deny-by-default sanitizer **minimum + sıkı** genişletildi: yalnız `form` + `input` + `textarea` eklendi; `<form>`'dan `action/method/target/onsubmit/formaction/name` striplenir (yalnız `data-dijimagic-form` + `data-dijimagic-form-action` + class/id/style/aria); `<input>` için `transformTags` **type ∈ {text,email,tel}** zorunlu kılar (password/file/hidden/image/submit/checkbox/… → `text`'e coerce); honeypot görünmez `type=text` input'tur. Runtime'a (`dijimagic-site-runtime.js`) `data-dijimagic-form` submit handler eklendi: honeypot kontrolü → sessiz bot drop, basit doğrulama (ad+e-posta+mesaj zorunlu, e-posta regex), `data-dijimagic-form-action`'a JSON POST → 2xx'te formu gizle + `data-dijimagic-form-success` göster; action yoksa (önizleme) **iyimser başarı** (fetch yok); fail-open, global sızıntı yok. Yeni public lead uç noktası `app/(sites)/s/[subdomain]/lead/route.ts` (force-dynamic, JSON parse + uzunluk sınırları + honeypot sessiz-drop + IP başına 5/dk in-memory hız sınırı + subdomain→user_id→`signups.email` sahip e-postası + `SITE_CSP`; teslim durumu public'e sızmaz). `assembleDocument` serve modunda her `[data-dijimagic-form]`'a `data-dijimagic-form-action="/s/<sub>/lead"` enjekte eder (önizlemede etmez). Prompt "form yok" direktifi **çalışan güvenli form** ile değiştirildi; gate (`renderGate`) hassas input (password/file/hidden/image) veya harici `action`/`formaction` taşıyan formu **`suspicious_form`** ile reddeder (güvenli iletişim formu geçer). Verify scripti `contact-form OK` ile yeni güvenlik iddialarını doğrular. CSP değişmedi (`connect-src 'self'` + `form-action 'self'`), AI `<script>` yok, yeni bağımlılık yok. NOT: e-posta yalnız `RESEND_API_KEY` set + sahibin `signups.email`'i varsa gider; yoksa form yine başarı gösterir (graceful), e-posta no-op.
- **Dosyalar:** `lib/website/codegen/sanitizeAllowlist.mjs`, `public/dijimagic-site-runtime.js`, `app/(sites)/s/[subdomain]/lead/route.ts`, `lib/website/store.ts`, `lib/website/codegen/assembleDocument.mjs`, `lib/website/codegen/assembleDocument.ts`, `app/(sites)/s/[subdomain]/route.ts`, `app/(sites)/s/[subdomain]/[slug]/route.ts`, `lib/website/codegen/htmlGenerateShared.mjs`, `lib/website/codegen/renderGate.mjs`, `scripts/verify-website-codegen.mjs`

## 2026-06-18 — Web Site Yöneticisi: tam-ekran kaydırmalı site galerisi (scroll-snap) + revize sonrası dinamik önizleme
- **Sorun:** Owner istedi: (A) oluşturulan site önizlemeleri büyük olsun, içerik alanını kaplasın (tam ekran hissi); (B) bir site revize edilince önizleme manuel yenileme olmadan dinamik güncellensin; (C) birden fazla site varken "sayfa kaydırır gibi" siteler tek tek, büyük paneller halinde gösterilsin.
- **Çözüm:** (A)+(C) `SiteList` 2 sütunlu kart ızgarasından **tek sütunlu, tam genişlik, dikey scroll-snap galerisine** dönüştürüldü: her site = `snap-start` + `min-h-[78vh]` BÜYÜK panel; sayfanın `app-content-surface` kabı sites varken `snap-y snap-mandatory scroll-smooth` olur → kaydırınca tek tek site geçilir. Panel içinde canlı anasayfa önizlemesi (`/website-thumb/[id]`) panel genişliğine ölçeklenip paneli doldurur; **izolasyon aynen korundu** (`sandbox="allow-scripts"` — allow-same-origin YOK, `pointer-events-none`, `aria-hidden`, `loading="lazy"`). Alt degrade üzerine site adı (`text-2xl font-bold`), sade-TR durum rozeti (amber yok), "Aç / Düzenle" rozeti ve Sil butonu yüzer; çoklu site için `siteCounter` (n/toplam) + zıplayan chevron ipucu. Staggered `animate-card-enter` + `--card-index` korundu. (B) `revise()` başarı dalında zaten `setReloadKey(k=>k+1)` ile iframe `key` değişip yeniden yükleniyordu (doğrulandı); ek olarak `approve()` (yayın) başarısına da `setReloadKey` eklendi. Önizleme tasarım yüksekliği `DESIGN_H` 820→960 yükseltilerek önizleme alanı büyütüldü (scale/scroll matematiği + cihaz toggle korundu).
- **Dosyalar:** `components/website/SiteList.tsx`, `app/web-site-yoneticisi/page.tsx`, `app/web-site-yoneticisi/[id]/onizleme/page.tsx`, `locales/tr.json`, `locales/en.json`

## 2026-06-18 — Web Site Yöneticisi önizleme: revize göstergesi büyük+ince + "Reddet" düzenlemeden ayrıştı
- **Sorun:** (1) "AI revize ediyor…" göstergesi (ikon + metin) küçük kalıyordu; owner daha büyük ama daha ince/zarif istedi. (2) "Reddet" butonu "Düzenle" ile aynı zorunlu-metinli geri bildirim panelini açıyordu (gönder butonu boş metinde pasif) → reddetme bir düzenlemeymiş gibi hissettiriyordu; oysa Reddet, mevcut tasarımı atıp sıfırdan yeni bir tasarım üreten ayrı bir aksiyon olmalı.
- **Çözüm:** (1) Önizleme overlay göstergesi ~%50 büyütüldü ve inceltildi: ikon `w-12 h-12` → `w-[4.5rem] h-[4.5rem]` (48→72px) + `strokeWidth={1.25}` (zarif çizgi); metin `text-base font-semibold` → `text-2xl font-light`; glow halo `-m-6` → `-m-9` ile orantılı büyütüldü; nabız animasyonu + reduced-motion guard korundu. (2) Reddet paneli artık ayrı: gönder butonu `disabled` Reddet için yalnız `working` (metin opsiyonel — boş metinle gönderilebilir), Düzenle için hâlâ `working || !feedback.trim()` (metin zorunlu). Reddet birincil buton etiketi yeni `rejectAction` anahtarı ("Reddet ve Yeniden Üret" / "Reject & Regenerate"); `rejectTitle`/`rejectHint`/`rejectPlaceholder` metni "bu tasarım silinir, sıfırdan yeni tasarım üretilir, not isteğe bağlı" şeklinde netleştirildi. Yönlendirme değişmedi: Reddet → `revise('reject')` (tam yeniden üretim, targetSlug yok), Düzenle → `revise('edit')` (cerrahi blok-patch).
- **Dosyalar:** `app/web-site-yoneticisi/[id]/onizleme/page.tsx`, `locales/tr.json`, `locales/en.json`

## 2026-06-18 — Web Site Yöneticisi: blok-patch `<main>` sarmalı veri-kaybı düzeltmesi (H Critical)
- **Sorun:** Tam-sayfa üreteci içerik bölümlerini tek bir `<main>` içine sarıyor (`<header data-dijimagic-id="b1">…</header><main><section data-dijimagic-id="b2">…</section>…</main><footer data-dijimagic-id="bN">…</footer>` — kendi prompt'unun zorunlu kıldığı yapı). Ama `extractBlocks` yalnız fragment kökünün DOĞRUDAN çocuklarını topluyordu → sadece `<header>` (b1) ve `<footer>` (bN) bulunuyordu; `<main>` içindeki TÜM `<section>`'lar GÖRÜNMEZdi. `mergeBlocks` gövdeyi bu eksik listeden YENİDEN birleştirdiğinden, geçerli bir hero/footer düzenlemesi gövdeyi yalnız `<header>…</header><footer>…</footer>`'a indiriyor → `<main>` ve TÜM içerik bölümleri sessizce siliniyordu. `renderGate` (bir h1, landmark, >40 bayt) enkazı GEÇİRİYOR, içi boşaltılmış sayfa "başarılı cerrahi düzenleme" olarak kalıcılaşıyordu. Doğrulama fixture'ı DÜZ gövde (no `<main>`) kullandığından `block-patch OK` hiçbir şey kanıtlamıyordu.
- **Çözüm:** (1) `extractBlocks` artık TÜM ağacı dolaşıp (`<main>` ve her sarmalayıcının içine inerek) `data-dijimagic-id` taşıyan HER öğeyi belge sırasıyla, tam `bodyHtml`'e göreli bayt aralığıyla (`start`/`end`, htmlparser2 indeksleri) toplar; her blok bayt-tam dilim (`html`). İç-içe id → yalnız EN ÜSTTEKİ blok sayılır (çift-sayma yok). (2) `mergeBlocks(originalBody, blocks, ops, newHtmlById)` ops'u liste yeniden-birleştirme yerine ORİJİNAL gövde string'i üzerinde BAYT-ARALIĞI SPLICE olarak uygular (azalan `at` sırasıyla) — `edit`/`delete`/`insert`/`move`. Op aralığı dışındaki HER ŞEY (`<main>` aç/kapa, boşluk, dokunulmamış bloklar) BAYT-İDENTİK kalır çünkü gövde hiç yeniden kurulmaz, yalnız splice edilir. (3) `applyBlockPatch`'e yapısal invaryant (savunma): birleşmeden sonra kalıcılaştırmadan önce — kaynaktaki her `data-dijimagic-id` (açık `delete` hedeflemedikçe) birleşmede mevcut; kaynaktaki üst-düzey sarmalayıcılar (`<main`/`<header`/`<footer`) korunmuş; `delete` yokken gövde kaynağın %50'sinden az değil. İhlal → `ok:false` → route tam-üretime düşer (zaten bağlı). (4) Doğrulama fixture'ı üretecin GERÇEK şekliyle (`<header b1><main><section b2><section b3></main><footer b4>`) yeniden yazıldı: extractBlocks 4 bloğu da (b2/b3 `<main>` içinde) bulur; hero (b1) düzenlemesi yalnız b1'i değiştirip `<main>`+b2+b3+b4'ü BAYT-İDENTİK korur; `<main>` içi b2 düzenlemesi yalnız b2'yi değiştirir; b3 silme yalnız b3'ü kaldırır; invaryant `<main>`/blok düşüren varsayımsal birleşmeyi yakalar.
- **Dosyalar:** `lib/website/codegen/blockMap.mjs`, `lib/website/codegen/applyBlockPatch.ts`, `lib/website/codegen/patchPlanner.ts`, `scripts/verify-website-codegen.mjs`

## 2026-06-18 — Web Site Yöneticisi: v2 revizyon "ilk 3 ücretsiz" (B) + prompt caching (D) + ISR/CDN bayrağı (E)
- **Sorun:** (B) Codegen v2 yolu (`generateWithCodegenV2`) HER revizyonda `computeGenerationCost` ile tam üretim maliyetini düşürüyordu → "ilk 3 revize ücretsiz, sonrası `WEBSITE_REVISION_COST`" politikasını yok sayıyordu (legacy yol doğru uyguluyordu). (D) Opus üretim çağrılarında prompt caching (`cache_control`) hiç yoktu; multipage'de N sayfa aynı sistem prompt'unu N kez tam ücretle işliyordu. (E) Yayınlanmış siteler her zaman `force-dynamic` ile servis ediliyordu (hiç CDN cache yok).
- **Çözüm:** (B) `generateWithCodegenV2` artık legacy yolla PARİTE: revizyonda `listVersions` ile `reason==='revision'` sayar → `usedRevisions < WEBSITE_FREE_REVISIONS ? 0 : WEBSITE_REVISION_COST`; ilk üretim `computeGenerationCost` ile DEĞİŞMEDİ. Charge-before + refund-on-fail + owner-bypass korundu. (D) `htmlGenerate.ts` Opus stream çağrılarında `system` artık içerik blokları dizisi: stabil önek bloğu (uzun anti-jenerik tasarım ethos'u — tüm sayfalarda BYTE-İDENTİK) `cache_control:{type:'ephemeral'}` taşır. Single-page → tek cache'li blok; multipage → [cache'li base ethos, sayfa-özgü ek (cache'siz)] → sayfa 2..N cache okur. Prompt İÇERİĞİ değişmedi (yalnız blok olarak sarıldı); temperature/budget_tokens yok. `buildMultipageSystemPrompt` artık `buildHtmlSystemPrompt` + yeni `buildMultipageSystemAppendix` birleşimi (concat byte-identik). (E) `WEBSITE_ISR` env bayrağı (default-OFF): `serveCommon.ts` `withSiteCacheHeaders` — bayrak '1' VE site immutable-published (`publishedVersionId` mevcut) ise yanıta `Cache-Control: public, s-maxage=300, stale-while-revalidate=86400` eklenir (header-tabanlı; `dynamic='force-dynamic'` export'una dokunulmadı → flag-off bugünkü davranışla byte-identik). Taslak/yayınlanmamış/preview yanıtları (ayrı route'lar) bayrak açık olsa bile cache ALMAZ; CSP/izolasyon korundu. Doğrulama: `node scripts/verify-website-credits.mjs` revize kuralını (used<3→0, used≥3→10) assert eder.
- **Dosyalar:** `app/api/website/[id]/generate/route.ts`, `lib/website/codegen/htmlGenerate.ts`, `lib/website/codegen/htmlGenerateShared.mjs`, `app/(sites)/s/[subdomain]/route.ts`, `app/(sites)/s/[subdomain]/[slug]/route.ts`, `lib/website/render/serveCommon.ts`, `scripts/verify-website-credits.mjs`

## 2026-06-17 — Web Site Yöneticisi: siteler listesi büyük canlı ana-sayfa önizleme kartları (#3)
- **Sorun:** Oluşturulan siteler listede zayıf, küçük kartlarda yalnız bir Globe ikonuyla görünüyordu; sitenin gerçek görünümü hiç yansımıyordu.
- **Çözüm:** Her kart artık sitenin **gerçek ANASAYFA'sını** canlı, küçültülmüş bir `<iframe>` önizlemesi olarak gösterir. (1) Yeni sahip-özel, chrome'suz Route Handler `app/website-thumb/[id]/route.ts` (`force-dynamic`, `getCurrentUser`+`getWebsite` ile sahiplik kontrolü) — TASLAK sayfaları `getPages` ile okur (yayın gerekmez, `/website-preview` ile aynı veri yolu) ve belge montajını TEK KAYNAKTAN tekrar kullanır: `format='html'` → `assembleDocument(mode:'serve')`, `format='sections'` → `renderSectionsDocument` (kopya yok, `/s/` ile birebir). Yanıta `/s/` ile AYNI `SITE_CSP` yazılır. Thumbnail'e özel hafif post-process (assembleDocument'e dokunmadan): `[data-dijimagic-reveal]` görünür yapılır + runtime script sökülür → küçük kutuda alt bölümler gizli kalmaz, motion thumbnail'i bozmaz. (2) `SiteList.tsx` yeniden tasarlandı: responsive büyük-kart grid (mobil 1 / lg 2 kolon), `max-w-7xl` sayfa konteyneri korunur, `animate-card-enter` + `--card-index` ile staggered reveal. Her kart 1280px masaüstü genişliğinde render edilip ResizeObserver ile kartın gerçek genişliğine `transform: scale()` ile sığdırılan 16/10 oranlı önizleme + site adı + sade TR durum rozeti (Taslak/Yayında/Yayından kaldırıldı — ham enum YOK, amber/sarı YOK) + "Aç / Düzenle" affordance taşır. Kartın kendisi tıklanabilir → detay (`/web-site-yoneticisi/[id]`), `hover:shadow-md`/`focus-visible`/`active` durumları; Sil butonu link dışında sağ üstte yüzer. İzolasyon: iframe `sandbox="allow-scripts"` (allow-same-origin YOK → opak origin), `pointer-events-none`, `aria-hidden`, `loading="lazy"`. i18n tr+en eklendi.
- **Dosyalar:** `app/website-thumb/[id]/route.ts` (yeni), `components/website/SiteList.tsx`, `locales/tr.json`, `locales/en.json`

## 2026-06-17 — Web Site Yöneticisi: üretim prompt'u frontend-design ilkeleriyle güçlendirildi (göz alıcı, ayırt edici çıktı)
- **Sorun:** Üretilen son-kullanıcı siteleri "çalışıyor" ama jenerik AI-şablon hissi taşıyabiliyordu — varsayılan tipografi, düz solid arka planlar, tek-düzlem gölge, timid renk. Prompt'un DESIGN ETHOS bölümü frontend-design felsefesini tam yansıtmıyordu.
- **Çözüm:** Üretim sistem prompt'unun DESIGN ETHOS'u **ek (additive)** olarak zenginleştirildi; HİÇBİR teknik kontrat gevşetilmedi. (1) `htmlGenerateShared.mjs` `buildHtmlSystemPrompt` ethos: ayırt edici TİPOGRAFİ (karakterli display + temiz body, tight tracking / relaxed leading, gerçek hiyerarşi), kararlı RENK (dominant `--accent` + keskin vurgular, full color freedom — varsayılan mavi/indigo yok), katmanlı DERİNLİK (base→raised→floating gölge merdiveni), ATMOSFER (gradient mesh + ince `data:image/svg+xml` grain — tek inline-style istisnası, renk yine sadece var() token), MOTION (tek orkestre `data-dijimagic-reveal` cascade + `data-dijimagic-delay` stagger, sadece transform/opacity, `transition-all` yok), SPATIAL kompozisyon (kasıtlı asimetri/overlap), her siteye ONE imza öğesi. IMAGES: `--gradient-overlay` ile editorial overlay + mix-blend. (2) `designSystem.ts` font/palette seçim felsefesi: karakterli display + temiz sans eşleştirme (Inter/Roboto/Arial display olarak YASAK), Google Fonts yüklenebilirlik şartı (headingHref tek geçerli `fonts.googleapis.com/css2` URL'i HER İKİ aileyi de yükler), dominant signature renk. **Korunan kontratlar (doğrulandı):** `var(--…)` token sözleşmesi, tüm `data-dijimagic-*` hook'ları (reveal/toggle/nav-toggle/smooth/mobile-nav/mobile-anim/href), tek `<h1>`, `{{IMG:}}`, NO script/inline-handler/form, compact py-10/12 default, mobil menü markup'ı, multipage data-dijimagic-href nav, referans direktifi (#7), revize kanalı (#9), untrusted karantina — davranış DEĞİŞMEDİ. Opus/Sonnet çağrılarında temperature/budget_tokens yok. `npm run build` → 0 TS hatası; `node scripts/verify-website-codegen.mjs` → tüm bölümler OK.
- **Dosyalar:** `lib/website/codegen/htmlGenerateShared.mjs`, `lib/website/codegen/designSystem.ts`

## 2026-06-17 — Web Site Yöneticisi: Veri Önceliği toggle (Referans Site / Manuel Veriler) — referans inşayı GERÇEKTEN yönlendirir (#7)
- **Sorun:** Sihirbaz referans URL'leri topluyordu ama hangi kaynağın **yetkili** olduğunu kullanıcı seçemiyordu. `buildCodegenContext` OTOMATİK karar veriyordu (referans içeriği varsa profili atla, yoksa global profile düş). Kullanıcı "siteyi referansa göre kur" diyemiyordu; özellikle referans içeriği zayıfsa istemeden global işletme profiline (başka bir firma olabilir) düşülebiliyordu.
- **Çözüm:** Uçtan uca **fonksiyonel** veri önceliği seçimi eklendi. (1) Tip: `ThemeTokens.dataSourcePriority?: 'reference' | 'manual' | null` (theme jsonb, migration yok, geriye dönük). (2) Sihirbaz: referans alanının altına WizardSelect ile "Veri Önceliği" — **Referans Site** / **Manuel Veriler**; varsayılan en az 1 referans URL varsa `reference`, yoksa `manual` (kullanıcı elle değiştirebilir). (3) Persist: seçim POST `/api/website` → `theme.dataSourcePriority`'ye yazılır (store spread'i otomatik taşır). (4) FONKSİYONEL dal — yeni saf `sourcePriority.mjs` `resolveSourceUsage(priority, hasRef)`: `'reference'` → referans yetkili, profil **HİÇ çekilmez** (içerik zayıf olsa bile, referans yoksa zarif not + crash yok); `'manual'` → profil yetkili, referans **enjekte edilmez**; `null/auto` → eski davranış (referans-varsa-yoksa-profil) **aynen**. (5) Referans gerçekten **multipage planı** + sayfa içeriğini yönlendirir: `buildReferenceDirective` TRUSTED yönergesi `buildPlanUserMessage` (sitemap) ve `buildHtmlUserMessage` (her sayfa) prompt'larına untrusted bloklardan ÖNCE basılır → "referans verisidir, benzer site kur; sayfa setini referansın yapısından türet" (referans içeriği yine karantinada, talimat olarak çalıştırılmaz). i18n tr+en eklendi; amber/sarı yok. verify'a SP1-SP5 + MP6b dalları eklendi → `source-priority OK`.
- **Dosyalar:** `lib/website/types.ts`, `lib/website/codegen/types.ts`, `lib/website/codegen/sourcePriority.mjs` (yeni), `lib/website/codegen/buildCodegenContext.ts`, `lib/website/codegen/multipagePlanShared.mjs`, `lib/website/codegen/htmlGenerateShared.mjs`, `components/website/CreateSiteWizard.tsx`, `locales/tr.json`, `locales/en.json`, `scripts/verify-website-codegen.mjs`

## 2026-06-17 — Web Site Yöneticisi: revize talimatı v2 motoruna bağlandı (#9) + revize UX iyileştirmeleri (#6, #8)
- **Sorun:** (#9) `WEBSITE_CODEGEN_V2='1'` açıkken "revize" hiçbir şey yapmıyordu — `generate/route.ts` erken `generateWithCodegenV2`'ye dallanıyor, o da `generateHtmlSite(userId, site)`'i **talimat parametresi olmadan** çağırıyordu; `buildCodegenContext` talimatı YALNIZ `theme.initialInstructions`'tan okuyordu. Kullanıcının revize metni (`instructions`) motora HİÇ ulaşmıyordu → her revize, orijinal kurulum talimatıyla siteyi yeniden üretip kullanıcının düzeltmesini yok sayıyordu (legacy yol `revisionMode`'u işliyordu; v2 regresyon). (#6) Revize başlayınca giriş kutusu (textarea+butonlar) istek uçarken görünür kalıyordu. (#8) "Revize ediliyor" göstergesi zayıftı (`Sparkles w-7 h-7` + `animate-pulse`).
- **Çözüm:** (#9) Revize talimatı uçtan uca bağlandı: route, parse edilmiş `instructions`+`revisionMode`'u `generateWithCodegenV2`'ye, oradan `generateHtmlSite(userId, site, { instructions, revisionMode })`'ya, oradan `buildCodegenContext(userId, website, opts)`'a aktarıyor. Bağlam çözümü: `instruction = clean(opts?.instructions) || clean(theme.initialInstructions)` — revize metni varsa orijinal kurulum talimatının **ÖNÜNE** geçer; boş/yok → ilk üretim davranışı **AYNEN** (initialInstructions). Tek `ctx` hem landing hem multipage akışına gittiği için talimat tasarım sistemi + her sayfanın üretim prompt'una ulaşır (`designSystem.ts`, `multipagePlanShared.mjs`, `htmlGenerateShared.mjs` → "trusted intent — prioritize this"). MVP semantiği: boş olmayan herhangi bir `instructions` = "bu geri bildirimi dahil ederek yeniden üret"; `revisionMode` ileri faz hedefli-patch için kabul ediliyor. Charge/refund/persist + `createVersion('revision')` DEĞİŞMEDİ. (#6) `revise()` başında `setBusy` ÖNCESİ `setPanel(null)` → giriş kutusu anında gizlenir. (#8) Gösterge büyütüldü: `Sparkles w-12 h-12 text-primary` + yeni `wsy-revising` keyframe (opacity .55→1, scale 1→1.18, 1.3s) + arka radyal emerald `wsy-revising-glow` halo; metin `text-base font-semibold text-primary`; `prefers-reduced-motion` guard'lı. Amber/sarı yok. `t('revising')` mevcut anahtar (yeni i18n gerekmedi).
- **Dosyalar:** `app/api/website/[id]/generate/route.ts`, `lib/website/codegen/generateHtmlSite.ts`, `lib/website/codegen/buildCodegenContext.ts`, `app/web-site-yoneticisi/[id]/onizleme/page.tsx`, `app/globals.css`

## 2026-06-17 — Web Site Yöneticisi (Kod-Üretim): ÇOKLU DİL — ek diller için yapı-koruyan metin çevirisi (locale başına sayfa)
- **Sorun:** Sihirbazda İngilizce (veya başka ek dil) seçilince site değişmiyordu — `generateHtmlSite` sayfaları **yalnız `website.defaultLocale`** ile persist ediyordu; ek dil sayfası hiç üretilmediği için dil değiştirme toggle'ı boş kalıyor / varsayılan dile düşüyordu. Servis (`serveCommon.pickLocale`/`findPageBySlug`/`findHomePage`) ve preview (`pages.filter(p=>p.locale===locale)`) zaten locale'e göre filtreliyordu; eksik olan tek şey locale başına persist edilmiş sayfalardı.
- **Çözüm:** Site **bir kez** varsayılan dilde üretiliyor (Opus — değişmedi); sonra her **ek dil** için her sayfanın **yapı-koruyan çevirisi** üretiliyor (tüm siteyi dil başına yeniden üretmek YOK — maliyet + drift önlendi). `translatePageHtml(html, from, to)` cheerio ile yalnız insan-okur **metin düğümlerini** + çevrilebilir **öznitelikleri** (`alt`/`title`/`aria-label`/`placeholder`) çıkarır, indeksler, **tek ucuz Sonnet çağrısı** (`claude-sonnet-4-6`, temperature/budget_tokens YOK; brandSynthesis kalıbı) ile çevirir; uzunluk eşleşmezse THROW. Yeniden ekleme indeks bazlı → tag/class/`data-dijimagic-*`/`href`/`src`/inline-style/{{IMG}}-çözülmüş URL'ler **birebir korunur**. Çevrilen HTML **re-gate** edilir; çeviri VEYA gate başarısızsa o sayfa+dil için **varsayılan dilin html'ine fall-back** (dil yine render olur, sadece çevrilmemiş) — bir çeviri kaçağı **tüm üretimi ASLA bozmaz**, loglanır. `seo.title`+`seo.description` de çevrilir (kısa, ayrı batch). Orchestrator hem **landing** hem **multipage** akışında ek-dil döngüsünü çalıştırır; varsayılan-dil üretimi **aynen**. v2 generate yolu artık `localeCount = website.locales.length` ile ücretlendiriliyor (sabit 1 yerine); charge-before + refund-on-fail aynen. Yeni bağımlılık YOK (cheerio zaten mevcut). verify'a MULTILANG bölümü eklendi (enjekte sahte translator: metin+alt/aria-label/title/placeholder çevriliyor; tag/class/data-dijimagic-*/href/src korunuyor; uzunluk-uyumsuzluğu→throw) → `multilang OK`.
- **Dosyalar:** `lib/website/codegen/translateHtml.mjs` (yeni — saf çekirdek), `lib/website/codegen/translateHtml.ts` (yeni — Sonnet sarmalayıcı), `lib/website/codegen/generateHtmlSite.ts`, `app/api/website/[id]/generate/route.ts`, `scripts/verify-website-codegen.mjs`

## 2026-06-17 — Web Site Yöneticisi (Kod-Üretim): nav rewrite slug-set-aware (liste-dışı slug → anasayfa, 404 önlendi) + v2 ölü cost pre-compute kaldırıldı
- **Sorun:** (1) `rewriteNavLinks`/`resolveNavHref` yalnız slug'un ŞEKLİNİ (url-safe) doğruluyordu, gerçek bir sayfa olup olmadığını değil. Model planlanmamış bir slug üretirse (örn. `data-dijimagic-href="blog"` ama `blog` sayfası yokken) `/s/<sub>/blog`'a çözülüp **404** veriyordu. (2) `generate/route.ts` ana POST'u `cost`/`pageCount`'u hesaplıyordu ama `WEBSITE_CODEGEN_V2='1'` iken bunları kullanmadan erken return ediyordu (v2 kendi cost'unu hesaplar) — ölü compute.
- **Çözüm:** (1) Nav rewrite **slug-set-aware** yapıldı: `assembleDocument`/`resolveNavHref`/`rewriteNavLinks` opsiyonel `knownSlugs: string[]` alıyor (sitenin GERÇEK sayfa slug'ları, `home` dahil). Bir slug yalnızca **hem url-safe HEM de knownSlugs içindeyse** kendi path'ine çözülür; url-safe ama liste-dışı slug → **anasayfa base'i** (404 yerine anasayfaya gider); enjeksiyon (`javascript:`/`../`) yine base'e düşer. `knownSlugs` verilmezse eski şekil-bazlı davranış korunur (geriye dönük uyumlu). Threading: serve home + slug route'ları `collectKnownSlugs(site)` (yeni helper, `serveCommon.ts`), preview sayfası `pages` listesinden hesaplıyor; landing → data-dijimagic-href yok → no-op. (2) v2 yolunda ölü cost/pageCount pre-compute kaldırıldı — hesap v2 erken-return'ünden SONRA, yalnız legacy yol için yapılıyor. Ücret/iade davranışı DEĞİŞMEDİ. verify'a MP11+MP12 eklendi (known→path, liste-dışı→anasayfa base, enjeksiyon→base, back-compat); `multipage OK`.
- **Dosyalar:** `lib/website/codegen/assembleDocument.mjs`, `lib/website/codegen/assembleDocument.ts`, `lib/website/render/serveCommon.ts`, `app/(sites)/s/[subdomain]/route.ts`, `app/(sites)/s/[subdomain]/[slug]/route.ts`, `app/website-preview/[id]/page.tsx`, `app/api/website/[id]/generate/route.ts`, `scripts/verify-website-codegen.mjs`

## 2026-06-17 — Web Site Yöneticisi (Kod-Üretim): GERÇEK multipage — ayrı sayfalar + sayfa-özel içerik + nav routing (data-dijimagic-href)
- **Sorun:** "Çok sayfalı" (multipage) seçilen siteler aslında **tek bir landing sayfası** üretiyordu; navigasyon ayrı sayfalara değil, aynı sayfadaki `#anchor`'lara gidiyordu. `generateHtmlSite` `siteType`'ı yok sayıp her zaman tek anasayfa üretiyordu (asıl bug).
- **Çözüm:** `generateHtmlSite` `siteType`'a göre dallanıyor. **landing** → mevcut tek-sayfa akışı AYNEN (regresyon yok). **multipage** → (1) **Sayfa planlama** (tek Opus çağrısı, `multipagePlan.ts` + saf `multipagePlanShared.mjs`): markaya özel 3–6 sayfalık site haritası (`{slug,title,navLabel,role,purpose}`); `validatePagePlan` ile url-safe/unique slug, home ilk + İletişim zorunlu, 6 cap, geçersiz slug coerce/dedupe, garbage/null → deterministik varsayılan plan. (2) **DesignSystem bir kez** üretilip tüm sayfalarda paylaşılıyor. (3) **Sayfa başına üretim** (`generatePageHtml`): her sayfa KENDİ amacına özel içerik + tüm sayfaları gösteren PAYLAŞILAN header/footer nav (`<a data-dijimagic-href="<slug>">`); tek `<h1>` = sayfa başlığı; aktif sayfa `aria-current="page"`; sayfa-içi `#anchor` yalnız o sayfanın bölümleri için. Var kontratı/spacing/mobil menü/{{IMG:}}/script-form yasağı/renk özgürlüğü AYNEN. (4) **Sayfa başına gate** + 1 self-repair; **all-or-nothing**: bir sayfa bile geçemezse tüm üretim fail (route iade) → eksik sayfaya nav linki ASLA çıkmaz; hangi sayfanın düştüğü loglanır. (5) `WebsitePageInput[]` döner; route `replacePages(user.id, id, result.pages)` ile hepsini yazar. (6) **Nav link rewrite** (`assembleDocument`): `data-dijimagic-href="<slug>"` → gerçek `href`; serve `linkBase=/s/<sub>` (path: home→base, slug→base/slug), preview `linkBase=/website-preview/<id>` (query: `?slug=...&locale=...` → iframe yeniden render, dashboard 404 olmaz). Yalnız doğrulanmış url-safe slug href'e girer (enjeksiyon yok); sanitize-öncesi rewrite → iç href sanitizer'dan geçer. **Custom domain:** kök base (`linkBase=''`) `resolveNavHref`'te zaten destekli (home→`/`, slug→`/x`); custom-domain servis route'u bağlanınca yalnız doğru `linkBase` geçmesi yeter — modülde TODO. verify'a multipage bölümü eklendi (`multipage OK`).
- **Dosyalar:** `lib/website/codegen/generateHtmlSite.ts`, `lib/website/codegen/multipagePlan.ts` (yeni), `lib/website/codegen/multipagePlanShared.mjs` (yeni), `lib/website/codegen/htmlGenerate.ts`, `lib/website/codegen/htmlGenerateShared.mjs`, `lib/website/codegen/assembleDocument.ts`, `lib/website/codegen/assembleDocument.mjs`, `app/api/website/[id]/generate/route.ts`, `app/(sites)/s/[subdomain]/route.ts`, `app/(sites)/s/[subdomain]/[slug]/route.ts`, `app/website-preview/[id]/page.tsx`, `scripts/verify-website-codegen.mjs`

## 2026-06-17 — Web Site Yöneticisi (Kod-Üretim): sihirbazda mobil menü animasyon seçimi (soldan/sağdan/yukarıdan)
- **Sorun:** Üretilen sitelerin mobil menü açılış animasyonu (perde yönü) üretim prompt'unda sabit `data-dijimagic-mobile-anim="left"` olarak gömülüydü; kullanıcı seçemiyordu. Runtime 3 yönü (left/right/top) zaten destekliyordu — seçim uçtan uca bağlanmamıştı.
- **Çözüm:** Seçim sihirbazdan üretime kadar bağlandı. (1) `CreateSiteWizard` içine kanonik `WizardSelect` ile "Mobil menü nasıl açılsın?" alanı eklendi (Soldan/Sağdan/Yukarıdan, default `left`), seçim POST `/api/website` payload'ında `theme.mobileMenuAnim` olarak gönderiliyor. (2) `ThemeTokens.mobileMenuAnim?: 'left'|'right'|'top'` (jsonb, migration yok, opsiyonel → eski siteler `left`). (3) `buildCodegenContext` `theme.mobileMenuAnim`'i `coerceMobileMenuAnim` ile okuyup (geçersiz/yok → `left`) `CodegenContext.mobileMenuAnim`'e koyuyor. (4) `buildHtmlSystemPrompt(ctx)` artık `ctx.mobileMenuAnim`'i alıp panelde `data-dijimagic-mobile-anim="<value>"` olarak basıyor (yön ipucu + overlay kenar yönergesi de seçime göre); arg yok/geçersiz → `left` (geriye dönük uyumlu; verify'ın arg'sız çağrısı bozulmaz). htmlGenerate hem ilk üretim hem self-repair'de `coreBuildHtmlSystemPrompt(ctx)`'e ctx geçiriyor. i18n: tr+en aynı key path (`mobileMenuAnimLabel/Hint/Left/Right/Top`). verify'a H2b regresyon testi eklendi (default + invalid coerce + 3 geçerli değer).
- **Dosyalar:** `components/website/CreateSiteWizard.tsx`, `lib/website/types.ts`, `lib/website/codegen/types.ts`, `lib/website/codegen/buildCodegenContext.ts`, `lib/website/codegen/htmlGenerateShared.mjs`, `lib/website/codegen/htmlGenerate.ts`, `scripts/verify-website-codegen.mjs`, `locales/tr.json`, `locales/en.json`

## 2026-06-17 — Web Site Yöneticisi (Kod-Üretim): mobil menü Tailwind-proof inline-stil slide toggle + 3 perde animasyonu
- **Sorun:** Üretilen sitelerde mobil hamburger menü (yalnız mobilde) **açık geliyordu ve ikon hiçbir şey yapmıyordu** (toggle çalışmıyordu). Kök neden: runtime `hidden` attribute / `is-open` class ile gizliyordu, ama panel Tailwind display utility'si (`flex`/`block`) de taşıyordu → `.flex{display:flex}` kuralı `[hidden]{display:none}` UA kuralını **eziyor**, panel görünür kalıyordu. Attribute/class ile gizleme Tailwind display utility'sini güvenilir biçimde yenemiyor.
- **Çözüm:** Runtime'ın mobil-nav bölümü baştan yazıldı. `[data-dijimagic-mobile-nav]` panelleri yalnızca runtime'ın sahip olduğu **INLINE stiller** (en yüksek özgüllük → her display class'ını yener) ile gizlenip gösteriliyor: `transition: transform .32s cubic-bezier(.4,0,.2,1), opacity .32s`, kapalı durum `data-dijimagic-mobile-anim` yönüne göre off-screen transform (left → translateX(-100%), right → translateX(100%), top → translateY(-100%)) + `opacity:0; pointer-events:none; aria-hidden="true"`. Init'te her panel KAPALI inline duruma zorlanır (yükte güvenilir gizli). Hamburger tıklaması (delegated) toggle eder; açık = translate(0,0) + opacity:1 + pointer-events:auto + aria-hidden=false + aria-expanded=true. İçerideki bir linke tıklayınca + Escape ile kapanır. `prefers-reduced-motion`: slide atlanır, görünürlük anında değişir. `hidden` artık mobil panelde KULLANILMIYOR (eski markup için legacy class/hidden fallback korundu). Reveal / generic toggle / smooth-scroll davranışları aynen korundu; sızan global yok, eksik öğede throw yok (fail-open). Üretim prompt'u (`buildHtmlSystemPrompt`) yeni markup contract'ı emitedecek şekilde güncellendi: hamburger `data-dijimagic-nav-toggle="mobilenav"` + panel `<nav id="mobilenav" data-dijimagic-mobile-nav data-dijimagic-mobile-anim="left">` (default left); panel fixed/absolute overlay olarak AI tarafından stillenir, runtime aç/kapa + slide'ı yönetir; mobil paneli `hidden` ile gizleme yasaklandı. Sanitize: `data-dijimagic-mobile-nav` + `data-dijimagic-mobile-anim` (`data-*` glob) ve aria-controls/aria-expanded/aria-label/aria-hidden (aria-* allowlist) hayatta kalıyor — verify'a regresyon testi (r12) eklendi.
- **Dosyalar:** `public/dijimagic-site-runtime.js`, `lib/website/codegen/htmlGenerateShared.mjs`, `lib/website/codegen/sanitizeAllowlist.mjs` (yorumlar), `scripts/verify-website-codegen.mjs`
...
- **Durum:** Kod tamam + bağımsız review'lı + build yeşil + saf-mantık verify (sanitize/tailwind/assemble/gate/context/designsystem/htmlgen/orchestrator) 8/8 geçer. Canlı uçtan-uca üretim + ekran görüntüsü, `ANTHROPIC_API_KEY` + `WEBSITE_CODEGEN_V2=1` ile **kullanıcı ortamında** doğrulanacak (bu izole ortamda API anahtarı/tarayıcı yok). Faz 2 (çok-sayfa+dil) ve Faz 3 (chat-edit + ISR + görsel cilası) ayrı planlar.
- **Dosyalar:** `lib/website/codegen/*` (buildCodegenContext, designSystem, htmlGenerate, generateHtmlSite, sanitize, tailwindCompile, assembleDocument, renderGate + .mjs çekirdekler), `app/(sites)/s/[subdomain]/route.ts` + `[slug]/route.ts`, `lib/website/render/{HtmlSiteRenderer,serveCommon,serveSectionsDocument,designVars}.ts`, `public/dijimagic-site-runtime.js`, `app/api/website/[id]/generate/route.ts`, `app/website-preview/[id]/page.tsx`, `next.config.mjs` (CSP), `locales/{tr,en}.json`, `supabase/migrations/20260617120000_website_html_format.sql`
...
- **Çözüm:**
  - **Tek-ekran wizard** (components/website/CreateSiteWizard.tsx): tam-sayfa, arka plan blur, çerçevede dönen yeşil "yılan" shimmer (globals.css `.wizard-snake-border`, @property+conic). Ad/tür/dil/font/referans + marka açıklaması (tarif) + logo + Sesle yaz, hepsi tek ekranda. "AI ile Oluştur" → taslak + logo + üretim → detay sayfasına (`?create=ai`) autostart. Eski iki-adımlı NewSiteModal kaldırıldı. Boş-durum animasyonu (WebsiteBuilderAnimation) korundu.
  - **Firecrawl referans tarama** (lib/website/referenceScanner.ts): `isFirecrawlReady()` ise `firecrawlScrape` (JS-render markdown) + layout sinyalleri; key/flag yoksa Cheerio'ya düşer (sıfır kesinti). Cheerio yolu da header/nav, footer, bölüm akışı, tema rengi çıkaracak şekilde zenginleştirildi. AI prompt'una yapı/layout vurgusu ("referansların düzenini yaklaştır; kullanıcı isteğini öncele").
  - **Ayrı önizleme/review sayfası** (app/web-site-yoneticisi/[id]/onizleme/page.tsx): masaüstü/tablet/mobil toggle + büyük ölçekli iframe + **Onayla** (→ yayınla) / **Reddet** / **Düzenle**. Detay sayfasında "Detaylı Önizle" ikonu.
  - **Reddet=radikal, Düzenle=cerrahi** (lib/website/ai/generate.ts `revisionMode` + lib/website/revisionContext.ts): Reddet → "tamamen farklı yön"; Düzenle → mevcut içerik özeti AI'a verilir, yalnız istenen yerler değişir, gerisi korunur.
  - **Kredi:** ilk üretim kredili + **3 ücretsiz revize**, sonrası WEBSITE_REVISION_COST. Mevcut sayfa varsa her yeniden üretim revizedir (boş talimatla kredi atlatma kapalı). Owner bypass korundu. Kredi script'i revize senaryolarıyla güncellendi.
  - **Adversarial review** (5 boyut, 31 ham → 15 doğrulanmış bulgu) sonrası düzeltildi: kritik kredi-bypass, 402 panel reset, edit modunda yeni-dil özet, i18n cihaz/tooltip etiketleri, çoklu-dil revize uyarısı, Firecrawl fail logu, review loading state, publish toast.
- **Dosyalar:** components/website/CreateSiteWizard.tsx (yeni), app/web-site-yoneticisi/{page.tsx,[id]/page.tsx,[id]/onizleme/page.tsx (yeni)}, lib/website/{referenceScanner.ts,ai/generate.ts,revisionContext.ts (yeni),credits.ts,types.ts}, app/api/website/[id]/generate/route.ts, app/globals.css, components/website/NewSiteModal.tsx (silindi), scripts/verify-website-credits.mjs, locales/{tr,en}.json, docs/superpowers/specs/2026-06-16-web-site-yoneticisi-wizard-review-akisi.md
...
- **Çözüm (5 paralel keşif workflow'u ile her yer haritalandı → tek tutarlı düzeltme):**
  ...
  - Diğer modüller (DijiAlgoritma/Optimizasyon/Hedef Kitle/Raporlar) aktif hesabı aynı cookie/bağlantı context'inden çözdüğü için context temizlenince **hepsinden bağ otomatik kalkar** (ayrı state yok — workflow doğruladı).
...
- **Doğrulama:** tsc 0 hata, next build EXIT=0.
- **Dosyalar:** lib/dijimagic/watchdog/{metaWatchdog,googleWatchdog}.ts, components/google/GoogleAccountDropdown.tsx, app/api/integrations/google-ads/{accounts,children,select-account,selected}/route.ts, app/api/marketing-setup/google-ads-accounts/route.ts

## 2026-06-15 — 🆕 Erken Uyarı: günlük reklam nöbetçisi (dijialgoritma alt sekmesi)
- **İstek:** Açılan tüm Meta/Google reklamlarını düzenli takip eden, para sızıntısı/hesap askısı/reddedilen-çalışmayan reklamı yakalayıp uyaran bir sistem. dijialgoritma'nın haftalık AI taramasından ayrı, **günlük + deterministik (LLM'siz = maliyetsiz)** bir nöbetçi.
- **Çözüm:** `lib/dijimagic/watchdog/` modülü — bağlı token'ın eriştiği tüm hesapları otomatik keşfeder (aktivite filtresi: yalnız son 30g harcayanlar), durum + aktif kampanya/reklam metriklerini salt-okuma çeker, 10 deterministik kural ile acil/bozulma tespit eder (hesap askısı/ödeme, reddedilen reklam, aktif-ama-çalışmayan, harcama-var-dönüşüm-yok, ROAS<1, CPA sıçraması, düşük teslimat, yüksek frekans, IS kaybı), `account_alerts`'e `wd_*` yazar (AI uyarılarına dokunmadan) + Resend ile günlük e-posta özeti. Günlük cron `0 5 * * *` (`WATCHDOG_ENABLED` flag, kapalıyken no-op). UI: dijialgoritma TEK sayfasında, reklam önerilerinin altında "Erken Uyarı" bölümü + "Şimdi tara" (ayrı sekme/sayfa AÇILMADI — owner kararı: tek sayfa). **Migration gerekmez** (alert_type serbest metin). Eşikler optimizasyon motoruyla tutarlı, para-birimi duyarlı.
- **Doğrulama:** `tsc --noEmit` 0 hata; `next build` EXIT=0. Canlıya alınmadı — deploy + `WATCHDOG_ENABLED=true` owner onayı bekliyor.
- **Dosyalar:** lib/dijimagic/watchdog/{types,rules,metaWatchdog,googleWatchdog,notify,runWatchdog}.ts, lib/dijimagic/featureFlag.ts, app/api/cron/erken-uyari/route.ts, app/api/dijimagic/watchdog/route.ts, app/dijialgoritma/page.tsx (Erken Uyarı bölümü, önerilerin altında), components/dijimagic/ErkenUyariSection.tsx, vercel.json, docs/superpowers/specs/2026-06-15-erken-uyari-gunluk-nobetci.md
...
- **Çözüm:**
  ...
  - `CookieConsent` path guard: `/s/`, `/website-preview`'da gizlenir (müşteri sitesine DijiMagic çerez banner'ı sızmaz).
...
- **Sorun (uçtan uca doğrulamada bulundu — İKİ ayrı CHECK):**
  1. `ai_engine_runs.platform` CHECK yalnız `('Meta','Google')` kabul ediyordu; `writeHierRunStatus` `platform='dijialgoritma_hier'` yazar → **23514 ile sessizce reddediliyordu** (try/catch yutuyordu).
...
- **Çözüm:** Migration `20260614000000_widen_ai_engine_runs_platform.sql` — platform `+'dijialgoritma_hier'`, status `+'partial'` (additive/güvenli; mevcut satırlar zaten uyumlu). **omddq'ya uygulandı + doğrulandı.**
- **Uçtan uca doğrulama:** (a) sentinel failed satır → `check_dijialgoritma_runs` 🔴 KIRMIZI ("son 8 günde 1 BAŞARISIZ koşu") → temizlik → normal. (b) `partial`+`dijialgoritma_hier` UPSERT (gerçek onConflict kolonlarıyla) başarılı. `idx_ai_engine_runs_unique (user_id,platform,account_id,run_date)` index'i mevcut (doğrulandı).
- **Dosyalar:** supabase/migrations/20260614000000_widen_ai_engine_runs_platform.sql

## 2026-06-14 — DijiAlgoritma güvenilirlik R9: lifecycle/persist sağlamlık
...
- **Dosyalar:** lib/dijimagic/ai/hierarchicalStore.ts, inngest/functions/perCampaignImprovements.ts

## 2026-06-14 — DijiAlgoritma güvenilirlik R7: eşzamanlılık + izlenebilirlik + eski akışı kapat
- **Sorun:** (a) Aynı kullanıcı için iki koşu çakışırsa (cron + on-demand) supersede/insert yarışı + ÇİFT Anthropic batch maliyeti olurdu. (b) Kartlar hangi batch'ten geldiği izlenemiyordu (run_id hep null). (c) Orphan uç `/api/dijimagic/improvements/scan` hâlâ eski per-ad akışını (ai_ad_improvements) tetikliyordu → açılırsa paralel eski kartlar + maliyet.
- **Çözüm:**
  ...
  - **Eski per-ad akışı kapatıldı:** `/api/dijimagic/improvements/scan` artık hiyerarşik event (`campaign-improvements.user`) fırlatır; `dijialgoritma/improvements.user`'ı hiçbir yol tetiklemiyor (function rollback için kayıtlı kalır).
- **Dosyalar:** inngest/functions/perCampaignImprovements.ts, app/api/dijimagic/improvements/scan/route.ts

## 2026-06-14 — DijiAlgoritma güvenilirlik R6: fetch paritesi (doğru kampanyalar + alt-ağaç eşleşmesi)
...
- **Dosyalar:** lib/dijimagic/googleDeepFetcher.ts, lib/dijimagic/metaDeepFetcher.ts

## 2026-06-14 — DijiAlgoritma güvenilirlik R4: atomiklik (kart kaybını önle + retry'de duplike yok)
...
## 2026-06-14 — DijiAlgoritma güvenilirlik R8: gözlemlenebilirlik döngüsünü kapat (failed satırları BİRİ OKUSUN)
- **Sorun:** R1 `ai_engine_runs`'a `dijialgoritma_hier` failed/stale satırları yazıyordu ama bunları okuyan/uyarı veren kimse yoktu — yazılan iz bir hafta görülmeden kalabilirdi (asıl hastalık SESSİZ HATA'nın diğer yarısı).
- **Çözüm:**
  - **Günlük sağlık e-postası** (`saglik_kontrol.py`): yeni `check_dijialgoritma_runs` — Supabase REST (service-role token YERELDE) ile son ~8 günde `failed`, stale `running` veya 8+ gündür başarı yoksa **🔴 kırmızı**; sağlıklıysa son başarılı koşu tarihi. Çalışan launchd kopyası (`~/.dijimagic-saglik-automation/`) senkronlandı.
  - **Admin Gözetim Merkezi:** `GET /api/admin/gozetim-merkezi` artık `dijialgoritmaHealth` döner (son 14 gün; failed/running/completed sayımı + son başarısızlıklar + `isUnhealthy`). Panelde KPI üstünde kırmızı/yeşil sağlık banner'ı (EN/TR i18n).
  - **Cron sayımı:** `VERCEL_CRONS` listesine `dijimagic-outcome-snapshots` eklendi (9→10; vercel.json ile uyum).
...
## 2026-06-14 — DijiAlgoritma güvenilirlik R1: sessizliği kır (run-status + failed izi)
...
- **Çözüm (R1 + key guard + parse):**
  - `writeHierRunStatus` (ai_engine_runs, platform='dijialgoritma_hier', migration'sız): koşu başında `running`, sonunda `completed`/`partial`/`failed` + sayım notu. Stale-`running` da başarısızlık sinyali.
...
- **Dosyalar:** lib/dijimagic/ai/systemPrompt.ts (capCacheBreakpoints + SysBlock), lib/dijimagic/ai/perCampaignPrompt.ts, lib/dijimagic/ai/agent.ts, lib/dijimagic/ai/perCampaignAgent.ts, src/tests/dijialgoritmaPlatformRules.test.ts

## 2026-06-14 — DijiAlgoritma "Yayınla" = CANLI yayın (PAUSED değil) — kullanıcı gereksinimi
- **Sorun:** DijiAlgoritma'nın ürün vizyonu: kullanıcı bir reklam kartını onaylayıp "Yayınla" dediğinde reklam ilgili platformda **eş zamanlı CANLI** yayınlanır. Faz 1'de güvenlik gerekçesiyle PAUSED yapılmıştı — bu, kullanıcının açık niyetine (tek-tık canlı yayın) AYKIRIYDI.
- **Çözüm:** DijiAlgoritma onay→yayın akışı artık **ACTIVE/ENABLED** (CANLI) yayınlar:
...
- **Dosyalar:** lib/dijimagic/meta/orchestrator.ts, app/api/dijimagic/create-ad/route.ts

## 2026-06-14 — DijiAlgoritma: scope'lu sayaç tutarlılığı + "kart yok" boş durumu (çoklu işletme)
...
- **Çözüm:**
  - `getHierarchyCounts(userId, scope?)` artık per-account scope açıkken seçili işletmenin kampanya ID'leri + hesabına göre sayar (kart listesiyle tutarlı). command-center scope'u çalıştırma blob'undan + `resolveDijiMagicScope`'tan kurar.
...
- **Dosyalar:** lib/dijimagic/ai/hierarchicalStore.ts, app/api/dijimagic/command-center/route.ts, components/dijimagic/hierarchy/HierarchicalImprovements.tsx, locales/{tr,en}.json

## 2026-06-14 — DijiAlgoritma denetim onarımı 4/N: adversarial review bulguları (4 onaylı)
...
- **Dosyalar:** app/api/dijimagic/create-ad/route.ts, lib/dijimagic/ai/campaignMetricsById.ts, components/dijimagic/hierarchy/AdCard.tsx, lib/dijimagic/ai/hierarchicalStore.ts, lib/dijimagic/resultTrackingStore.ts, app/api/dijimagic/improvements/hierarchy/route.ts

## 2026-06-14 — DijiAlgoritma denetim onarımı 3/N: Meta dönüşüm sayımı + tarama görünürlüğü
...
- **Çözüm:**
  - **Dönüşüm:** `lib/dijimagic/metaConversions.ts` (`countMetaConversions`) — her dönüşüm ailesinde birleşik değeri tercih eder (`??`, toplama yok → çift-sayım giderildi), aileler arası standart event'leri toplar (satın alma/lead/kayıt/abonelik/iletişim/başvuru/mesajlaşma). `metaDeepFetcher` + `campaignMetricsById` tek kaynağı kullanır.
...
- **Dosyalar:** lib/dijimagic/metaConversions.ts (yeni), lib/dijimagic/metaDeepFetcher.ts, lib/dijimagic/ai/campaignMetricsById.ts, lib/dijimagic/googleDeepFetcher.ts

## 2026-06-14 — DijiAlgoritma denetim onarımı 2/N: outcome ölçüm döngüsü + kart şeffaflığı
...
- **Çözüm:**
  - **Ölçüm döngüsü:** `fetchCampaignMetricsById` (PAUSED dahil ID ile canlı metrik — Meta insights / Google GAQL). Hiyerarşik yayında (`applied`) kaynak kampanyanın "before" metrikleri kaydedilir + yeni kampanya ID'si saklanır. Yeni cron `/api/cron/dijimagic-outcome-snapshots` (günlük) penceresi dolan kayıtlarda yeni kampanyayı ölçüp `recordAfterSnapshot` ile delta+outcome üretir; yeni kampanya veri üretmediyse grace (pencere×2) sonra `insufficient_data` (sahte veri YOK).
  - **Kartta gösterim:** Hiyerarşi GET'i `dijimagic_recommendation_results`'ı `proposal_id` ile eşleyip applied kartlara `outcome` iliştirir; AdCard "Sonuç: iyileşti/geriledi/…" rozeti gösterir.
...
- **Dosyalar:** lib/dijimagic/ai/campaignMetricsById.ts (yeni), lib/dijimagic/resultTrackingStore.ts (listDueBeforeRecorded), app/api/cron/dijimagic-outcome-snapshots/route.ts (yeni), vercel.json, app/api/dijimagic/improvements/hierarchy/{route.ts,decision/route.ts}, lib/dijimagic/ai/hierarchicalStore.ts, components/dijimagic/hierarchy/AdCard.tsx, locales/{tr,en}.json

## 2026-06-14 — DijiAlgoritma denetim onarımı 1/N: IDOR güvenlik + yayın paritesi (önizleme = yayın)
...
- **Dosyalar:** lib/dijimagic/ai/{hierarchicalStore,types,improvementToProposal,perCampaignPrompt}.ts, lib/dijimagic/adCreator.ts, app/api/dijimagic/improvements/hierarchy/decision/route.ts, app/api/dijimagic/create-ad/route.ts

## 2026-06-14 — DijiAlgoritma Faz 4: ölü kod temizliği + dayanıklılık + erişilebilirlik
...
- **Dosyalar:** components/dijimagic/hierarchy/AccountAlertsBanner.tsx, lib/dijimagic/ai/hierarchicalStore.ts; silindi: components/dijimagic/{ImprovementCardGrid,ImprovementCard}.tsx

## 2026-06-14 — DijiAlgoritma: rakip taraması varsayılan açıldı
- **Sorun:** Rakip Apify scrape yolu `DIJIALGORITMA_SCRAPE_COMPETITORS` flag'iyle varsayılan KAPALIydı; bu yüzden rakip verisi prod'da fiilen hiç toplanmıyor, "rakipleri araştır" vaadi devre dışıydı.
- **Çözüm:** Kullanıcı kararıyla (2026-06-14) varsayılan AÇIK yapıldı — yalnız `DIJIALGORITMA_SCRAPE_COMPETITORS=false` ile kapatılabilir. Apify token yoksa veya beyan edilen rakip yoksa güvenli no-op (gereksiz maliyet/crash yok). Beyan edilen rakipler (max 3, 7 günlük cache) artık her haftalık taramada güncellenir ve prompt'a girer.
- **Dosyalar:** lib/dijimagic/ai/competitorScanStep.ts

## 2026-06-13 — DijiAlgoritma Faz 3.2: rakip analizi prompt yarışı giderildi
...
- **Çözüm:** `perCampaignImprovements`'a `scan.user` ile birebir aynı `scrape-competitors` step'i eklendi — `gatherUserScanInputs` cache'i okumadan ÖNCE beyan edilen rakipleri tazeler. Flag (`DIJIALGORITMA_SCRAPE_COMPETITORS`) kapalıysa anında no-op (sıfır regresyon); açıkken rakip bloğu artık yarışsız prompt'a girer. 7 günlük per-rakip cache çift-scrape'i önler.
- **Dosyalar:** inngest/functions/perCampaignImprovements.ts

## 2026-06-13 — DijiAlgoritma Faz 3.1: deterministik yapısal motor per-campaign akışına bağlandı
...
- **Dosyalar:** inngest/functions/perCampaignImprovements.ts, lib/dijimagic/ai/perCampaignPrompt.ts

## 2026-06-13 — DijiAlgoritma Faz 2: karar yüzeyi + yayın akışı bütünlüğü
...
- **Dosyalar:** components/dijimagic/hierarchy/{HierCardActions,CampaignCard,AdsetCard,DrilldownModal,HierarchicalImprovements}.tsx, components/dijimagic/AdCreationWizard.tsx, app/dijialgoritma/page.tsx, app/api/dijimagic/improvements/hierarchy/decision/route.ts, lib/dijimagic/ai/improvementToProposal.ts, locales/{tr,en}.json

## 2026-06-13 — DijiAlgoritma Faz 1: yayın güvenliği + derin hedefleme verisi (3 kritik)
- **Sorun:** DijiAlgoritma derin analizinde üç kritik açık: (1) Google reklamları "PAUSED" denip aslında `ENABLED` (canlı) yayınlanıyor → anında harcama + yanlış mesaj; (2) Meta'da aktif kampanya içindeki PAUSED ad set/reklamlar da analiz ediliyordu ("sadece aktif" ihlali); (3) hedefleme/lokasyon/dil verisi fetcher'larda hiç çekilmediği halde prompt AI'dan bunları analiz etmesini istiyordu → halüsinasyon riski.
- **Çözüm:**
  - **Google PAUSED:** `createFullCampaign`'e opsiyonel `status` parametresi (default `ENABLED` → manuel sihirbaz davranışı korunur, sıfır regresyon); DijiAlgoritma `create-ad` Google branch'ı `status:'PAUSED'` geçer. Kampanya PAUSED → alt öğeler ENABLED olsa bile yayın olmaz; "PAUSED" mesajı artık gerçek.
...
- **Dosyalar:** lib/google-ads/create-campaign.ts, app/api/dijimagic/create-ad/route.ts, lib/dijimagic/metaDeepFetcher.ts, lib/dijimagic/googleDeepFetcher.ts, lib/dijimagic/analysisTypes.ts, lib/dijimagic/ai/perCampaignPrompt.ts
...
- **Çözüm:** 4 fazda, çalışan yayın akışı korunarak (her fazdan sonra `tsc` + sonda tam `next build`, 0 hata) düzeltildi:
  - **Faz A:** Meta Siyaset enum `ISSUES_ELECTIONS_POLITICS` (+ backend normalize); Google gelir enum `INCOME_RANGE_90_UP`; Google Display `optimizedTargetingEnabled` flat; Meta CBO Engagement/Awareness bütçesiz-kampanya bug'ı (frontend ile hizalandı).
  - **Faz B (Google):** Observation/Targeting `targeting_setting`; Display conversion goals; PMax standalone TARGET_CPA/ROAS → Max*; PMax Final URL expansion `urlExpansionOptOut`; PMax TOTAL bütçe no-op kaldırıldı; `networkSettings` yalnız SEARCH; negatif keyword BROAD; proximity (yarıçap) hedefleri API'ye gönderiliyor.
  - **Faz C (Meta):** Saved audience artık uygulanıyor (targeting Meta'dan çekilip inline); SALES/LEADS+WEBSITE `promoted_object`'e pixel; manuel yerleşim enum (`messenger_home`/`facebook_reels`); DISAPPROVED/PENDING_BILLING_INFO listelerde görünür; lifetime+bitiş-yok guard.
  - **Faz D:** Carousel-edit veri kaybı (edit bloklanıyor); katalog dead-end (CATALOG sunulmuyor); carousel video accept kapatıldı; AdEditDrawer CTA genişletildi + yüklenen CTA korunuyor; Google RSA içerik edit AdService (`ads:mutate`).
- **Ertelenen (kalan-iş):** behavior/demographic `flexible_spec` ayrımı, age_max koşullu clamp (advantage_audience toggle), performans hedefi sessiz-ezme, PMax audience signals (AudienceService), TIS UI parametreleri, mevcut-gönderi CTA, DCO authoring / Collection / `/generatepreviews` (yeni özellik). Regresyon/yeni-UI riski nedeniyle ayrı ele alınacak.
- **Dosyalar:** lib/google-ads/{create-campaign,create-performance-max-campaign}.ts, components/google/{CampaignEditPanel,wizard/shared/WizardHelpers,wizard/pmax/steps/PMaxStepBudget,detail/DemographicEditor}.tsx, app/api/integrations/google-ads/{campaigns/[campaignId]/update,ads/[adId]/update}/route.ts, app/api/meta/{campaigns,adsets,ads,ads/details}/route.ts, components/meta/{CampaignWizard,AdEditDrawer,wizard/{StepCampaign,TabDetails,CarouselEditor}}.tsx, components/CampaignCreateModal.tsx, lib/meta/spec/objectiveSpec.ts, locales/{tr,en}.json
...
- **Sorun:** Çekirdek shell + bazı modüller EN'de çalışıyordu ama hedef-kitle, strateji, dijialgoritma, gözetim, Meta/Google sunum katmanı ve birçok dosya hardcoded Türkçe idi — EN seçen kullanıcı "yarı İngilizce" görüyordu. Dropdown opsiyonları, "Seçin" placeholder'ları, gizli/koşullu durumlar, owner-only alanlar dahil.
...
- **Güvenlik ağı (yeni scriptler):** `scripts/i18n-parity.mjs` (tr/en leaf-key seti eşit), `scripts/i18n-usage-check.mjs` (kodda çağrılan her t() anahtarı JSON'da var), `scripts/i18n-merge.mjs` (keyMap merkezi merge). Her batch sonrası 3 kapı + tamlık taraması.
- **Doğrulama:** parite tr/en **5762=5762**; usage-check **0 eksik** (3802 çağrı); `tsc --noEmit` **0 hata**; **`next build` 0 hata** (tüm sayfalar); tamlık taraması gerçek-gap **0**.
- **Dosyalar:** locales/tr.json + en.json, ~90 component/page, 3 yeni i18n script.

## 2026-06-12 — Dönüşüm Sihirbazı: güvenlik + dürüstlük + rehber içerikleri (denetim kapanışı)
- **Sorun:** 7 ajanlı derin denetim bulguları: (1) `ENCRYPTION_KEY` yoksa setup refresh token'ı sessizce düz metin kaydediliyordu (güvenlik açığı); (2) `marketing_setups.status` insert'te `pending` kalıp hiç güncellenmiyordu; (3) GTM trigger'ları tag'lerin aksine güncellenmiyordu (filtre değişse eski tanım kalırdı); (4) version oluşmadan da adım "done" raporlanabiliyordu; (5) Adım 4'teki 5 platform kurulum rehberi sekmesi boştu (başlık var, içerik yok — no-fake-data ihlali).
- **Çözüm:** (1) Plaintext fallback kapatıldı — key yoksa `saveGoogleSetupToken` false döner, kullanıcı açıkça hata görür; `.env.local`'e key üretildi (Vercel'e aynı değer eklenmeli). (2) `logStep` artık üst kaydın durumunu türetir: tüm adımlar terminal → `done`/`error`, değilse `running`. (3) `upsertTrigger` mevcut trigger'ı fingerprint ile PUT eder (tag'lerle simetrik). (4) Version oluşmazsa ve yeni tag yazıldıysa `gtm_version_failed` fırlatılır; "no changes" yeniden-çalıştırması hata sayılmaz. (5) 5 platform için gerçek adım adım kurulum talimatı yazıldı (Next.js/WordPress/Shopify/Webflow/Diğer — TR+EN, 4'er adım) ve Deployment'ta render edildi. **Bilinçli ürün kararı (kullanıcı onayladı):** Adım 3 "Onayla ve Kuruluma Başla" = tek onay; GTM doğrudan canlıya yayınlanır, ikinci bir yayın onayı YOKTUR (tek-tık akışı). Ayrıca doğrulandı: omddq'da `marketing_setups`/`setup_steps`/`capi_events` tabloları mevcut; Meta CAPI doğrulama event'i zaten `event_id` (dedup) gönderiyor — denetimdeki iki bulgu yanlış alarmdı. Stape GTM MCP reposu kullanılmayacak/fork edilmeyecek (karar kaydı).
- **Dosyalar:** `lib/marketing-setup/setupStore.ts`, `lib/marketing-setup/gtmClient.ts`, `components/marketing-setup/steps/Deployment.tsx`, `locales/{tr,en}.json`

## 2026-06-12 — URL slug'ları sidebar başlıklarıyla hizalandı (TR/EN) + Yardım Merkezi sayfası
- **Sorun:** Bazı sayfa URL'leri sidebar başlığıyla uyuşmuyordu: "Dönüşüm Sihirbazı" → `/marketing-kurulumu`, "SEO Plus" → `/seo`, "CRM Sistemi" → `/crm`, "İşletme Profili" → `/dijialgoritma/isletme-profili` (yanlış modül altında). Ayrıca profil menüsündeki "Yardım Merkezi" ölü linkti (`href='#'`, sayfası yoktu).
- **Çözüm:** Route'lar başlıklarla hizalandı, TR/EN slug sistemi tam uygulandı (site canlı kullanıcıya açık olmadığı için 301 yok): `/donusum-sihirbazi` (EN `/en/conversion-wizard`), `/seo-plus` (EN aynı), `/crm-sistemi` (EN `/en/crm-system`), `/isletme-profili` (EN `/en/business-profile`, dijialgoritma altından çıkarıldı + kendi layout'u). Slug haritaları (routes.ts + middleware), tabRoutes (`seo`→`seo-plus`), nav, OAuth dönüş yolları, robots.ts disallow listesi ve tüm iç linkler güncellendi. Yardım Merkezi gerçek içerikle açıldı: `/yardim-merkezi` (EN `/en/help-center`) — 7 maddelik SSS (accordion) + info@dijimagic.com destek kartı, tam i18n; dropdown'daki hardcoded "İşletme Profili" etiketi de `sidebar.isletmeProfili` çevirisine taşındı. Meta/Google reklam route'larına dokunulmadı (entegrasyon koruması). Doğrulama: tsc temiz + eski slug kalıntı taraması sıfır.
- **Dosyalar:** `app/{donusum-sihirbazi,seo-plus,crm-sistemi,isletme-profili,yardim-merkezi}/` (taşıma + yeni layout/sayfa), `lib/routes.ts`, `lib/nav.ts`, `lib/tabRoutes.ts`, `middleware.ts`, `app/robots.ts`, `app/dashboard/HomePage.tsx`, `app/api/oauth/setup-google/{start,callback}/route.ts`, `components/marketing-setup/MarketingSetupWizard.tsx`, `components/UserProfileDropdown.tsx`, `locales/{tr,en}.json`
...
- **Dosyalar:** `supabase/migrations/20260612000000_login_attempts.sql` (yeni), `scripts/db-apply-migration.mjs` (yeni), `app/api/auth/login/route.ts`, `app/login/page.tsx`, `lib/dijimagic/businessSourceScanner.ts`, `lib/dijimagic/socialSourceScanner.ts`

## 2026-06-12 — KRİTİK para açığı: auth'suz AI üretim endpoint'leri + sınırsız kredi iadesi
- **Sorun:** AI üretim endpoint'leri (`tasarim/generate-image`, `generate-video`, `enhance-prompt`, `upload`, `dijimagic/chat`, `seo/analyze`) ne kimlik ne kredi/abonelik kontrolü yapıyordu; kredi düşümü yalnız istemci JS'inde olduğundan (1) login olmadan internetten herkes FAL/Anthropic/PageSpeed faturası yaktırabiliyor, (2) abone doğrudan endpoint çağırıp kredi sistemini atlayabiliyordu. Ayrıca `POST /api/credits/refund` keyfi `amount` ile sınırsız bedava kredi bastırıyordu.
- **Çözüm:** Yeni `lib/billing/featureGuard.ts` (`chargeFeature`): sunucu-taraflı kimlik + (gerekirse) aktif abonelik + atomik kredi düşümü + başarısızlıkta otomatik iade; owner bypass. Endpoint'lere uygulandı: tasarım görsel/video (design_generation, 20 kredi, hata→iade), enhance-prompt/upload (kimlik), dijimagic/chat (abonelik + dijialgoritma_chat 5 kredi, nodejs runtime'a alındı), seo/analyze (abonelik). İstemci çift-düşümü kaldırıldı (tasarım/dijialgoritma/SEO sayfaları artık sunucu bakiyesini yeniden çeker). Tehlikeli `/api/credits/refund` route'u **silindi**; iade artık yalnız sunucu içinde guard tarafından yapılır. İç akışlar (SEO cron) lib fonksiyonlarını doğrudan çağırdığı için etkilenmez. `tsc --noEmit` 0 hata.
- **Davranış değişikliği (gözden geçir):** DijiAlgoritma/Tasarım içindeki ad-creative görsel üretimi (MetaCreativePanel/AdImageGenerator) ve manuel SEO makale içeriği artık kredi tüketir (önceden bedavaydı) — "bedava AI yok" ilkesi gereği bilinçli.
- **Dosyalar:** `lib/billing/featureGuard.ts` (yeni), `app/api/tasarim/{generate-image,generate-video,enhance-prompt,upload}/route.ts`, `app/api/dijimagic/chat/route.ts`, `app/api/seo/analyze/route.ts`, `app/api/credits/refund/route.ts` (silindi), `components/providers/CreditProvider.tsx`, `app/tasarim/[[...segments]]/page.tsx`, `app/dijialgoritma/page.tsx`, `components/seo/SeoArticlesTab.tsx`
...
- **Dosyalar:** `lib/dijimagic/officialAdsDocsRefresh.ts`, `lib/dijimagic/officialAdsRefreshRunner.ts`, `inngest/functions/officialAdsRefresh.ts`, `app/api/inngest/route.ts`, `app/api/cron/official-ads-refresh/route.ts`
...
- **Dosyalar:** `lib/dijimagic/officialAdsDocsRefresh.ts`, `scripts/apply-official-ads-migration.mjs`, `package.json`
...
- **Çözüm:** (1) `lib/strategy/expertPlan.ts` — markadan gerekçeli `ExpertCampaignPlan` (hedef kitle/lokasyon/demografi/amaç/bütçe/CTA/metin) üretir: AI (Claude) + deterministik korkuluklar (amaç deterministik, bütçe min'in altına inmez, CTA `allowed_values`'a göre doğrulanır, AI hazır değilse sahte veri yok). (2) `lib/dijimagic/turkishText.ts` Türkçe-bilinçli şehir eşleme → İstanbul bug fix. (3) `POST /api/strategy/instances/[id]/expert-plan` (flag `EXPERT_PLAN_ENABLED` default-off, platform başına paralel). (4) Strateji'ye "Uzman Plan" sekmesi (gerekçe kartları + metin varyantları). Advisory — publish/wizard/Apify dokunulmadı.
- **Dosyalar:** `lib/strategy/expertPlan.ts`, `lib/dijimagic/turkishText.ts`, `lib/dijimagic/businessSourceScanner.ts`, `app/api/strategy/instances/[id]/expert-plan/route.ts`, `components/strateji/ExpertPlanView.tsx`, `app/strateji/[id]/[[...tab]]/page.tsx`, `.env.example`, ilgili testler

## 2026-06-10 — Google Sihirli Tara: outcome persist + account_id atfı (hesap-scope Google)
- **Sorun:** Google Sihirli Tara sonuçları `dijimagic_recommendation_results`'a **hiç kaydedilmiyordu** (yalnız Meta kaydediyordu) → Google öğrenmesi ve hesap-scope atfı yoktu. Persist route da `platform:'meta'` sabitti.
...
- **Dosyalar:** `components/optimization/GoogleScanResults.tsx`, `app/api/dijimagic/optimization/recommendations/route.ts`, `app/optimizasyon/[[...segments]]/page.tsx`, `_learnings/global/account-scope.md` (dijimagic-brain)
...
- **Dosyalar:** `app/optimizasyon/[[...segments]]/page.tsx`, `_learnings/global/engineering-guardrails.md` (dijimagic-brain)

## 2026-06-10 — Öğrenen Beyin hesap-scope: öneri-sonucu kayıtlarına account_id atfı
- **Sorun:** Öğrenme aktif/seçili reklam hesabına göre gelişmeli; ama `dijimagic_recommendation_results`'ta `account_id` kolonu yok ve `metadata.account_id` boştu → beyin outcome'ları hesaba atfedemiyordu (ilk veride 4/4 unattributed). Cross-account/cross-business ders sızıntısı riski.
- **Çözüm:** (1) Beyin hesap-farkında: `collect-outcomes.mjs` artık `by_account` + `account_attribution` üretir; `_learnings/global/account-scope.md` zorunlu ilke + doktrin. (2) Atıf akışı (client+server, Meta entegrasyonuna dokunmadan): `/api/meta/status` zaten dönen `adAccountId` → optimizasyon sayfasında yakalanır → `MagicScanResults` persist POST'una `accountId` eklenir → `/api/dijimagic/optimization/recommendations` okur → `recordBeforeSnapshot` `metadata.account_id`'ye yazar. Meta fetcher/publish/magic-scan route **değişmedi**; yalnız sunum+persist. 2026-06-10 öncesi kayıtlar unattributed kalır (backfill yok, prod-risk).
- **Dosyalar:** `lib/dijimagic/resultTrackingStore.ts`, `app/api/dijimagic/optimization/recommendations/route.ts`, `components/optimization/MagicScanResults.tsx`, `app/optimizasyon/[[...segments]]/page.tsx`, `scripts/brain/collect-outcomes.mjs`, `_learnings/{global/account-scope.md,outcome-measurement.md,doctrine/cloud-analysis.md,INDEX.md}` (dijimagic-brain)

## 2026-06-10 — "DijiMagic Sağlık Merkezi" otomatik izleme + günlük e-posta
...
- **Çözüm:** 10 parçayı kontrol eden `saglik_kontrol.py` kuruldu → her gün 09:00 (launchd `com.dijimagic.saglik`) tek özet e-postası (`onursuay@hotmail.com`). Kontroller: prod `/api/health`, ana sayfa, Supabase (omddq), Vercel deploy+9 cron, yerel Beyin job (`com.dijimagic.brain.collect`), Beyin verisi tazeliği, GitHub yedek (ana repo + `dijimagic-brain`), kritik `.env.local` anahtarları, self-report. Konu sabit "DijiMagic Sağlık Merkezi"; durum gövdede ✓ / nabız atışı ECG çizgisi (inline SVG, sarı/amber yok) / 🔴 ile gösterilir. SMTP kimliği yerelde (`~/.dijimagic-saglik-automation/smtp_config.json`, gitignore). Bulut sessizlik koruması: sır-olmayan durum özeti `dijimagic-brain`'e push edilir; haftalık bulut routine (Pazartesi 10:00) tazeliğe bakıp bilgisayar günlerce kapalıysa uyarı maili gönderir.
- **Dosyalar:** `_automation/{saglik_kontrol.py,com.dijimagic.saglik.plist,README.md,smtp_config.example.json}`, `docs/CHANGELOG.md`, `.gitignore`, `~/.dijimagic-saglik-automation/**` (runtime, gitsiz), `~/Library/LaunchAgents/com.dijimagic.saglik.plist`

## 2026-06-09 — Kendi kendine öğrenen + otomatik yedeklenen "Öğrenen Beyin" (4 katman)
- **Sorun:** DijiMagic'in dağınık dersleri (CLAUDE.md + memory) yapılandırılmamıştı; AI önerilerinin gerçek ürün-sonucu (ROAS/CTR etkisi) ölçülüp derse dönüşmüyordu; geliştirme bilgisi otomatik yedeklenmiyordu.
- **Çözüm:** 4 katmanlı sistem kuruldu. (1) `_learnings/` öğrenen beyin (README/INDEX/_TEMPLATE/global/units/doctrine, tohumlanmış gerçek derslerle). (2) CLAUDE.md'ye bağlayıcı döngü kuralı: ÖNCE OKU / SONRA YAZ (sormadan) / SONUCU YAZ (✅/❌/⚠️ + KÖK NEDEN, uydurma rakam yasak). (3) Ayrı **private** `dijimagic-brain` repo'su yedeği — push öncesi genişletilmiş secret-scan, git check-ignore ile hariç tutma (NFC/NFD-safe). (4) Güvenli hibrit periyodik öğrenme: yerel `collect-outcomes.mjs` (Supabase omddq SALT-OKUNUR → anonim agrega `_data/latest.json` → launchd günlük 08:00 → push; token yerelde) + haftalık bulut routine (Pazar 23:00 İstanbul, yalnız repo JSON'ını okuyup kök-neden analizi yazar, hassas kaynağa bağlanmaz). Öğrenme kapsamı = ürün sonucu (`dijimagic_recommendation_results` + `dijimagic_action_outcomes`); AI engine/publish'e **dokunulmadı** (closed-loop default-OFF).
- **Dosyalar:** `_learnings/**` (ayrı dijimagic-brain repo'su), `scripts/brain/{collect-outcomes.mjs,secret-scan.sh,run-collect.sh,install-launchd.sh}`, `CLAUDE.md` (Öğrenen Beyin bölümü), `.gitignore` (`_learnings/`)
...
- **Dosyalar:** `lib/dijimagic/officialAdsKnowledgeParser.ts`, `officialAdsChangeNotifier.ts`, `officialAdsKnowledgeDecision.ts`, `officialAdsDocsRefresh.ts`, `lib/dijimagic/ai/docs/officialKnowledgeBlock.ts`, `lib/dijimagic/ai/{systemPrompt,perCampaignPrompt,agent,perCampaignAgent}.ts`, `inngest/functions/{dijialgoritmaScan,perCampaignImprovements}.ts`, `app/api/cron/official-ads-refresh/route.ts`, `app/api/admin/gozetim-merkezi/official-ads/{pending,decision}/route.ts`, `app/gozetim-merkezi/OfficialAdsKnowledgePanel.tsx`, `.env.example`, ilgili testler
...
- **Dosyalar:** `lib/firecrawl/{types,client,pageSelector,scrapeSite}.ts`, `lib/dijimagic/businessSourceScanner.ts`, `src/tests/firecrawl{Client,PageSelector,ScrapeSite}.test.ts`, `src/tests/businessSourceScannerFirecrawl.test.ts`, `.env.example`
...
- **Sorun:** DijiAlgoritma / Optimizasyon / Strateji / sohbet, Meta'nın sistem mekaniğini (Breakdown Effect, learning phase, marjinal vs. ortalama CPA, auction overlap, pacing, ad relevance) bilmeden öneri/kopya üretiyordu — "yüksek ortalama CPA'lı segmenti durdur" gibi klasik hatalara açıktı.
...
- **Dosyalar:** `lib/seo/topicSelector.ts`, `lib/dijimagic/prompts.ts`
...
## 2026-06-02 — DijiAlgoritma: URL, header, AI reklam, otomatik tarama, dropdown iyileştirmeleri
- **Sorun:** (1) `/dijimagic` URL'si "DijiAlgoritma" başlığıyla uyumsuz; bazı EN slug'lar (abonelik/hesabim/faturalarim) çevrilmemiş. (2) Öneri ticker'ı hesap seçiciye sıkışıp kesiliyor; "AI Analiz" rozeti + düz "Son güncelleme" yazısı. (3) AI Reklam Oluştur, haftalık tarama hiç çalışmadığında "AI reklam önerisi üretilemedi" hatası veriyor; platform kartlarında M/G harfleri. (4) Geliştirme kartları yalnız Pazar cron'dan üretiliyor → yeni kullanıcı/owner "Pazar gece" boş uyarısı görüyor. (5) İşletme dropdown'unda "Meta + Google" / "Yalnızca …" yazıları.
- **Çözüm:** (1) `app/dijimagic`→`app/dijialgoritma` rename; middleware + `lib/routes.ts` slug haritaları (dijialgoritma↔dijialgorithm, abonelik↔subscription, hesabim↔account, faturalarim↔invoices) + crm/email-marketing prefix tutarlılığı; iç linkler güncellendi (redirect gerekmedi — sistem henüz açılmadı). (2) Ticker 3 bölgeli layout ile ortalandı; "AI Analiz" kaldırıldı, son güncelleme renkli pill'e alındı. (3) Platform seçilince boş yanıt gelirse otomatik `forceGenerate=true` ile canlı üretim (publish akışı değişmedi); M/G → kurumsal `/platform-icons` logoları; modal metinleri i18n. (4) Yeni `POST /api/dijimagic/improvements/bootstrap` kart yoksa gerçek hiyerarşik taramayı tetikler (sahte veri yok, cooldown'lı); "İlk analiziniz hazırlanıyor…" durumu + Batch bitene kadar yoklama; "Pazar gece" metni kaldırıldı. (5) Platform yazıları yerine hesap adı yanında küçük (14px) Meta/Google logoları. TypeScript + `next build`: 0 hata.
- **Dosyalar:** `app/dijialgoritma/*` (rename), `middleware.ts`, `lib/routes.ts`, `lib/nav.ts`, `app/robots.ts`, `app/dashboard/HomePage.tsx`, `components/UserProfileDropdown.tsx`, `components/account/MultiAccountDropdown.tsx`, `components/account/BusinessSwitcherDropdown.tsx`, `components/seo/SeoSitesPanel.tsx`, `components/dijimagic/DijiAlgoritmaHeader.tsx`, `components/dijimagic/CommandCenterHeader.tsx`, `components/dijimagic/AdCreationWizard.tsx`, `components/dijimagic/hierarchy/HierarchicalImprovements.tsx`, `app/api/dijimagic/improvements/bootstrap/route.ts`, `locales/tr.json`, `locales/en.json`
...
- **Dosyalar:** `components/seo/SeoArticlesTab.tsx`, `lib/dijimagic/prompts.ts`, `app/api/seo/publish/route.ts`
...
- **Sorun:** Google Cloud Verification Center, `helpful-envoy-489009-v2` (Google Ads DijiMagic) projesinde gizlilik politikası maddesini "Your privacy policy URL is improperly formatted" ile reddetti. Sayfa (`/en/privacy-policy`) HTTP 200 + tam HTML SSR olmasına rağmen: (1) gerçek `robots.txt` yoktu — `/robots.txt` `noindex`'li dashboard HTML'i dönüyordu; (2) root layout'taki `cookies()` kullanımı + middleware cookie set'i sayfayı `Cache-Control: private, no-cache, no-store` yapıyor, Google'a halka açık statik hukuki belge yerine özel/dinamik sayfa gibi görünüyordu; (3) legal sayfalarda explicit `robots`/`canonical`/`metadataBase` yoktu.
...
- **Dosyalar:** components/account/UnifiedAccountSwitcher.tsx, app/api/dijimagic/improvements/hierarchy/route.ts, components/dijimagic/hierarchy/HierarchicalImprovements.tsx

## 2026-05-31 — DijiAlgoritma: Hesap Sağlık kartı arka yüz metin sıkışması giderildi
...
- **Dosyalar:** components/dijimagic/hierarchy/AccountAlertsBanner.tsx

## 2026-05-31 — DijiAlgoritma: Hesap Sağlık kartları responsive grid düzeltmesi
...
- **Sorun:** SEO, CRM, Marketing'e uygulanan genişlik + animasyon + okunabilir tipografi değişikliklerinin kalan tüm modüllere (Strateji, Optimizasyon, DijiAlgoritma, Hedef Kitle, Tasarım, Raporlar, Entegrasyon) de taşınması istendi. Meta/Google reklam alanları hariç.
- **Çözüm:** **Strateji** — max-w-6xl→7xl, başlıklar text-base, info kartı+liste animate-card-enter. **Optimizasyon** — Meta+Google kampanya kartları kademeli animate-card-enter. **DijiAlgoritma** — command center ana konteyneri animate-card-enter (max-w-[1440px] korundu). **Hedef Kitle** — max-w-7xl, GoogleAudienceView+AudienceList wrappers animate-card-enter. **Tasarım** — kredi göstergesi amber ihlali (bg-amber-50/text-amber-500/700) → bg-primary/5+text-primary. **Raporlar** — max-w-7xl (2 konum), KPI grid+trend grafiği animate-card-enter. **Entegrasyon** — max-w-7xl (2 konum), tüm platform bağlantı kartları hover:shadow-md. Meta/Google entegrasyon koduna dokunulmadı.
- **Dosyalar:** app/strateji/page.tsx, app/optimizasyon/page.tsx, app/dijimagic/page.tsx, app/hedef-kitle/page.tsx, app/tasarim/page.tsx, app/raporlar/page.tsx, app/entegrasyon/page.tsx
...
- **Çözüm:** Lead durumu değişince gerçek Meta senkronu (additive, idempotent — mevcut Meta altyapısına dokunulmaz):
  - **Olumlu** → "DijiMagic CRM — Olumlu Lead'ler" CUSTOMER_LIST custom audience'a SHA-256 hash'li e-posta/telefon eklenir (lookalike/retargeting tohumu) + bir kez CAPI `QualifiedLead` custom olayı (`system_generated`; standart Lead metriğini şişirmez).
  - **Olumsuz** → "DijiMagic CRM — Olumsuz Lead'ler" audience'a eklenir (hedeflemeden hariç tutma) + olumlu listeden çıkarılır.
...
## 2026-05-30 — Multi-slot fetch wrapper (Faz 3A): DijiAlgoritma/analytics modülleri için multi-slot deep-fetch helper'ları
- **İhtiyaç:** Mevcut `fetchMetaDeep` / `fetchGoogleDeep` tek hesap çalışır (override pattern); DijiAlgoritma, Strateji, Optimizasyon vb. modüller multi-slot aware olabilmek için tüm seçili slot'ların verisini iterate edip birleştirmeli.
- **Çözüm:** Yeni `lib/dijimagic/multiSlotFetcher.ts` — `fetchMetaDeepAllSlots(userId)` ve `fetchGoogleDeepAllSlots(userId)`:
...
- **Dosyalar:** `lib/dijimagic/multiSlotFetcher.ts` (yeni). tsc 0 hata; next build temiz.
- **Sonraki adım**: Bu helper'ları gerçek çağrılarda (dijialgoritmaScan, perCampaignImprovements vb.) opt-in olarak kullanmak. Module-by-module ilerleyeceğiz.
...
- **Kalan iş**: Faz 2B (`/entegrasyon` sadeleştirme — hesap detayı kaldır, sadece bağlı/değil göster), Faz 3 (DijiAlgoritma/Strateji/Optimizasyon vb. modülleri `getSelectedAdAccounts` ile çoklu slot aware yap).
...
- **Kalan iş (Faz 2 + 3):** /entegrasyon UI sadeleştirme (sadece bağlı/değil), reklam sayfalarında 2-slot dropdown selector, sihirbazın slot sistemine bağlanması, DijiAlgoritma/Strateji vb. modüllerin multi-slot aware olması.
...
- **Çözüm:** (1) `siteScanner.ts` yeniden yapılandırıldı — Claude artık BİRİNCİL analizci: sayfa içerikleri (başlık + meta + ilk metin) + tıklanabilir öğeler + deterministik kanıt havuzu Claude'a verilir; Claude işletme türünü belirler ve KANIT-TEMELLİ öneri üretir (`reason` zorunlu; kanıtsız event önermez kuralı sistem promptunda). `SiteScanResult`'a `businessAnalysis` + `RecommendedEvent.reason` eklendi. Claude erişilemezse deterministik fallback. UI'da yeni "Site Analizi (AI)" kartı + her önerinin altında kanıt cümlesi. (2) SiteScanner mount'ta `/api/dijimagic/business-profile`'dan `website_url`'yi çekip URL kutusunu ön-besler (kullanıcı yine değiştirip silebilir). 3 yeni i18n anahtarı TR+EN; parity tam (2998/2998); tsc 0 hata; next build temiz.
...
- **Not (prod):** Üretimin canlıda çalışması için Vercel'de `ANTHROPIC_API_KEY` (zorunlu), `SITE_CREDENTIALS_KEY` (WordPress parola), `CRON_SECRET` + omddq'da `article_schedules` / `site_connections` / `dijimagic_articles` SEO kolonları gerekir.
...
- **Dosyalar:** `lib/dijimagic/businessProfileStore.ts`
...
- **Sorun:** Proje iki AI sağlayıcısını karışık kullanıyordu — reklam optimizasyonu Claude, ama DijiMagic sohbet/niyet, SEO konu+makale, Tasarım prompt iyileştirme, reklam metni üretimi ve birkaç zenginleştirme motoru OpenAI (`gpt-4o-mini`). Üretime çıkarken tek AI istendi: Claude.
- **Çözüm:** Paylaşılan Claude yardımcısı `lib/anthropic/text.ts` eklendi (`claudeText`, `claudeJson`, `claudeStream` — edge+node uyumlu raw fetch, model `getClaudeModel()` = `claude-sonnet-4-6`, JSON çıktısı fence-toleranslı). 10 OpenAI çağrı noktası Claude'a taşındı: DijiMagic sohbet (streaming), niyet algılama, Tasarım enhance-prompt, SEO konu seçici + makale üretici, reklam metni üreticisi (`adCreator`), analiz özetleyici (OpenAI-primary/Claude-fallback → tek Claude), kampanya niyet motoru, rakip tema analizcisi, rakip sorgu genişletici. Deterministik fallback'ler korundu (anahtar yoksa sistem kırılmaz). Strateji'deki kullanıcı-yüzlü "OpenAI key ekleyin" metni nötrleştirildi. **Not:** Çok-AI Karar Masası (Faz 4, `multiAiDecisionDesk`/`aiProviders`) bilinçli çoklu-sağlayıcı ensemble olduğu ve anahtarsız otomatik "skipped" düştüğü için olduğu gibi bırakıldı. tsc + production build temiz.
- **Dosyalar:** `lib/anthropic/text.ts` (yeni), `app/api/dijimagic/chat/route.ts`, `app/api/dijimagic/detect-intent/route.ts`, `app/api/tasarim/enhance-prompt/route.ts`, `lib/seo/topicSelector.ts`, `lib/seo/articleGenerator.ts`, `lib/dijimagic/adCreator.ts`, `lib/dijimagic/aiAnalysisSummarizer.ts`, `lib/dijimagic/campaignIntentEngine.ts`, `lib/dijimagic/competitorAnalyzer.ts`, `lib/dijimagic/competitorQueryExpander.ts`, `components/strateji/BlueprintView.tsx`
...
- **Çözüm:** `UserProfileDropdown`'dan "Kurumsal" bloğu (ve kullanılmayan `Lock` import'u) kaldırıldı. Ana sayfa footer'ına referans tasarımındaki dark dropdown dil seçici (Globe ikon + bayrak + chevron, 🇹🇷 Türkçe / 🇬🇧 English) eklendi — DijiMagic'in gerçek i18n mekanizmasıyla (`NEXT_LOCALE` cookie + `mapPathToLocale`), dışarı tıklayınca kapanır, aktif dil emerald ile işaretli.
...
- **Çözüm:** `upsertProfile` constraint-agnostik **find-then-write** yapıldı (business_key varsa `(user_id, business_key)`, yoksa legacy `(user_id, business_key IS NULL)` ile bul → update/insert; `onConflict`'e bağlı değil). `business-profile` GET/POST, `DIJIMAGIC_PER_ACCOUNT_SCOPE` açıkken aktif işletme scope'una göre çalışır: GET `getProfileForScope` ile o işletmenin profilini çeker (yoksa null → UI "profil oluştur"), POST `business_key`/`meta_account_id`/`google_customer_id`'yi profile bağlar. Migration: `user_business_profiles` `UNIQUE(user_id)` → partial unique `(user_id, business_key) WHERE business_key IS NOT NULL` (çoklu profile izin; bağsız/legacy NULL serbest — geriye uyumlu). Account switcher işletme seçince full reload yaptığından profil sayfası otomatik doğru profili yükler. `tsc` ✓, `next build` ✓.
- **Dosyalar:** `lib/dijimagic/businessProfileStore.ts`, `app/api/dijimagic/business-profile/route.ts`, `supabase/migrations/20260524010000_business_profiles_per_account_unique.sql`
...
- **Sorun:** (1) Modül sayfalarının (Strateji / Optimizasyon / Hedef Kitle / Tasarım / Raporlar / SEO / Entegrasyon) sağ içerik alanı bembeyazdı, boş/modern olmayan duruyordu. (2) DijiAlgoritma "Hesap Sağlık Durumu" kartları sabit 3'lü grid'deydi; 4. kart alt satıra kayıyordu. (3) Meta tablosunda "Kampanya Bütçesi" / "Reklam Seti" etiketlerinin yanındaki info (ℹ️) ikonu gereksizdi.
- **Çözüm:** (1) `app/globals.css`'e `.app-content-surface` utility eklendi (hafif yeşil gradient `from-emerald-50/40 via-white to-emerald-50/20`, DijiAlgoritma sayfasıyla tutarlı); 7 modül sayfasının kök içerik container'ı `bg-gray-50` → `app-content-surface`. (2) `AccountAlertsBanner` grid'i kart sayısına göre `gridTemplateColumns: repeat(N, minmax(0,1fr))` → kaç kart olursa olsun TEK SATIR, ekrana sığacak şekilde otomatik daralır (taşma yok). (3) Meta tablosunda iki bütçe etiketinin `Info` ikonu + tooltip'i kaldırıldı (etiketler kaldı), kullanılmayan `Info` import'u temizlendi. Meta/Google API'ye dokunulmadı. `tsc` ✓, `next build` ✓.
- **Dosyalar:** `app/globals.css`, `app/strateji/page.tsx`, `app/optimizasyon/page.tsx`, `app/hedef-kitle/page.tsx`, `app/tasarim/page.tsx`, `app/raporlar/page.tsx`, `app/seo/page.tsx`, `app/entegrasyon/page.tsx`, `components/dijimagic/hierarchy/AccountAlertsBanner.tsx`, `app/dashboard/reklam/meta/components/MetaTableReal.tsx`
...
- **Çözüm:** **(Faz 0)** Additive migration: `user_business_profiles`'a `business_key`/`meta_account_id`/`google_customer_id`, `user_business_intelligence`'a `business_key`, `account_alerts`'e `account_id`/`business_key` (hepsi NULLABLE; `UNIQUE(user_id)` korundu → sıfır regresyon). **(Faz 1, `DIJIMAGIC_PER_ACCOUNT_SCOPE` flag arkasında)** Tarama artık seçili işletmenin Meta+Google hesabını + KENDİ profilini kullanır (`getProfileForScope`/`getBusinessContextForScope`); eşleşen profil yoksa profilsiz tarar ve off-brand yanlış uyarısı ÜRETMEZ. `account_alerts` hesap boyutuyla yazılır; `supersedePendingAccountAlerts` yalnız ilgili hesabı süpürür (Antso taraması Belgemod uyarısını silmez); hierarchy endpoint'i `account_id`'ye göre filtreler. Cron, flag açıkken kullanıcının her işletmesi için ayrı scope'lu event fan-out yapar (kapalıyken kullanıcı başına tek legacy event). Meta/Google fetch entegrasyon koduna dokunulmadı (yalnız mevcut `override` parametresi geçildi). `tsc` ✓, `next build` ✓.
- **Dosyalar:** `supabase/migrations/20260524000000_per_account_business_profiles.sql` (yeni), `scripts/apply-per-account-business-profiles-migration.mjs` (yeni), `lib/dijimagic/businessKey.ts` (yeni), `lib/dijimagic/businessProfileStore.ts`, `lib/dijimagic/businessContextStore.ts`, `lib/dijimagic/ai/scanUser.ts`, `lib/dijimagic/ai/hierarchicalStore.ts`, `inngest/functions/perCampaignImprovements.ts`, `inngest/functions/dijialgoritmaScan.ts`, `app/api/cron/dijialgoritma-scan/route.ts`, `app/api/dijimagic/improvements/hierarchy/route.ts`
...
## 2026-05-23 — DijiAlgoritma Geliştirme Kartları da işletmeye scope edildi (asıl belgemod fix)
- **Sorun:** İşletme seçimi Command Center'ı doğru filtreliyordu (DB kanıtlı) ama kullanıcının asıl baktığı **"Geliştirme Kartları"** hâlâ tüm hesapların kartlarını (örn. belgemod "Çelik Kaynakçı") gösteriyordu. Kök neden: kartlar `GET /api/dijimagic/improvements/hierarchy` → `getImprovementHierarchy(userId)`'den geliyor; tablo (`campaign_improvements`) `source_platform` + `campaign_id` taşıyor ama **hesap kimliği sütunu yok** ve okuma yalnız `user_id`'ye göreydi → işletmeden bağımsız tüm kartlar dönüyordu.
- **Çözüm:** hierarchy endpoint'i, seçili işletmenin **scope'lu günlük analizindeki kampanya kimliklerine** göre kartları filtreler (`{platform}:{campaignId}` eşleşmesi). Böylece migration/şema değişikliği gerekmeden başka hesabın kartları düşer; aynı platformdaki iki hesap (Metropol vs belgemod, ikisi de Google) bile ayrılır. Eşleşen scope'lu analiz hazır değilse `scopePending` döner (yanlış kart yerine "hazırlanıyor"). Sayfa, Command Center scope'u oturunca `improvementRefreshKey` bump ederek kartları yeniden çeker. `DIJIMAGIC_PER_ACCOUNT_SCOPE` kapalıyken tüm kartlar (mevcut davranış). `tsc` ✓.
- **Dosyalar:** `app/api/dijimagic/improvements/hierarchy/route.ts`, `components/dijimagic/hierarchy/HierarchicalImprovements.tsx`, `app/dijimagic/page.tsx`

## 2026-05-23 — DijiAlgoritma scope-duyarlı client cache (v2) + işletme cookie düzeltmesi
- **Sorun:** (1) İşletme cookie'si Next 15 route handler'da `cookies().set()` ile yazılınca yanıta eklenmiyordu → scope hep boşa düşüyordu. (2) `/dijimagic` localStorage cache (v1) scope'tan bağımsızdı → işletme değişse de eski (belgemod) snapshot ekranda kalıyordu.
- **Çözüm:** (1) Cookie `res.cookies.set` ile YANIT üzerine yazılır (garantili Set-Cookie). (2) Cache anahtarı v1→v2; snapshot `dijimagic_business_scope` imzasıyla etiketlenir, yüklemede imza uyuşmazsa gösterilmez → taze fetch. `tsc` ✓.
- **Dosyalar:** `app/api/dijimagic/business-scope/route.ts`, `lib/dijimagic/clientCache.ts`, `app/dijimagic/page.tsx`
...
## 2026-05-23 — DijiAlgoritma per-account Faz 3.4: İşletme bazlı scope (otomatik isim eşleştirme)
- **Sorun:** DijiAlgoritma seçili Meta + seçili Google'ı körlemesine **birleştiriyordu**. Meta "Fikret Petrol" seçiliyken Google hâlâ "belgemod" aktif olduğundan kartlarda belgemod verisi karışıyordu (Faz 3.3b scope kapısı tek başına çözmedi — iki platform bağımsız seçiliyordu).
- **Çözüm:** Kullanıcının kayıtlı Meta + Google hesapları **otomatik isim eşleştirmesiyle** "işletme"lere gruplanır (`groupIntoBusinesses`: Türkçe-duyarlı normalize + TLD/yasal-ek temizliği + içerik eşleşmesi). DijiAlgoritma seçici **işletme moduna** geçer; bir işletme seçilince yalnız **o işletmenin** Meta+Google'ı çekilir — eşi olmayan platform (örn. yalnız-Meta işletmesi) için diğer platform hiç çağrılmaz, başka hesabın verisi karışmaz. Seçim `dijimagic_business_scope` cookie'sinde tutulur; `runDeepAnalysis` opsiyonel scope override ile yalnız o hesapları fetch'ler; command-center / refresh / daily-run scope imzasını tek kaynaktan (`resolveDijiMagicScope`) hesaplar → uyuşmazlık/gereksiz yeniden-analiz yok. Tümü **`DIJIMAGIC_PER_ACCOUNT_SCOPE` flag'i arkasında (default KAPALI)** — kapalıyken mevcut birleşik davranış birebir korunur, sıfır regresyon. Yeni UI tamamen EN/TR. `tsc` ✓.
- **Dosyalar:** `lib/account/businessGroups.ts` (yeni), `lib/dijimagic/businessScope.ts` (yeni), `app/api/dijimagic/business-scope/route.ts` (yeni), `components/account/BusinessSwitcherDropdown.tsx` (yeni), `components/account/UnifiedAccountSwitcher.tsx`, `lib/dijimagic/deepAnalysis.ts`, `lib/dijimagic/metaDeepFetcher.ts`, `lib/dijimagic/googleDeepFetcher.ts`, `app/api/dijimagic/command-center/route.ts`, `app/api/dijimagic/command-center/refresh/route.ts`, `app/api/dijimagic/daily-run/route.ts`, `app/api/account/registered/route.ts`, `hooks/useRegisteredAccounts.ts`, `locales/tr.json`, `locales/en.json`
...
- **Sorun:** `DIJIMAGIC_PER_ACCOUNT_SCOPE` açıkken DijiAlgoritma'da hesap değiştirince veri değişmiyordu — mevcut günlük analizler flag öncesi üretildiği için `account_scope=null`'dı; kapı null'ı "geriye-uyum" sayıp eski (birleşik) veriyi gösteriyordu.
...
- **Dosyalar:** `app/api/dijimagic/command-center/route.ts`, `app/api/dijimagic/command-center/refresh/route.ts`

## 2026-05-22 — Çoklu Reklam Hesabı Faz 3.3b: DijiAlgoritma per-account analiz (belgemod fix, flag arkasında)
- **Sorun:** DijiAlgoritma command-center, kullanıcının seçili Meta+Google hesaplarının birleşik günlük analizini (per-user, günde 1) gösteriyordu; başka hesaba geçince hâlâ önceki seçimin verisi görünüyordu (belgemod).
- **Çözüm:** `dijimagic_daily_runs.account_scope` (aktif seçim imzası) eklendi; tamamlanan analiz `upsertDailyRun`'da DB seçiminden otomatik damgalanır (cron/POST/inngest tek noktadan, pipeline'a dağıtık dokunuş yok). command-center, çalışmanın imzası aktif seçimle eşleşmezse `scope_mismatch` döner; DijiAlgoritma sayfası yeni `/api/dijimagic/command-center/refresh` ile o hesap için analizi yeniden üretip gösterir (AI engine açıkken bootstrap command_center_data üretmediği için gerekli). Hepsi **`DIJIMAGIC_PER_ACCOUNT_SCOPE` flag'i arkasında (default KAPALI)** — kapalıyken mevcut per-user davranış birebir korunur, sıfır regresyon. `tsc` ✓.
- **Dosyalar:** `lib/dijimagic/dailyRunStore.ts`, `app/api/dijimagic/command-center/route.ts`, `app/api/dijimagic/command-center/refresh/route.ts` (yeni), `app/dijimagic/page.tsx`, `lib/dijimagic/featureFlag.ts`

## 2026-05-22 — Çoklu Reklam Hesabı Faz 3.3a: DijiAlgoritma'ya birleşik seçici
- **Sorun:** DijiAlgoritma özel header (Topbar değil) kullandığı için hesap seçici hiç yoktu.
- **Çözüm:** Kendi kendine yeten `UnifiedAccountSwitcher` (tetikleyici buton + birleşik dropdown + veri çekimi tek bileşende) `DijiAlgoritmaHeader`'a eklendi. Flag kapalıyken render etmez. Switcher ile hesap değiştirme global aktif hesabı değiştirir (tüm modülleri etkiler). **NOT:** DijiAlgoritma'nın KENDİ command-center verisinin aktif hesaba göre değişmesi (belgemod fix) Faz 3.3b'de — `account_scope` migration gerektirir. `tsc` ✓.
- **Dosyalar:** `components/account/UnifiedAccountSwitcher.tsx` (yeni), `components/dijimagic/DijiAlgoritmaHeader.tsx`
...
- **Çözüm:** Dropdown artık modülün platformuna göre kapsamlı: `/strateji` ve `/meta-ads` yalnız-Meta → Google bölümü + "Google hesabı ekle" gizlenir; rozet o sayfada görünen hesap sayısını gösterir. `/optimizasyon`, `/hedef-kitle`, `/dijimagic` (iki platformu da kullanan) hepsini gösterir. Böylece Strateji'de Google seçimi/yönlendirmesi olmaz. `tsc` ✓.
...
- **Çözüm:** Hesap seçimi artık bağlam-duyarlı (`navigateAfterSwitch`): çok-platformlu modülde (`/optimizasyon`, `/hedef-kitle`) aynı sayfa + `?platform=X` (ilgili sekme açılır, modülden çıkılmaz); `/dijimagic` reload; sadece-Meta modülde (Strateji/Meta sayfası) Meta → reload, Google → `/google-ads`. Optimizasyon `?platform` okuyup `source` sekmesini ayarlar. Meta seçimi de aynı mantıkla yönlendirir (stale `?platform` sorunu olmaz). `tsc` ✓.
...
- **Çözüm:** Google hesabı seçilince mevcut Meta sayfasını reload etmek yerine `/google-ads`'e yönlendirilir; o Google hesabının verisi orada görünür. Meta seçimi olduğu gibi kalır (Meta sayfası zaten Meta'yı gösterir). DijiAlgoritma cache de temizlenir. `tsc` ✓.
- **Dosyalar:** `components/account/MultiAccountDropdown.tsx`
...
- **Not:** Hedef Kitle + DijiAlgoritma'da seçici görünürlüğü ve DijiAlgoritma per-account sonraki adımlarda.
...
## 2026-05-22 — DijiAlgoritma: Hesap değişiminde bayat client cache temizliği (Madde 1 ön hazırlık)
- **Sorun:** Aktif reklam hesabı değiştiğinde DijiAlgoritma'nın `localStorage`/`sessionStorage` snapshot'ı önceki hesaba aitti ve sayfa yenilemede eski hesabın kartları kısa süre "yanıp sönüyordu". (Optimizasyon, Strateji, Hedef Kitle zaten sunucuda `ad_account_id`'ye göre filtreli/önbellekli olduğu için hesap değişiminde doğru yeniden bağlanıyor — onlarda değişiklik gerekmedi.)
- **Çözüm:** Cache anahtarlarını ve geçersiz kılmayı tek kaynakta toplayan `lib/dijimagic/clientCache.ts` (`clearDijiAlgoritmaClientCache()`, SSR-güvenli). Meta hesap geçişinde (`Topbar.handleSelectAccount`) `window.location.reload()` öncesi çağrılıyor; DijiAlgoritma sayfası anahtarları artık bu modülden alıyor (drift yok). Tarama pipeline'ına ve şemaya dokunulmadı. `tsc` ✓.
- **Not:** DijiAlgoritma'nın aktif-hesaba göre sunucu tarafı analizi (`dijimagic_daily_runs` Meta+Google birleşik toplam, `UNIQUE(user_id,run_date)`) çoklu-hesap tarama orkestrasyonu gerektirdiğinden Madde 2 kapsamında ele alınacak.
- **Dosyalar:** `lib/dijimagic/clientCache.ts` (yeni), `components/Topbar.tsx`, `app/dijimagic/page.tsx`
...
## 2026-05-22 — DijiAlgoritma: Google Arama Ağı reklam önerileri ("Reklam 0" bug fix)
...
## 2026-05-21 — DijiAlgoritma Faz 3 · UI cila 10: reklam yayından önce düzenlenebilir (kart-içi edit)
...
- **Dosyalar:** `lib/dijimagic/ai/hierarchicalStore.ts`, `app/api/dijimagic/improvements/hierarchy/decision/route.ts`, `components/dijimagic/hierarchy/{AdCard,DrilldownModal,HierarchicalImprovements}.tsx`, `locales/{tr,en}.json`, `docs/CHANGELOG.md`

## 2026-05-21 — DijiAlgoritma Faz 3 · UI cila 9: flip kart yazı kesilmesi düzeltildi (auto-height)
...
- **Dosyalar:** `components/dijimagic/hierarchy/AccountAlertsBanner.tsx`, `docs/CHANGELOG.md`

## 2026-05-21 — DijiAlgoritma Faz 3 · UI cila 8: sağlık flip kartları koyu tema
...
## 2026-05-21 — DijiAlgoritma Faz 3 · UI cila 7: ad set kart altı Geri/İleri navigasyonu
...
- **Dosyalar:** `components/dijimagic/hierarchy/{AdsetCard,DrilldownModal}.tsx`, `locales/{tr,en}.json`, `docs/CHANGELOG.md`

## 2026-05-21 — DijiAlgoritma Faz 3 · UI cila 6: butonlar yalnız reklamda + modal başlık sabit değil + objective adı + rakip vurgu
...
- **Dosyalar:** `components/dijimagic/hierarchy/{AdsetCard,DrilldownModal,CampaignCard,AdCard,shared}.tsx`, `lib/dijimagic/translations/meta-enums.ts`, `lib/dijimagic/ai/perCampaignPrompt.ts`, `docs/CHANGELOG.md`

## 2026-05-21 — DijiAlgoritma Faz 3 · UI cila 5: flip tap ikonu + ad set butonları geri
...
- **Çözüm:** `AccountAlertsBanner` ön yüz altı: yazı yerine **animasyonlu `Pointer` (tap) ikonu** (`dijimagicTap` keyframe — hafif bas-bırak; kullanıcının `tap.png`'si yerine benzer lucide ikonu). `AdsetCard`'a `HierCardActions` (advisory: Onayla/Reddet/Uygulandı İşaretle/Geri Al) **geri eklendi** + `DrilldownModal`'da karar handler'ları bağlandı. Kampanya kartı yine **butonsuz** (yalnız UYGULA). `build` ✓.
- **Dosyalar:** `components/dijimagic/hierarchy/{AccountAlertsBanner,AdsetCard,DrilldownModal}.tsx`, `docs/CHANGELOG.md`

## 2026-05-21 — DijiAlgoritma Faz 3 · UI cila 4: simetrik kampanya kartı + Title Case adlar
...
- **Dosyalar:** `components/dijimagic/hierarchy/{shared,CampaignCard,AdsetCard,AdCard,DrilldownModal}.tsx`, `docs/CHANGELOG.md`

## 2026-05-21 — DijiAlgoritma Faz 3 · UI cila 3: yatay kampanya kartı + buton sadeleştirme + ortalı flip başlık
...
- **Dosyalar:** `components/dijimagic/hierarchy/{AccountAlertsBanner,CampaignCard,AdsetCard,HierarchicalImprovements,DrilldownModal}.tsx`, `locales/{tr,en}.json`, `docs/CHANGELOG.md`

## 2026-05-21 — DijiAlgoritma Faz 3 · UI cila 2: flip-box sağlık kartları + yatay modal + font −1
...
- **Dosyalar:** `components/dijimagic/hierarchy/{shared,AccountAlertsBanner,AdsetCard,AdCard,CampaignCard,DrilldownModal,HierCardActions}.tsx`, `locales/{tr,en}.json`, `docs/CHANGELOG.md`

## 2026-05-21 — DijiAlgoritma Faz 3 · UI cila: hesap sağlık kartları + popup drill-down + logo ikonlar + büyük yazı
- **Sorun:** Canlı /dijimagic incelemesinde 5 talep: (1) "Hesap Sağlık Uyarıları" → "Hesap Sağlık Durumu" + animasyonlu dikdörtgen kartlar + başlık ikonu, (2) Geliştirme Kartları başlığına ikon, (3) kart sol üstünde Meta/Google **yazı yerine logo**, (4) yazılar çok küçük (okunmuyor), (5) "Ad Set'leri Gör" inline yerine **popup** + net Kampanya→Tür→Reklam Seti→Reklam hiyerarşisi.
...
- **Dosyalar:** `components/dijimagic/hierarchy/{shared,AccountAlertsBanner,DrilldownModal,HierarchicalImprovements,CampaignCard,AdsetCard,AdCard,HierCardActions}.tsx`, `locales/{tr,en}.json`, `docs/CHANGELOG.md`

## 2026-05-21 — DijiAlgoritma Faz 3 · Adım 6+7: Hiyerarşik drill-down UI + i18n
...
- **Dosyalar:** `lib/dijimagic/ai/perCampaignPrompt.ts`, `lib/dijimagic/ai/perCampaignAgent.ts`, `lib/dijimagic/ai/hierarchicalStore.ts`, `inngest/functions/perCampaignImprovements.ts`, `app/api/inngest/route.ts`, `app/api/cron/dijialgoritma-scan/route.ts`, `app/api/dijimagic/improvements/hierarchy/route.ts`, `app/api/dijimagic/improvements/hierarchy/decision/route.ts`, `docs/CHANGELOG.md`

## 2026-05-21 — DijiAlgoritma Faz 3 · Adım 2: Bilingual enum çeviri katmanı
...
- **Çözüm:** Yeni `lib/dijimagic/translations/` katmanı — `types.ts` + `meta-enums.ts` + `google-enums.ts` (objective / optimization goal / CTA / placement / bidding / status / ad type / match type domain'leri, TR+EN map) + `index.ts`: `translateEnum(value, locale, platform?)` + `translateEnumList`, `normKey` normalizasyonu (`Advantage+ Placements` → `ADVANTAGE_PLACEMENTS`), bilinmeyen değer "Title Case"e düşer — ham SNAKE_CASE asla görünmez. `humanizeTr.ts` geriye dönük uyumlu kalıp merkezî `translateEnum`'a delege ediyor (regresyon yok; mevcut kartlar otomatik zenginleşti). `tsc` 0 hata.
- **Dosyalar:** `lib/dijimagic/translations/{types,meta-enums,google-enums,index}.ts`, `lib/dijimagic/ai/humanizeTr.ts`, `docs/CHANGELOG.md`

## 2026-05-21 — DijiAlgoritma Faz 3 · Adım 1: Hiyerarşik geliştirme tabloları (omddq'ya uygulandı)
...
- **Dosyalar:** `docs/dijialgoritma_per_ad_refactor_plan.md` (yeni), `docs/CHANGELOG.md`
...
- **Dosyalar:** `lib/dijimagic/ai/systemPrompt.ts`, `docs/CHANGELOG.md`
...
- **Sorun:** İki Supabase projesi karışıklığı. `omddq…` canlı (tüm tablolar var: user_business_profiles, ai_engine_runs, dijimagic_daily_runs, ai_suggestions — REST 200), `fbqr…` ölü (HTTP 000/DNS yok). 2026-05-19 env düzenlemesinde `NEXT_PUBLIC_SUPABASE_URL` fbqr→omddq değişmiş ama `SUPABASE_URL` fbqr'de kalmış (split). Üstüne kod tutarsızdı: ana server client `SUPABASE_URL||NEXT_PUBLIC` (→ölü fbqr), Meta modülleri `NEXT_PUBLIC||SUPABASE_URL` (→canlı omddq). Sonuç: AI engine'in yazdığı proje ≠ UI'ın okuduğu proje — audit'in "ai_engine_runs boş" bulgusunun kök nedeni.
...
- **Dosyalar:** `lib/dijimagic/ai/types.ts`, `lib/dijimagic/ai/adSpecPayload.ts` (yeni), `lib/dijimagic/ai/agent.ts`, `lib/dijimagic/ai/systemPrompt.ts`, `src/tests/dijialgoritmaAdSpecPayload.test.ts` (yeni), `docs/CHANGELOG.md`

## 2026-05-20 — A4: Rakip reklam analizi AI motoruna bağlandı (3. ayak)
- **Sorun:** Apify rakip altyapısı çalışıyordu ama AI tarama motoruna HİÇ bağlı değildi (`lib/dijimagic/ai/*` içinde sıfır competitor referansı) — üç ayaklı analizin 3. ayağı kopuktu. Ayrıca scrape kullanıcının beyan ettiği rakip adlarıyla değil, kampanya keyword'leriyle çalışıyordu.
- **Çözüm:** İki yol eklendi. (1) READ (her zaman açık, ücretsiz): `competitorBrief.ts` pure builder + `competitorScanStep.loadCompetitorBrief` → cache'li `dijimagic_competitor_insights` + `dijimagic_competitor_ads` verisini "## Rakip Reklam Analizi" bloğuna çevirip platforma özel Claude payload'ına ekler (Google metinsiz kayıtları dürüstçe "metin mevcut değil" etiketler). gatherUserScanInputs competitorContext'i yükler, buildUserBrief gömer. (2) WRITE (flag ile, **varsayılan KAPALI**): `scrapeDeclaredCompetitors` — `DIJIALGORITMA_SCRAPE_COMPETITORS=true` ise beyan edilen rakipleri (max 3) Apify ile tarar, per-competitor 7 günlük cache, paralel, soft-fail; Inngest'e fetch'ten önce `scrape-competitors` step'i olarak eklendi. competitorScanner'a opsiyonel maxRecords cost cap (non-breaking). 6/6 smoke testi, A1-A3 regresyon yok. Token etkisi ~500-700/platform. NOT: write yolu prod maliyeti nedeniyle dormant — Onur Vercel'de flag açıp canlı test edince devreye girer.
- **Dosyalar:** `lib/dijimagic/ai/competitorBrief.ts` (yeni), `lib/dijimagic/ai/competitorScanStep.ts` (yeni), `lib/dijimagic/ai/scanUser.ts`, `lib/dijimagic/ai/agent.ts`, `lib/dijimagic/ai/systemPrompt.ts`, `lib/dijimagic/competitorScanner.ts`, `inngest/functions/dijialgoritmaScan.ts`, `src/tests/dijialgoritmaCompetitorBrief.test.ts` (yeni), `docs/CHANGELOG.md`
...
- **Dosyalar:** `lib/dijimagic/apifyCompetitorProvider.ts`, `src/tests/apifyCompetitorNormalizer.test.ts` (yeni), `docs/CHANGELOG.md`
...
- **Dosyalar:** `lib/dijimagic/ai/docs/meta_ad_rules_curated.ts` (yeni), `lib/dijimagic/ai/docs/google_ads_rules_curated.ts` (yeni), `lib/dijimagic/ai/systemPrompt.ts`, `lib/dijimagic/ai/agent.ts`, `src/tests/dijialgoritmaPlatformRules.test.ts` (yeni), `docs/CHANGELOG.md`
...
- **Çözüm:** Yeni pure builder `lib/dijimagic/ai/scanBusinessBrief.ts` eklendi; `getBusinessContextForUser` (profil + rakipler + intelligence tek noktada) sonucunu Claude payload'ına gidecek tam markdown'a çeviriyor. Kullanıcı beyanı = birincil/kırpılmaz (iş tanımı tam metin), intelligence = ikincil/kapatılabilir enrichment. `loadBusinessContext` minimal sorgu yerine bu builder'ı kullanıyor; `buildUserBrief` 1500 char clamp'i kaldırıldı; system prompt'a "marka uygunluğu (ZORUNLU)" direktifi eklendi. Token etkisi: Belgemod örneğinde ~150 → ~920 token (+770, 50K bütçenin çok altında — sıkıştırma gereksiz). 10/10 builder smoke testi geçti.
- **Dosyalar:** `lib/dijimagic/ai/scanBusinessBrief.ts` (yeni), `lib/dijimagic/ai/scanUser.ts`, `lib/dijimagic/ai/systemPrompt.ts`, `src/tests/dijialgoritmaScanBusinessBrief.test.ts` (yeni), `docs/CHANGELOG.md`

## 2026-05-20 — DijiAlgoritma bağlam audit raporu
...
- **Çözüm:** Salt okuma + kod izleme + canlı Apify testiyle audit yapıldı. Bulgular: aktif filtre Meta/Google ✅; platform docs entegrasyonu ❌ (system prompt'ta yok); Apify altyapısı var ama AI motoruna bağlı değil ❌ (lib/dijimagic/ai/* içinde sıfır competitor referansı); business profile context kısmi ⚠ (6 alan gidiyor; marka adı/ürünler/rakipler/intelligence gitmiyor); tam ad spec üretilmiyor ❌. Canlı test: iki actor da SUCCEEDED (<$0.01), ama Meta normalizer nested snapshot'ı kaçırıyor + Google actor reklam metni döndürmüyor. 6 öncelikli aksiyon (A1-A6) listelendi; en yüksek getiri A3 (mevcut user_business_intelligence'ı payload'a eklemek).
- **Dosyalar:** `docs/dijialgoritma_context_audit.md` (yeni)

## 2026-05-20 — DijiAlgoritma proje amacı + Google resmi dokümanlar + index
- **Sorun:** DijiAlgoritma'nın iş kuralları (üç ayaklı analiz, aktif-only, business profile zorunluluğu, tam ad spec çıktısı) ve resmi platform dokümanlarının AI engine'e nasıl bağlanacağı yazılı referans olarak yoktu.
- **Çözüm:** `docs/dijialgoritma_proje_amaci.md` (sistem amacı + 7 değişmez iş kuralı + DoD) yazıldı. Google Ads 2026 PDF'leri (Temel/Orta + İleri Seviye + kaynakça) sadık transkripsiyonla Markdown'a aktarıldı. `docs/resmi_dokumanlar_index.md` ile her dokümanın hangi AI görevinde referans alınacağı + system prompt'a bağlama stratejisi (Seçenek A cache'li gömme / Seçenek B RAG-lite) tanımlandı.
- **Dosyalar:** `docs/dijialgoritma_proje_amaci.md` (yeni), `docs/google_ads_resmi_dokumanlari.md` (yeni, ~51KB), `docs/resmi_dokumanlar_index.md` (yeni), `docs/CHANGELOG.md`
...
- **Çözüm:** [inngest/functions/dijialgoritmaScan.ts](inngest/functions/dijialgoritmaScan.ts) içinde separator `|` → `_` ve emniyet için izinli set dışındaki karakterleri `_` ile değiştiren regex eklendi; sonra 64 char slice. Hatanın gerçek mesajı Inngest REST API `GET /v2/runs/{runId}/trace?includeOutput=true` ile çekildi.
- **Dosyalar:** `inngest/functions/dijialgoritmaScan.ts`, `docs/CHANGELOG.md`

## 2026-05-19 — Inngest sync fix (concurrency 10→5)
- **Sorun:** Cron `dijialgoritma/scan.user` event'ini yolluyordu ama Inngest function tetiklenmiyordu (`ai_engine_runs` boş). Inngest REST API ile sync denendiğinde `POST /v2/apps/dijimagic-ai-engine/syncs` `422 concurrency_limit` döndü — "function concurrency 10 > plan limit 5". Bu yüzden Vercel integration her sync denemesinde reddediliyor, function kayıt edilemiyor; queue'daki event (`01KS12K…`) işleme alınamıyordu.
- **Çözüm:** `inngest/functions/dijialgoritmaScan.ts` içindeki `concurrency: { limit: 10 }` → `5` indirildi (Inngest Free plan tavanı). Header yorumu da güncellendi. Deploy sonrası `POST /v2/apps/dijimagic-ai-engine/syncs` ile manuel sync tetiklendi, bekleyen event işlenmeye başladı.
- **Dosyalar:** `inngest/functions/dijialgoritmaScan.ts`, `docs/CHANGELOG.md`

## 2026-05-19 — DijiAlgoritma AI Engine Test Planı
...
- **Çözüm:** `docs/dijialgoritma_test_plan.md` yazıldı. İçerik: ön koşul tablosu, Preview test prosedürü (3 manuel tetikleme yöntemi — UI bootstrap / cron simulation / single-user POST — `CRON_SECRET` Authorization header kullanımı dahil), 5 SQL doğrulama sorgusu (`ai_engine_runs` token aralıkları 20K-80K input normal kabul edildi, `ai_suggestions` confidence varyansı `STDDEV>=10`, `ai_alerts` severity dağılımı, reasoning kalite check, günlük token bütçe + USD tahmini Sonnet 4.6 fiyatlamasıyla), 6 red flag eşiği (süre>5dk, token>200K, STDDEV<5, hata oranı>%20, generic reasoning, tutarsız öneri) ve her biri için tek-komut rollback talimatı, production'a açma 8-maddelik final checklist. Setup adımları: Vercel env pull yapıldı + Vercel Development env'inde olmayan local-only key'ler (SUPABASE_SERVICE_KEY, APIFY_*, vd.) yedekten merge edildi. Supabase migration kullanıcı tarafından Dashboard SQL Editor üzerinden uygulandı (4 tablo: `ai_engine_runs`, `ai_alerts`, `ai_opportunities`, `ai_suggestions`).
- **Dosyalar:** `docs/dijialgoritma_test_plan.md`

## 2026-05-19 — DijiAlgoritma AI Engine Upgrade Faz 2: Rebuild (Claude Sonnet 4.6 + Tool Use + Agentic Loop)
...
- **Çözüm:** Tam Faz 2 mimarisi kuruldu. `@anthropic-ai/sdk` ^0.97 + `inngest` ^4.4 dependency'leri eklendi. 4 yeni Supabase tablosu (`ai_engine_runs`, `ai_alerts`, `ai_opportunities`, `ai_suggestions`) + RLS + touch trigger migration'ı yazıldı. 6+1 tool tanımı yapıldı (`get_account_overview`, `get_campaign_metrics`, `get_adset_breakdown`, `get_creative_performance`, `compare_vs_benchmark`, `detect_anomaly`, `rule_engine_evidence` — mevcut deterministic rule engine'i koruyup Claude'a evidence kaynağı olarak verdi). Claude Sonnet 4.6 + adaptive thinking + prompt-cached system prompt ile manual agentic loop yazıldı (max 15 iteration, 16K max_tokens, typed Anthropic exception handling, JSON çıktısı parse + validate). Per-user scan orchestrator fetcher'ları parallel çağırıp her platform için Claude'u koşturuyor, sonuç hem `ai_*` tablolarına hem `dijimagic_daily_runs.command_center_data`'ya (mevcut UI ile uyumlu DeepAnalysisResult formatında) yazıyor — frontend bozulmadan gerçek confidence + reasoning gösteriyor. Inngest function + serve endpoint + cron endpoint (`/api/cron/dijialgoritma-scan`) yazıldı; Inngest yoksa graceful inline fallback. `USE_AI_ENGINE` feature flag eski/yeni flow arasında geçiş yapıyor — flag ON iken `/api/dijimagic/daily-run` no-op'a düşüyor (çakışma yok), POST manuel bootstrap AI engine'e delege ediyor. `vercel.json`'a yeni cron (`0 5 * * *` 08:00 TR), `package.json`'a `db:migrate:ai-engine` script. Meta/Google entegrasyon dosyalarına dokunulmadı — fetcher'lar sarmalandı. `npm run build` başarılı (tüm rotalar derlendi, TS hatasız).
- **Dosyalar:** `package.json`, `package-lock.json`, `vercel.json`, `app/api/dijimagic/daily-run/route.ts` (feature flag delegation), `lib/anthropic/client.ts`, `inngest/client.ts`, `inngest/functions/dijialgoritmaScan.ts`, `app/api/inngest/route.ts`, `app/api/cron/dijialgoritma-scan/route.ts`, `lib/dijimagic/featureFlag.ts`, `lib/dijimagic/ai/types.ts`, `lib/dijimagic/ai/systemPrompt.ts`, `lib/dijimagic/ai/tools/index.ts`, `lib/dijimagic/ai/agent.ts`, `lib/dijimagic/ai/persist.ts`, `lib/dijimagic/ai/scanUser.ts`, `supabase/migrations/20260519000000_create_ai_engine_tables.sql`, `scripts/apply-ai-engine-migration.mjs`

## 2026-05-19 — DijiAlgoritma AI Engine Upgrade Faz 1: Audit Raporu
- **Sorun:** DijiAlgoritma "Kural Motoru" generic, template-driven öneriler üretiyor — verim alınamıyor. Claude API + tool use + agentic loop ile rebuild gerekiyor; önce mevcut sistemin tam haritası ve teşhisi lazım.
- **Çözüm:** Kapsamlı audit yapıldı. Sayfa & UI haritası, 13 API rotasının tablosu, Meta + Google rule engine derin incelemesi, confidence skorlarının sahte olduğu kanıtlandı ([adCreator.ts:474](lib/dijimagic/adCreator.ts#L474)), kullanılan/kullanılmayan metrikler, 10 Supabase tablosu durumu, Anthropic / Inngest / Vercel cron altyapısı incelendi. 6 kök neden tespit edildi (kopuk confidence, dar metrik whitelist, per-account context yok, generic template, tek-AI decision desk, teşhis-aksiyon kopuklugu). Korunacak / yenilenecek / deprecate edilecek liste ile Faz 2 mimarisi belgelendi. Faz 2 (rebuild) onay bekliyor — kod değişikliği yok.
- **Dosyalar:** `docs/dijialgoritma_audit.md`
...
## 2026-05-15 — DijiAlgoritma Faz 1: Yanıltıcı UI Temizliği
...
- **Dosyalar:** `components/dijimagic/ApprovalFlowPreview.tsx`, `components/dijimagic/HealthOverviewCards.tsx`
...
- **Çözüm:** `AudienceCard` DRAFT kartlarına "Düzenle" (Pencil) butonu eklendi. Butona basıldığında wizard edit modda açılır: mevcut `dijimagic_spec_json` parse edilerek Custom/Lookalike/Saved state'i reconstruct edilir, form dolu gelir. Kaydet'e basıldığında `PATCH /api/audiences/[id]` çağrılır (Meta'ya gönderim yapılmaz — kullanıcı listeden "Meta'ya Gönder" ile manuel tetikler). Confirm ekranı edit/create moduna göre farklı metin gösterir. `UnifiedAudience`'a `dijimagicSpecJson?` eklendi. Tüm 99 test geçiyor.
...
- **Dosyalar:** `supabase/migrations/20260515100000_audiences_user_id_rls.sql` (yeni), `lib/meta/context.ts`, `app/api/audiences/route.ts`, `app/api/audiences/[id]/route.ts`, `app/api/audiences/[id]/create/route.ts`, `app/api/audiences/sync/route.ts`, `lib/dijimagic/metaDeepFetcher.ts`
...
- **Sorun:** Kayıt formu dolduran herkes email doğrulamasından sonra otomatik olarak `/dashboard`'a düşüyor ve DijiMagic'in tüm iç panellerine (Optimizasyon, Strateji, DijiAlgoritma, Hedef Kitle, Tasarım, vb.) doğrudan erişebiliyordu. Owner manuel onay/red verme imkânı, ön görüşme akışı, owner bildirim maili, başvuru loglaması ve "değerlendirme aşamasında" UX yoktu.
...
- **Sorun:** Faz 1 backend guard production'da çalışıyor ancak owner hesabı (onursuay@hotmail.com) `subscriptions` tablosunda paid kayıt taşımadığı için "Optimizasyon için aktif bir abonelik gerekli." mesajıyla bloklanıyordu. Magic-scan tetiklenemediği için `dijimagic_recommendation_results` tablosuna kayıt düşmüyordu.
...
- **Dosyalar:** `lib/dijimagic/apifySocialConfig.ts`, `.env.local`, `.env.example`, `CLAUDE.md`
...
- **Dosyalar:** `lib/dijimagic/apifySocialRunner.ts`, `.env.local`, `.env.example`, `CLAUDE.md`
...
- **Dosyalar:** `app/dijimagic/isletme-profili/page.tsx`
...
- **Dosyalar:** `lib/dijimagic/businessProfileStore.ts`, `app/api/dijimagic/business-profile/route.ts`, `app/api/dijimagic/business-profile/scan/route.ts`, `app/dijimagic/isletme-profili/page.tsx`, `CLAUDE.md`
...
- **Sorun:** `/dijimagic/isletme-profili` sayfasında içerik aşağı kaymıyor, bilgiler görünmüyordu
...
- **Çözüm:** `/dijimagic/isletme-profili` sayfası oluşturuldu; profil ve rakipler bölüm bölüm görüntülenir, "Düzenle" butonu mevcut onboarding modalını edit modda açar. Sidebar'da DijiMagic altına "İşletme Profili" sub-item eklendi
- **Dosyalar:** `app/dijimagic/isletme-profili/page.tsx`, `lib/nav.ts`
...
- **Dosyalar:** `components/dijimagic/BusinessProfileOnboarding.tsx`
...
- **Çözüm:**
  ...
  - **`components/dijimagic/BusinessProfileOnboarding.tsx`**: Modal container `overflow-hidden` ile rounded radius'tan taşan footer arka planı engellendi. X butonu artık `onClose` her tanımlandığında görünüyor (eski `isEditMode && onClose` koşulu kaldırıldı). ESC tuşu desteği eklendi.
  - **`components/dijimagic/BusinessProfileGuard.tsx`**: `onClose` her zaman geçiyor; kullanıcı modal'ı kapatınca onboarding **tamamlanmış sayılmıyor** — Guard kilit ekranı (`{area} kilidi açık değil`) bırakılıyor ve "İşletme Profilini Tamamla" butonu modal'ı yeniden açıyor. Yalnızca `onComplete` `state='completed'` set ediyor.
...
- **Dosyalar:** `app/api/admin/gozetim-merkezi/route.ts`, `app/api/admin/business-profiles/route.ts`, `app/gozetim-merkezi/GozetimMerkeziClient.tsx`, `components/dijimagic/BusinessProfileOnboarding.tsx`, `components/dijimagic/BusinessProfileGuard.tsx`, `src/tests/gozetimMerkeziAccess.test.ts`, `src/tests/businessProfileOnboardingModal.test.ts`
...
- **Çözüm:**
  - **`lib/dijimagic/apifySocialConfig.ts` (yeni):** Env-based Apify config helper. `APIFY_API_TOKEN`, platform-specific actor ID env'leri (APIFY_INSTAGRAM/FACEBOOK/LINKEDIN/YOUTUBE/TIKTOK_*_ACTOR_ID), `getApifyToken()`, `getApifyActorId(platform)`, `isApifyReady(platform)`, `buildActorInput(platform, url)`. Token/secret loglanmaz.
  - **`lib/dijimagic/apifySocialRunner.ts` (yeni):** Apify REST API runner. Async flow: actor start → poll status → dataset fetch. Timeout 90s, rate-limit (429), failed actor durumları net raporlanır (`apify_token_missing`, `apify_actor_missing`, `apify_run_failed`, `apify_dataset_empty`, `apify_timeout`, `apify_rate_limited`). Sistem kırılmaz.
  - **`lib/dijimagic/socialProfileNormalizer.ts` (yeni):** Per-platform normalizer (Instagram, Facebook, LinkedIn, YouTube, TikTok). `NormalizedSocialProfile` ortak shape. Defensive parsing — field yoksa uydurma yok. `normalizeSocialProfile(platform, raw, url)` dispatch.
  - **`lib/dijimagic/socialSourceScanner.ts` (güncellendi):** Apify token + actor ID mevcutsa → Apify actor dene → normalize → output. Apify fail/empty → public metadata fallback. `error_message` formatında `|provider:{value}` her durumda yazılır. Gözetim Merkezi mevcut parser ile uyumlu.
...
- **Dosyalar:** `lib/dijimagic/apifySocialConfig.ts`, `lib/dijimagic/apifySocialRunner.ts`, `lib/dijimagic/socialProfileNormalizer.ts`, `lib/dijimagic/socialSourceScanner.ts`, `src/tests/apifySocialScanner.test.ts`
...
- **Çözüm:**
  ...
  - **Access guard:** Profili olmayan kullanıcı `/dijimagic`, `/strateji`, `/hedef-kitle`, `/google-ads`, `/meta-ads` route'larında kilitleniyor.
  - **Üretim motoru bağlamaları:** DijiAlgoritma `generate-ad` (adCreator prompt'u), Strateji `generate-plan` (ai-generator user prompt prefix), proposal engine orchestrator (`businessKeywords` → competitor query expander).
  - **DijiAlgoritma kart UI:** Meta/Google üst section başlıkları kaldırıldı; tek 3 kolon grid; kartın sol üstüne platform logosu (Meta/Google SVG); "Kampanya Türü" satırı eklendi; teknik enum'lar humanize (`OUTCOME_ENGAGEMENT` → "Etkileşim").
...
- **Dosyalar:** `supabase/migrations/20260513000000_create_business_intelligence_profile.sql`, `lib/dijimagic/sectorCatalog.ts`, `lib/dijimagic/sectorLocationIntelligence.ts`, `lib/dijimagic/businessSourceScanner.ts`, `lib/dijimagic/businessProfileStore.ts`, `lib/dijimagic/businessProfileValidation.ts`, `lib/dijimagic/businessContextStore.ts`, `lib/dijimagic/businessIntelligenceBuilder.ts`, `app/api/dijimagic/business-profile/route.ts`, `app/api/dijimagic/business-profile/sectors/route.ts`, `app/api/dijimagic/business-profile/scan/route.ts`, `app/api/admin/business-profiles/route.ts`, `app/api/dijimagic/generate-ad/route.ts`, `app/api/strategy/instances/[id]/generate-plan/route.ts`, `lib/dijimagic/adCreator.ts`, `lib/dijimagic/proposalEngineOrchestrator.ts`, `lib/strategy/ai-generator.ts`, `lib/strategy/job-runner.ts`, `components/dijimagic/BusinessProfileOnboarding.tsx`, `components/dijimagic/BusinessProfileGuard.tsx`, `components/dijimagic/AiAdSuggestions.tsx`, `components/dijimagic/AdPreviewCard.tsx`, `app/dijimagic/layout.tsx`, `app/strateji/layout.tsx`, `app/hedef-kitle/layout.tsx`, `app/google-ads/page.tsx`, `app/meta-ads/page.tsx`, `src/tests/businessIntelligenceProfile.test.ts`, `src/tests/dijialgoritmaProposalCardLayout.test.ts`
...
- **Dosyalar:** `app/api/dijimagic/generate-ad/route.ts`
...
- **Çözüm:** `lib/dijimagic/competitorQueryExpander.ts` oluşturuldu. `CampaignIntentProfile` + kampanya sinyallerinden (servis adı, domain, offer type, detected_keywords, keyword list, reklam grubu adları) platform-spesifik `CompetitorQueryPlan` üretir. Google için niyet odaklı arama sorguları, Meta için sosyal reklam dili sorgular ayrı üretilir; platformlar karışmaz. Deterministic heuristic önce çalışır; confidence < 50 ise OpenAI ile genişletilir; LLM başarısız olursa sistem kırılmaz. `lib/dijimagic/competitorScanner.ts`'e `deriveCompetitorQueryPlan` fonksiyonu ve `CompetitorScanResult`'a diagnostic alanları eklendi (`usedQuery`, `querySource`, `queryPlanConfidence`, `queryPlanReason`, `noResultReason`, `rawCount`, `usefulCount`). `meta-ad-library/route.ts` ve `google-auction/route.ts`'e `diagnostic` response bloğu eklendi; rakip bulunamadı sebebi 5 kategoriye ayrıldı (query üretilemedi, sonuç yok, actor hata, usefulCount=0, platform auth eksik). 16/16 unit test geçiyor.
- **Dosyalar:** `lib/dijimagic/competitorQueryExpander.ts` (yeni), `lib/dijimagic/competitorScanner.ts`, `app/api/dijimagic/competitors/meta-ad-library/route.ts`, `app/api/dijimagic/competitors/google-auction/route.ts`, `src/tests/competitorQueryExpander.test.ts` (yeni), `docs/CHANGELOG.md`

---

## 2026-05-12 — Campaign Intent Engine
- **Sorun:** DijiAlgoritma proposal üretmeden önce kampanyanın gerçek iş bağlamını (ne sattığı, kime sattığı, dönüşüm hedefi) anlamıyordu. Bu yüzden "Sitemizi Ziyaret Edin" gibi jenerik başlıklar üretebiliyordu.
- **Çözüm:** `lib/dijimagic/campaignIntentEngine.ts` oluşturuldu. Kampanya adı, reklam seti adları, reklam metinleri ve landing page içeriğinden `CampaignIntentProfile` çıkarır (business_domain, offer_type, service_or_product, target_audience, conversion_goal, funnel_stage, detected_keywords, vb.). Önce deterministic heuristic (domain signal lookup tablosu), düşük confidence'da OpenAI LLM ile zenginleştirme. `lib/dijimagic/landingPageAnalyzer.ts` oluşturuldu: URL fetch (8s timeout), HTML'den title/meta/h1-h3/body text çıkarma, normalize + summary. Fetch hatası sistemi kırmaz; error missing_data'ya yazılır. `supabase/migrations/20260512100000_create_campaign_intent_profiles.sql` migration eklendi: (user_id, platform, campaign_id) unique, 7 gün TTL, RLS aktif. `adCreator.ts` güncellendi: `buildPrompt` ve `generateFullAutoProposals` fonksiyonlarına `intentProfilesByCampaignId` parametresi eklendi; sistem prompt'a jenerik başlık yasağı kuralı, her kampanya bloğuna KAMPANYA INTENT bölümü eklendi. `generate-ad/route.ts` güncellendi: decision desk sonrası intent profil build edilip `generateFullAutoProposals`'a geçiriliyor. 10/10 unit test geçiyor.
- **Dosyalar:** `lib/dijimagic/campaignIntentEngine.ts` (yeni), `lib/dijimagic/landingPageAnalyzer.ts` (yeni), `supabase/migrations/20260512100000_create_campaign_intent_profiles.sql` (yeni), `lib/dijimagic/adCreator.ts`, `app/api/dijimagic/generate-ad/route.ts`, `src/tests/campaignIntentEngine.test.ts` (yeni), `docs/CHANGELOG.md`

---

## 2026-05-12 — Proposal Policy Guard (Faz B)
- **Sorun:** DijiAlgoritma'nın ürettiği öneriler UI'ya ve publish akışına, platform kuralları açısından hiç kontrol edilmeden düşüyordu. Google başlıklarında ünlem, 30 karakter üzeri başlık, 90 karakter üzeri açıklama, teknik enum görünürlüğü (MAXIMIZE_CONVERSIONS, OUTCOME_ENGAGEMENT vb.) ve Meta'da geçersiz objective/destination kombinasyonları (OUTCOME_ENGAGEMENT + ON_AD) gözden kaçıyordu.
- **Çözüm:** `lib/dijimagic/proposalPolicyGuard.ts` oluşturuldu. Google guard: ünlem kaldırma, emoji kaldırma, başlık 30 / açıklama 90 karakter limiti (normalize + violation), teknik enum Türkçeye çevirme, ALLCAPS yoğunluğu, generic headline tespiti. Meta guard: capability matrix (`capabilityMatrix.ts`) ile objective/destination uyumluluk kontrolü, geçersiz destination'ları fallback ile normalize etme (ON_AD→ON_PAGE, WHATSAPP→ON_PAGE vb.), teknik enum temizleme. Official Ads Knowledge Base forbidden_values entegrasyonu. `adCreator.ts`'e `applyPolicyGuardToProposals` çağrısı eklendi; rejected öneriler proposal listesinden çıkarılıyor. `FullAdProposal` tipine geriye uyumlu `policyStatus?`, `policyViolations?`, `policySummary?` alanları eklendi. `AdPreviewCard.tsx` review_required durumunda "Platform Kuralı Uyarısı" bandı gösteriyor. `AiAdSuggestions.tsx` rejected proposal'ları filtreden geçiriyor. Unit testler eklendi.
- **Dosyalar:** `lib/dijimagic/proposalPolicyGuard.ts` (yeni), `lib/dijimagic/adCreator.ts`, `components/dijimagic/AdPreviewCard.tsx`, `components/dijimagic/AiAdSuggestions.tsx`, `src/tests/proposalPolicyGuard.test.ts` (yeni), `docs/CHANGELOG.md`
...
- **Dosyalar:** `supabase/migrations/20260512000000_create_official_ads_knowledge_base.sql`, `lib/dijimagic/officialAdsKnowledgeStore.ts`, `lib/dijimagic/adCreator.ts`, `docs/CHANGELOG.md`
...
- **Dosyalar:** `components/dijimagic/AiAdSuggestions.tsx`, `docs/CHANGELOG.md`

## 2026-05-11 — Slash Divider Overlay + AI Analiz Yetenekleri Kaldırıldı
- **Sorun:** ONAYLA/REDDET butonları arasında "/" işareti için ayrı padding (px-2) ile boşluk açılıyordu. Slash butonlar arasına gap yaratıyordu. "AI Analiz Yetenekleri" bölümü /dijimagic sayfasında gereksiz yer kaplıyordu.
...
- **Dosyalar:** `components/dijimagic/AiAdSuggestions.tsx`, `app/dijimagic/page.tsx`, `docs/CHANGELOG.md`
...
- **Dosyalar:** `components/dijimagic/AdPreviewCard.tsx`, `components/dijimagic/AiAdSuggestions.tsx`, `docs/CHANGELOG.md`
...
- **Dosyalar:** `components/dijimagic/AiAdSuggestions.tsx`, `components/dijimagic/AdCreationWizard.tsx`, `docs/CHANGELOG.md`

## 2026-05-11 — DijiAlgoritma Proposal Card Aksiyon Akışı + UI Temizliği
...
- **Çözüm:** (1) `AdPreviewCard.tsx` tamamen yeniden tasarlandı: kart arka planı `bg-[#0f172a]` (dark navy), border `border-[#23314d]`, hover `border-emerald-400/40`, iç preview panel `bg-[#151f33]`. Badge renkleri `bg-indigo-500/20 text-indigo-200`, güven metni `text-slate-400`, label/value çiftleri slate tonları, beklenen performans `text-emerald-400`, AI gerekçesi `text-indigo-400` başlıkla. CTA enum'ları humanize edildi (`SEND_MESSAGE → Mesaj Gönder` vb.). (2) `app/dijimagic/page.tsx`'te sağ içerik alanına 7 adet vektörel deney tüpü SVG eklendi — `pointer-events-none`, `absolute inset-0`, opacity 0.055–0.07, emerald/cyan/indigo tonlar, farklı boyut ve rotasyonlarda, CSS `@keyframes` ile yavaş yüzen animasyon. Sidebar'a hiç dokunulmadı.
- **Dosyalar:** `components/dijimagic/AdPreviewCard.tsx`, `app/dijimagic/page.tsx`

## 2026-05-11 — Competitor Insight insightError Yüzey Çıkarma + Table-Missing Görünürlüğü
- **Sorun:** `dijimagic_competitor_insights` tablosu production'da henüz yaratılmamışsa (migration uygulanmamış), `upsertCompetitorInsight` içindeki `isTableMissingError` dalı hata fırlatmak yerine `return null` yapıyordu. Route'un `catch` bloğu bunu yakalamıyordu; `insightRow = null` oluyordu ama `insightError` set edilmiyordu. Sonuç: response'da `insightId=null`, hiç `insightError` yok — sorun tamamen sessiz kalıyordu.
...
- **Dosyalar:** `lib/dijimagic/competitorInsightStore.ts`, `app/api/dijimagic/competitors/meta-ad-library/route.ts`, `app/api/dijimagic/competitors/google-auction/route.ts`
...
- **Dosyalar:** `lib/dijimagic/apifyCompetitorProvider.ts`, `app/api/dijimagic/competitors/google-auction/route.ts`
...
- **Dosyalar:** `lib/dijimagic/apifyCompetitorProvider.ts`
...
- **Dosyalar:** `lib/dijimagic/apifyCompetitorProvider.ts`, `app/api/dijimagic/competitors/meta-ad-library/route.ts`, `app/api/dijimagic/competitors/google-auction/route.ts`

## 2026-05-11 — DijiAlgoritma Command Center Sadeleştirildi + Öneri Kartları Modernize Edildi
- **Sorun:** /dijimagic ekranında KPI alanı ve Onay Geçmişi görünüyordu, sayfa odağını dağıtıyordu. Öneri kartları düzdü, platform bazlı renkli çerçeveler ve üst çizgiler vardı. "Onayla ve Yayınla" butonu kart arka planından ayrışmıyordu.
...
- **Dosyalar:** `app/dijimagic/page.tsx`, `components/dijimagic/AdPreviewCard.tsx`, `components/dijimagic/AiAdSuggestions.tsx`
...
- **Dosyalar:** `lib/dijimagic/adCreator.ts`, `app/api/dijimagic/generate-ad/route.ts`, `components/dijimagic/AiAdSuggestions.tsx`
...
- **Dosyalar:** `components/dijimagic/KpiDashboard.tsx`
...
- **Dosyalar:** `components/dijimagic/ApprovalHistoryPanel.tsx`
...
- **Sorun:** DijiAlgoritma KPI özet alanı Meta ve Google metriklerini tek toplamda gösteriyordu. CTR ve CPC hangi platforma ait belli değildi.
...
- **Sorun:** `/dijimagic` fresh açılışında `generate-ad` endpoint'i 2 kez çağrılıyordu. `connectedPlatforms` prop'u parent re-render'da yeni dizi referansı olarak gelince `fetchProposals` useCallback yeniden oluşuyor, useEffect ikinci kez tetikleniyordu.
...
- **Dosyalar:** `components/dijimagic/AiAdSuggestions.tsx`

## 2026-05-11 — Faz 2D: Identity Key Fix — session_id → user_id
- **Sorun:** 15 dijimagic route, DB anahtarı olarak `session_id` (her login'de değişen rastgele UUID) kullanıyordu. CRON `user_id` (signups.id — stabil) ile yazıyor, UI `session_id` ile okuyordu. Veri asla görüntülenemiyordu.
- **Çözüm:** Tüm dijimagic API route'larında `cookieStore.get('session_id')` → `cookieStore.get('user_id')` olarak değiştirildi (15 dosya, 20 satır).
...
- **Dosyalar:** `lib/dijimagic/apifyCompetitorProvider.ts`, `app/api/dijimagic/competitors/google-auction/route.ts`, `app/api/dijimagic/competitors/meta-ad-library/route.ts`
...
- **Dosyalar:** `lib/dijimagic/apifyCompetitorProvider.ts`, `lib/dijimagic/competitorScanner.ts`, `app/api/dijimagic/competitors/google-auction/route.ts`, `app/api/dijimagic/competitors/meta-ad-library/route.ts`, `.env.example`, `docs/CHANGELOG.md`
...
- **Çözüm:**
  - **dijimagic_recommendation_results migration** (yeni) — `supabase/migrations/20260510008000_create_dijimagic_recommendation_results.sql`: proposal_id, approval_id, source_campaign_id, platform, before_snapshot, after_snapshot, metric_delta, outcome (pending/improved/no_change/declined/insufficient_data), outcome_summary, status, RLS user-owned.
  ...
  - **ApprovalHistoryPanel.tsx** güncellendi — expand açılınca `/api/dijimagic/results` lazy fetch; outcome badge (improved=emerald, declined=red, no_change/insufficient_data/pending=gray) satır header'ında gösterilir; expanded detail'de "Öneri Sonucu" satırı eklendi.
- **Dosyalar:** `supabase/migrations/20260510008000_create_dijimagic_recommendation_results.sql`, `lib/dijimagic/resultTrackingStore.ts`, `app/api/dijimagic/results/route.ts`, `components/dijimagic/ApprovalHistoryPanel.tsx`
...
- **Dosyalar:** `lib/dijimagic/publishSafety.ts`, `lib/dijimagic/publishPayloadValidator.ts`, `lib/dijimagic/policyGuard.ts`, `app/api/dijimagic/one-click-approve/route.ts`, `components/dijimagic/OneClickApproveDialog.tsx`
...
- **Çözüm:**
  ...
  - **CompetitorDashboard.tsx** güncellendi — Header'a Meta / Google bağlantı durumu badge'leri eklendi (connected=emerald, missing_key/unavailable=gray). Google status `/api/dijimagic/competitors/google-auction` üzerinden fire-and-forget ile kontrol edilir.
  - **.env.example** güncellendi — `SERPAPI_API_KEY` ve `GOOGLE_ADS_TRANSPARENCY_PROVIDER=serpapi` eklendi.
- **Dosyalar:** `lib/dijimagic/googleTransparencyConnector.ts`, `lib/dijimagic/competitorScanner.ts`, `app/api/dijimagic/competitors/google-auction/route.ts`, `components/dijimagic/CompetitorDashboard.tsx`, `.env.example`
...
- **Çözüm:**
  - **dijimagic_approval_versions tablosu** — `supabase/migrations/20260510007000_create_dijimagic_approval_versions.sql`: approval başına versiyon satırları (original/edited/regenerated/manual); unique (approval_id, version_number); RLS user-owned; audit immutability (UPDATE/DELETE policy yok).
  ...
  - **GET /api/dijimagic/approvals** — Her kayda `decision_badge` (latest judge decision özeti) eklendi; tek batch sorgusu (N+1 yok). List endpoint korundu.
  - **GET /api/dijimagic/approvals/[id]** — `decisionRows` (tam rol çıktıları) + `versionCount` eklendi; concurrent fetch ile performans korundu.
  - **POST/GET /api/dijimagic/approvals/[id]/versions** — Yeni route; GET versiyon listesi, POST versiyon oluşturma (`original` için idempotent).
...
- **Dosyalar:** `supabase/migrations/20260510007000_*`, `lib/dijimagic/approvalStore.ts`, `lib/dijimagic/modelDecisionStore.ts`, `app/api/dijimagic/approvals/route.ts`, `app/api/dijimagic/approvals/[id]/route.ts`, `app/api/dijimagic/approvals/[id]/versions/route.ts`, `components/dijimagic/DecisionDeskSummary.tsx`, `components/dijimagic/ApprovalVersionPanel.tsx`, `components/dijimagic/AiAdSuggestions.tsx`, `components/dijimagic/ApprovalHistoryPanel.tsx`
...
- **Çözüm:**
  - **dijimagic_model_decisions tablosu** — `supabase/migrations/20260510006000_create_dijimagic_model_decisions.sql`: 5 rol (strategist/creative/risk_policy/technical_validator/judge) ve 4 provider (openai/anthropic/gemini/deterministic) için audit kayıtları; RLS ile kullanıcı yalnızca kendi kayıtlarını okur.
  ...
  - **generate-ad/route.ts** — `DIJIMAGIC_MULTI_AI_ENABLED=true` ise synthesis paketleri üzerinden desk çalışır; disabled ise hiçbir provider çağrısı olmaz. Pending approval mapping ve publish lifecycle dokunulmadı.
...
- **Dosyalar:** `supabase/migrations/20260510006000_create_dijimagic_model_decisions.sql`, `lib/dijimagic/multiAiTypes.ts`, `lib/dijimagic/aiProviders/providerGuards.ts`, `lib/dijimagic/aiProviders/openaiProvider.ts`, `lib/dijimagic/aiProviders/anthropicProvider.ts`, `lib/dijimagic/aiProviders/geminiProvider.ts`, `lib/dijimagic/modelDecisionStore.ts`, `lib/dijimagic/multiAiDecisionDesk.ts`, `lib/dijimagic/adCreator.ts` (additive), `app/api/dijimagic/generate-ad/route.ts` (additive)
...
- **Çözüm:**
  - **CRON_SECRET fail-secure** — `app/api/dijimagic/daily-run/route.ts` ve `app/api/strategy/jobs/runner/route.ts`: `CRON_SECRET` env yokken production'da 503 döner; local/dev'de bypass açık. Önceki `if (cronSecret && ...)` pattern güvensizdi.
  - **META_WEBHOOK_VERIFY_TOKEN** — `app/api/meta/webhook/route.ts`: Hardcoded `'dijimagic_webhook_verify_2024'` fallback kaldırıldı. Env yoksa GET verification 503 döner; production'da sessiz bypass yok.
  ...
  - **`.env.example` oluşturuldu** — A–I grupları: Supabase, AI Providers (OpenAI/Anthropic/Gemini), Google/Google Ads, Meta, DijiAlgoritma Safety Flags, Cron/Security, Competitor Intelligence, Payments, App/Auth. Tüm değerler placeholder.
- **Dosyalar:** `.env.example` (yeni), `app/api/dijimagic/daily-run/route.ts`, `app/api/strategy/jobs/runner/route.ts`, `app/api/meta/webhook/route.ts`, `lib/meta/crypto.ts`, `lib/supabase/client.ts`
...
- **Dosyalar:**
  - YENİ `lib/dijimagic/synthesisTypes.ts`
  - YENİ `lib/dijimagic/synthesisEngine.ts`
  - `lib/dijimagic/adCreator.ts` (synthesis context entegre)
  - `app/api/dijimagic/generate-ad/route.ts` (synthesis packages build + pass)
  - `docs/CHANGELOG.md`
...
- **Doğrulama:** `npx tsc --noEmit` → 0 error · `npm run build` → ✓ /api/dijimagic/competitors/{analyze,google-auction,meta-ad-library} ve /api/dijimagic/generate-ad temiz derlendi · /dijimagic bundle 24.9 kB sabit (server-only persistence; client bundle büyümedi).
...
- **Dosyalar:**
  - `supabase/migrations/20260510005000_create_dijimagic_competitor_ads.sql` (yeni)
  - `supabase/migrations/20260510005100_create_dijimagic_competitor_insights.sql` (yeni)
  - `lib/dijimagic/competitorAdStore.ts` (yeni)
  - `lib/dijimagic/competitorInsightStore.ts` (yeni)
  - `app/api/dijimagic/competitors/meta-ad-library/route.ts` (additive persist + camelCase response korundu)
  - `app/api/dijimagic/competitors/analyze/route.ts` (additive persist + campaign type binding)
  - `app/api/dijimagic/competitors/google-auction/route.ts` (dürüst unsupported response)
  - `components/dijimagic/CompetitorDashboard.tsx` (kalıcı analiz banner'ı)
  - `lib/dijimagic/adCreator.ts` (additive persistedCompetitorContext param)
  - `app/api/dijimagic/generate-ad/route.ts` (kalıcı insight prompt context wiring)
...
- **Korunanlar:**
  - `lib/dijimagic/platformKnowledge.ts` — DOKUNULMADI (legacy hardcoded knowledge devam ediyor; runStructuralAnalysis aynen çalışıyor).
  - `lib/dijimagic/googleRuleEngine.ts` — DOKUNULMADI (12 kural aynen).
  - `lib/dijimagic/meta/diagnosis.ts`, `decision.ts`, `orchestrator.ts` — DOKUNULMADI.
  - `lib/dijimagic/metaDeepFetcher.ts`, `googleDeepFetcher.ts` — DOKUNULMADI (zorunlu değildi).
  - `app/api/integrations/meta/**`, `app/api/integrations/google-ads/**`, `app/api/integrations/tiktok-ads/**` — DOKUNULMADI.
  - `app/api/dijimagic/one-click-approve/route.ts`, `execute-action/route.ts`, daily-run wiring (route'tan değil deepAnalysis'ten orchestre edilir) — DEĞİŞMEDİ.
...
- **Doğrulama:** `npx tsc --noEmit` → 0 error · `npm run build` → ✓ tüm route'lar derlendi · /dijimagic bundle 24.9 kB sabit (backend-only) · Migration JSONB literal'ları (99 adet) Node.js JSON.parse ile doğrulandı.
...
- **Dosyalar:**
  - `supabase/migrations/20260510004000_create_dijimagic_platform_doctrine.sql` (yeni, 10 campaign type seed)
  - `lib/dijimagic/campaignTypeIntelligence.ts` (yeni)
  - `lib/dijimagic/platformDoctrineStore.ts` (yeni)
  - `lib/dijimagic/analysisTypes.ts` (additive `campaignTypeIntelligence` alanı)
  - `lib/dijimagic/deepAnalysis.ts` (try/catch enrichment loop)
  - `lib/dijimagic/adCreator.ts` (doctrine summaries + buildPrompt opsiyonel param + 1 satır system prompt kural)
...
- **Çözüm:**
  - **Pending count metric**: `app/dijimagic/page.tsx`'e yeni state `approvalsPendingCount` + `refreshApprovalsPendingCount()` callback (mount + onApprovalChanged'da fetch). `CommandCenterHeader` yeni prop `approvalsPendingCount?: number` — tanımlıysa onu, değilse `health.pendingApprovals` (drafts.length) fallback. Endpoint hata verirse mevcut değer korunur (graceful degrade).
...
- **Korunanlar:**
  ...
  - Manual "AI Reklam Oluştur" CTA ([page.tsx:368](app/dijimagic/page.tsx#L368) `setShowAdWizard(true)`) — wizardProposal=null kalır → boş wizard açılır (Düzenle ile karışmaz)
  - `lib/dijimagic/meta/orchestrator.ts`, adCreator, daily-run, deepFetcher'lar, diagnosis, decision — DOKUNULMADI
...
- **Doğrulama:** `npx tsc --noEmit` → 0 error · `npm run build` → ✓ tüm route'lar derlendi · /dijimagic bundle 23.5→24.9 kB.
- **Migration apply gereksinimi:** YOK. Faz 0C'deki `dijimagic_pending_approvals` tablosu yeterli (edited_payload kolonu zaten Faz 0C'de hazırlanmıştı, bu fazda kullanılmaya başlandı).
- **Dosyalar:**
  - `components/dijimagic/ApprovalHistoryPanel.tsx` (yeni)
  - `app/dijimagic/page.tsx` (pending count state + history panel render + onApprovalChanged)
  - `components/dijimagic/CommandCenterHeader.tsx` (approvalsPendingCount prop + fallback)
  - `components/dijimagic/AiAdSuggestions.tsx` (Düzenle aktif + handleEdit + onApprovalChanged callback)
...
- **Çözüm:**
  - **Yeni migration** `20260510003000_create_dijimagic_pending_approvals.sql` — `signups(id) UUID FK + ON DELETE CASCADE`, status enum (pending/approved/rejected/hold/editing/published/failed/expired), proposal_snapshot JSONB, rejection_reason/hold_reason/status_reason, edited_payload, approved_at/rejected_at/held_at/published_at/failed_at, publish_audit_id (FK dijimagic_publish_audit_log), metadata JSONB, 8 index, 4 RLS policy + unique (user_id, proposal_id) duplicate guard.
  - **Yeni helper** `lib/dijimagic/approvalStore.ts` — `bulkInsertPendingApprovalsIfMissing()` (proposal listesi için tek-roundtrip insert + ON CONFLICT DO NOTHING), `upsertPendingApprovalFromProposal()` (per-row, protected status'ları korur), `listApprovals(filters)`, `getApprovalById()`, `updateApprovalStatus()` (transition guard'lı), `markApprovalPublished()`, `markApprovalFailed()`, `recordPublishAttemptOnApproval()` (status değiştirmez, sadece metadata.last_publish_attempt yazar), `countPendingApprovals()`. Tablo yoksa AUDIT_LOSS log + null/false döner.
  - **Yeni API route** `app/api/dijimagic/approvals/route.ts` — GET: list/filter/count (status csv, platform, limit, ?count=1).
  - **Yeni API route** `app/api/dijimagic/approvals/[id]/route.ts` — GET single, PATCH status update (sadece pending/rejected/hold/editing — published/approved/failed direkt yazılamaz). `ALLOWED_USER_TRANSITIONS` matrix ile geçiş guard'ı: `INVALID_TRANSITION → 409`, `NOT_FOUND → 404`, `TABLE_MISSING → 503`.
  ...
  - **AiAdSuggestions UI** — yeniden yazıldı:
    - Mount'ta `/api/dijimagic/approvals?limit=200` ile approval state çekilir.
...
- **Korunanlar:**
  - `lib/dijimagic/meta/orchestrator.ts`, `lib/dijimagic/meta/diagnosis.ts`, `lib/dijimagic/meta/decision.ts`, `lib/dijimagic/adCreator.ts`, `lib/dijimagic/deepAnalysis.ts`, `lib/dijimagic/metaDeepFetcher.ts`, `lib/dijimagic/googleDeepFetcher.ts` — DOKUNULMADI.
  - `app/api/integrations/meta/**`, `app/api/integrations/google-ads/**`, `app/api/integrations/tiktok-ads/**` — DOKUNULMADI.
  - `app/api/dijimagic/execute-action/route.ts`, `daily-run`, `command-center`, `competitors/*`, Search/Display/PMax wizard'ları — DOKUNULMADI.
  ...
  - Command Center metrikleri (CommandCenterHeader pendingApprovals = ccData.drafts.length) bu fazda DEĞİŞTİRİLMEDİ — `/api/dijimagic/approvals?count=1` endpoint'i hazır, UI binding sonraki minor PR'da yapılabilir.
...
- **Doğrulama:** `npx tsc --noEmit` → 0 error · `npm run build` → ✓ tüm route'lar derlendi · /dijimagic bundle 21.6→23.5 kB.
- **Migration apply gereksinimi:** `supabase/migrations/20260510003000_create_dijimagic_pending_approvals.sql` production'da uygulanmalı. Faz 0A'daki `dijimagic_publish_audit_log` zaten dependency (publish_audit_id FK).
- **Dosyalar:**
  - `supabase/migrations/20260510003000_create_dijimagic_pending_approvals.sql` (yeni)
  - `lib/dijimagic/approvalStore.ts` (yeni)
  - `app/api/dijimagic/approvals/route.ts` (yeni)
  - `app/api/dijimagic/approvals/[id]/route.ts` (yeni)
  - `app/api/dijimagic/generate-ad/route.ts` (approval upsert wiring)
  - `app/api/dijimagic/one-click-approve/route.ts` (approvalId binding + status update)
  - `components/dijimagic/AiAdSuggestions.tsx` (yeniden yazıldı: aksiyon row + modal'lar)
  - `components/dijimagic/OneClickApproveDialog.tsx` (approvalId + onPublished props)

---

## 2026-05-10 — Faz 0B: Publish audit binding + budget guard + explicit confirmation
- **Sorun:** Faz 0A'da dijimagic_publish_audit_log tablosu ve publishAuditStore helper'ı eklendi ama publish akışına bağlanmamıştı. Ek olarak: budget guard yoktu (proposal'daki dailyBudget cap'siz Meta'ya iletiliyordu), idle phase'de explicit checkbox onay yoktu, partial failure orphan kaynaklar hiçbir yerde izlenmiyordu.
- **Çözüm:**
  - **Budget guard** (`one-click-approve/route.ts`): `evaluateBudgetGuard()` helper Meta API çağrısından ÖNCE çalışır. Cap kaynağı `process.env.DIJIMAGIC_MAX_DAILY_BUDGET_TRY` (default 1000 TRY). 3 blok kodu: `BUDGET_GUARD_BLOCKED` (cap aşımı), `BUDGET_MISSING_OR_INVALID` (NaN/<=0), `UNSUPPORTED_BUDGET_CURRENCY` (TRY dışı). Tümü 422 + audit `blocked`.
...
- **Korunanlar:**
  - `lib/dijimagic/meta/orchestrator.ts` — DOKUNULMADI. Partial result mevcut yapısı yeterli.
  - `app/api/integrations/meta/**`, `app/api/integrations/google-ads/**` — DOKUNULMADI.
  - `lib/dijimagic/adCreator.ts`, AI proposal promptları, daily-run, deepAnalysis, googleDeepFetcher, metaDeepFetcher, diagnosis, decision — DOKUNULMADI.
...
- **Doğrulama:** `npx tsc --noEmit` → 0 error · `npm run build` → ✓ tüm route'lar derlendi · /dijimagic bundle 21.3→21.6 kB (idle phase detayları).
- **Dosyalar:**
  - `app/api/dijimagic/one-click-approve/route.ts` (budget guard + audit binding)
  - `components/dijimagic/OneClickApproveDialog.tsx` (idle phase: checkbox + detaylar)
- **Migration apply:** Faz 0A migration'larının (özellikle `dijimagic_publish_audit_log`) production'da uygulanmış olması gerekir. Uygulanmadıysa `recordPublishAuditAttempt` AUDIT_LOSS log atar ama publish akışı kırılmaz; response'da `auditWarning: 'AUDIT_LOG_WRITE_FAILED'` döner.

---

## 2026-05-10 — Faz 0A: DijiAlgoritma audit foundation (DB migration + RLS + audit log)
- **Sorun:** DijiAlgoritma Merkezi'nin onay/yayın denetim altyapısı eksikti — `dijimagic_action_outcomes` migration'da kayıtlı değildi (sadece `docs/sql/` altında manuel SQL), `learningStore.ts` tablo yoksa sessizce no-op'a düşüyordu (audit kaybı), `dijimagic_daily_runs`/`dijimagic_articles` için FK + RLS yoktu, publish denetimi için bir tablo da yoktu.
- **Çözüm:**
  - Yeni migration `20260510000000_create_dijimagic_action_outcomes.sql` — `docs/sql/dijimagic_action_outcomes.sql` formal migration'a taşındı; idempotent (IF NOT EXISTS / DROP POLICY IF EXISTS); RLS aktif; per-user select/insert/update policy; user_id TEXT olarak korundu (mevcut veriyle uyum).
  - Yeni migration `20260510001000_create_dijimagic_publish_audit_log.sql` — yeni `dijimagic_publish_audit_log` tablosu (UUID FK signups(id) ON DELETE CASCADE; status: pending/success/failed/blocked/orphaned/rolled_back; payload_hash, payload/response_excerpt JSONB, orphan_resources JSONB, budget_amount/currency); 4 index; RLS + policies.
  - Yeni migration `20260510002000_dijimagic_runs_articles_rls_fk.sql` — `dijimagic_daily_runs`/`dijimagic_articles` için TEXT user_id'yi UUID'ye cast eder ve `signups(id) ON DELETE CASCADE` FK ekler. Geçersiz UUID veya orphan referans varsa **RAISE EXCEPTION** ile durur (transaction rollback olur, veri değişmez); manuel cleanup gerektiğini bildirir. RLS aktif; per-user select/insert/update/delete policy. Idempotent (zaten UUID/FK varsa skip eder).
  - `lib/dijimagic/learningStore.ts` — tablo yokken sessiz no-op davranışı **AUDIT_LOSS** structured log'a dönüştürüldü (insert için `console.error`, list için `console.warn`); fonksiyon hala `null`/`[]` döner, çağıran flow kırılmaz.
  - Yeni helper `lib/dijimagic/publishAuditStore.ts` — `recordPublishAuditAttempt()`, `updatePublishAuditStatus()`, `hashPayload()` (SHA-256), `sanitizeResponseExcerpt()` (recursive token/secret redact + size cap). Faz 0A'da hiçbir publish endpoint'ine bağlanmadı; sonraki fazda non-blocking şekilde kullanılacak.
...
- **Dosyalar:**
  - `supabase/migrations/20260510000000_create_dijimagic_action_outcomes.sql` (yeni)
  - `supabase/migrations/20260510001000_create_dijimagic_publish_audit_log.sql` (yeni)
  - `supabase/migrations/20260510002000_dijimagic_runs_articles_rls_fk.sql` (yeni)
  - `lib/dijimagic/learningStore.ts` (AUDIT_LOSS log)
  - `lib/dijimagic/publishAuditStore.ts` (yeni helper, henüz bağlanmadı)
...
## 2026-04-20 — DijiAlgoritma persisted stale error için hot-heal
...
- **Çözüm:** app/dijimagic/page.tsx içine hot-heal useEffect: ccData.errors içinde "Meta bağlantısı" içeren bir hata varsa, localStorage cache temizlenir ve `triggerBackgroundBootstrap()` tetiklenerek yeni bir daily-run (fix uygulanmış) çalıştırılır. Tek seferlik guard (`healedRef`).
- **Dosyalar:** app/dijimagic/page.tsx

---

## 2026-04-20 — DijiAlgoritma "Meta bağlantısı yok" yanılgısı düzeltildi
- **Sorun:** Entegrasyon sayfası Meta'yı "Bağlı" gösterirken DijiAlgoritma "Meta bağlantısı bulunamadı" uyarısı veriyordu. İki sayfa farklı kaynak okuyor: Entegrasyon cookie (süre dolmuşsa bile token varsa "bağlı"), DijiAlgoritma ise `meta_connections` DB tablosundaki `status='active'` + expiry kontrolü.
- **Çözüm:** `metaDeepFetcher.ts` içine 3. tier cookie fallback eklendi. DB lookup başarısız olursa, cookie'deki `meta_access_token` + `meta_selected_ad_account_id` + expiry kontrolü yapılır. Geçerliyse DijiAlgoritma da bağlantıyı Entegrasyon ile aynı kaynaktan çözer. Integration kodu değiştirilmedi.
- **Dosyalar:** lib/dijimagic/metaDeepFetcher.ts
...
## 2026-04-20 — DijiMagic sayfa genişlik + KPI font küçültme
...
- **Dosyalar:** app/dijimagic/page.tsx, components/dijimagic/KpiDashboard.tsx
...
- **Çözüm:**
  - `lib/dijimagic/metaDeepFetcher.ts`: ad fetch field'larına `creative{body,title,link_url,call_to_action_type,object_story_spec}` eklendi. Artık her ad için gerçek `creativeBody`, `creativeTitle`, `callToActionType`, `linkUrl` DB'ye geliyor.
  - `lib/dijimagic/analysisTypes.ts`: `AdInsight` tipine yukarıdaki 4 alan opsiyonel olarak eklendi.
  - `lib/dijimagic/competitorAnalyzer.ts`: `THEME_LEXICON` (6 kategori, kategoriye 15-20 kelime) + `CTA_PHRASES` (28 ifade) + `CTA_BUTTON_LABEL` (20 Meta enum → TR). `analyzeUserAds` artık creative body + title + name birleşimi üzerinde tema çıkarıyor; ayrıca `ad.callToActionType` (Meta enum) `ctaTypes`'a ekleniyor.
  ...
  - `lib/dijimagic/adCreator.ts`: AI response post-processing'de **hard guard** — `competitorAds.length === 0` ise `competitorInsight` zorla şu metne overridelenir: "Meta Ad Library'den rakip reklam bulunamadı (anahtar kelime eşleşmesi yok). Karşılaştırma yapılmadı." AI artık rakip yokken uydurma karşılaştırma üretemez.
- **Dosyalar:** lib/dijimagic/analysisTypes.ts, lib/dijimagic/metaDeepFetcher.ts, lib/dijimagic/competitorAnalyzer.ts, lib/dijimagic/adCreator.ts
...
- **Dosyalar:** components/dijimagic/AdPreviewCard.tsx

---

## 2026-04-20 — DijiAlgoritma hata banner'ları: "Yeniden Bağla" CTA'sı ile aksiyona çevrildi
...
## 2026-04-20 — DijiAlgoritma sayfası: hafif yeşil arkaplan + daha okunaklı typography
- **Sorun:** DijiMagic sayfa arkaplanı beyaz/gri boş duruyordu ve yazılar küçüktü; Meta Ads wizard'ının kullandığı yumuşak yeşil gradient + daha büyük font hiyerarşisi referans alınarak benzer aesthetic istendi.
- **Çözüm:** (1) `app/dijimagic/page.tsx` arkaplan `bg-gray-50` → `bg-gradient-to-b from-emerald-50/40 via-white to-emerald-50/20`. (2) Banner metni `text-xs` → `text-sm`. (3) `KpiDashboard` etiket `text-[11px]`→`text-[13px]`, değer `text-lg`→`text-xl`, ikon alanı 7×7→8×8, padding 4→5. (4) `CommandCenterHeader` başlık `text-xl`→`text-2xl`, açıklama `text-xs`→`text-sm`, stats kutuları ikon 7→9, etiket `text-[9px]`→`text-[11px]`, değer `text-sm`→`text-base`; durum pill'leri ve "AI Reklam Oluştur" butonu daha büyük. (5) `AiAdSuggestions` başlık `text-lg`→`text-xl`, alt metin `text-xs`→`text-sm`, butonlar daha büyük. (6) `AnalysisCapabilities` başlık + kart fontları büyütüldü, kart padding 4→5.
- **Dosyalar:** app/dijimagic/page.tsx, components/dijimagic/KpiDashboard.tsx, components/dijimagic/CommandCenterHeader.tsx, components/dijimagic/AiAdSuggestions.tsx, components/dijimagic/AnalysisCapabilities.tsx
...
- **Çözüm:** `CLAUDE.md`'ye **UI Renk Kuralı (YASAK)** bölümü eklendi — `bg-amber-*`, `text-amber-*`, `border-amber-*`, `bg-yellow-*`, `text-yellow-*`, `border-yellow-*` class'ları artık projede kullanılmayacak. Onayları gösteren renk paleti tanımlandı (info=gray, important=primary, critical=red, success=emerald). Mevcut tüm `components/dijimagic/*` ve `app/dijimagic/*` dosyalarındaki amber/yellow class'ları toplu olarak gray tonlarına çevrildi.
- **Dosyalar:** CLAUDE.md, app/dijimagic/page.tsx, components/dijimagic/* (20+ dosya)

---

## 2026-04-20 — DijiAlgoritma otomatik bootstrap + cron 16:15'e alındı
- **Sorun:** Kullanıcı DijiMagic sayfasını açtığında hiç veri yoksa "İlk Analizi Başlat" butonuna basmak zorunda kalıyordu. Talep edilen davranış: sayfa açılır, kullanıcı hiçbir şey yapmadan arka planda analiz başlar ve hazır olunca görünür.
- **Çözüm:** (1) `app/dijimagic/page.tsx` — sayfa mount edildiğinde eğer backend'de de cache'te de veri yoksa otomatik olarak `/api/dijimagic/daily-run` POST tetiklenir (arka planda), bu sırada banner'da loader + "İlk analiz arka planda hazırlanıyor…" mesajı gösterilir; bitince otomatik fetch ile veri yerleşir. (2) Vercel cron `0 13 * * *` → `15 13 * * *` (16:15 Istanbul). UI metinleri "16:15" olarak güncellendi.
- **Dosyalar:** app/dijimagic/page.tsx, vercel.json
...
## 2026-04-20 — DijiAlgoritma: "Taranıyor" ve tüm loading flash'ları kaldırıldı
...
- **Çözüm:** (1) CommandCenterHeader: loading UI tamamen kaldırıldı — her durumda statü pill'i "AI Analiz / Kural Motoru / Hazır" olarak gösterilir, health yoksa stats kutularında "—" placeholder; skeleton animate-pulse silindi. (2) KpiDashboard: loading skeleton kaldırıldı, kpis yoksa "—" gösterir. (3) AiAdSuggestions: proposals + diagnoses + decisions için localStorage cache (`dijimagic_proposals_cache_v1`); cache varsa loading=false başlar; cache+proposal yoksa hiçbir skeleton render edilmez, component null döner.
- **Dosyalar:** components/dijimagic/CommandCenterHeader.tsx, components/dijimagic/KpiDashboard.tsx, components/dijimagic/AiAdSuggestions.tsx

---

## 2026-04-20 — DijiAlgoritma sayfa yenilemede "Taranıyor" görünmez (localStorage cache)
...
- **Dosyalar:** vercel.json, app/dijimagic/page.tsx
...
- **Dosyalar:** components/dijimagic/AdPreviewCard.tsx, public/digital-ads.jpg

---

## 2026-04-20 — DijiAlgoritma: Kampanya İzleme (InsightStream) kaldırıldı
...
- **Çözüm:** `app/dijimagic/page.tsx` içinden InsightStream render'ı, import'u ve `insightsForStream` derivation'ı kaldırıldı. Component dosyası korundu. İzleme artık sadece cron + daily-run içinden arka planda yapılıyor; UI'da görünmüyor.
- **Dosyalar:** app/dijimagic/page.tsx

---

## 2026-04-20 — DijiAlgoritma: sayfa yenileme artık tarama tetiklemiyor
...
- **Çözüm:** `/api/dijimagic/command-center` artık sadece persisted veri döner; persisted yoksa `data: null` ile anında yanıtlar (UI "İlk Analizi Başlat" boş state gösterir). Önceki live `runDeepAnalysis()` fallback'i kaldırıldı. `/api/dijimagic/generate-ad` da aynı mantıkla: `forceGenerate=false` (normal yükleme) → persisted varsa hemen döner, yoksa boş döner — live üretim yalnızca kullanıcı "Yeniden Oluştur" butonuna bastığında (`forceGenerate=true`) çalışır. Taramalar sadece günlük cron (`GET /api/dijimagic/daily-run`) ve kullanıcının manuel tetiklediği "İlk Analizi Başlat" / "Yeniden Oluştur" yollarıyla yapılır.
- **Dosyalar:** app/api/dijimagic/command-center/route.ts, app/api/dijimagic/generate-ad/route.ts

---

## 2026-04-20 — DijiAlgoritma sayfasında eski kartlar kaldırıldı
- **Sorun:** Günlük Brifing, Hesap Durumu (HealthScore), Bütçe Dağılımı, Haftalık Özet kartları artık kullanılmıyor.
- **Çözüm:** `app/dijimagic/page.tsx` içinden DailyBriefing, HealthScore, SmartBudgetPanel, WeeklyReport render'ları ve import'ları kaldırıldı. Component dosyaları korundu (başka bir yerde ihtiyaç olursa).
- **Dosyalar:** app/dijimagic/page.tsx

---

## 2026-04-20 — DijiAlgoritma otomatik akış: teşhis + tek-tık onay
- **Sorun:** Teşhis / preflight / creative / orchestrator modülleri hazır olsa da yalnızca wizard üzerinden (adım adım) erişilebiliyordu. Kullanıcı "DijiMagic sayfası açılsın, hazır öneri + teşhis + tek tıkla onay" deneyimi bekliyordu.
- **Çözüm:** (1) `generate-ad` ve `daily-run` çağrıları artık proposal'larla birlikte `diagnoses` + `decisions` de üretip kayda yazıyor (sadece Meta; mevcut akışı bozmuyor). (2) Yeni `/api/dijimagic/one-click-approve` endpoint'i — auto-discover (tek page/pixel/form varsa otomatik seçer), gerekirse `NEEDS_INPUT` ile kullanıcıdan eksik seçimleri ister, preflight → AI görsel üret → Meta'ya yükle → orchestrator full stack (campaign+adset+ad+creative, tümü PAUSED) → learning store kaydı — hepsi tek istekte. (3) UI: `AiAdSuggestions` her Meta proposal'ı altında teşhis mini-kart (root cause + confidence + önerilen aksiyon) ve "Tek Tıkla Onayla" butonu gösterir; buton `OneClickApproveDialog`'u açar (progress ekranı + NEEDS_INPUT pickerları).
- **Dosyalar:** app/api/dijimagic/generate-ad/route.ts, app/api/dijimagic/daily-run/route.ts, app/api/dijimagic/one-click-approve/route.ts, components/dijimagic/OneClickApproveDialog.tsx, components/dijimagic/AiAdSuggestions.tsx

---

## 2026-04-20 — DijiAlgoritma DiagnosisPanel + Learning Layer v1 (veri biriktirme)
...
- **Çözüm:** `DiagnosisPanel` standalone bileşeni eklendi — kampanya listesini alır, `/api/dijimagic/diagnose` çağırır, root cause + evidence + önerilen aksiyonları expandable card'lar ile gösterir (drop-in; mevcut sayfalara otomatik entegre edilmedi, parent component'in yerleştirmesi gerekir). Learning Layer v1: `lib/dijimagic/learningStore.ts` + `/api/dijimagic/actions/record` + `/api/dijimagic/actions/outcomes` endpoint'leri — öneriler ve kullanıcı uygulamaları (applied/rejected) kayda geçer; sonuç analizi v2'ye bırakıldı. Supabase tablosu `dijimagic_action_outcomes` yoksa sessizce no-op'a düşer (hata yok). SQL şeması `docs/sql/dijimagic_action_outcomes.sql`'de.
- **Dosyalar:** components/dijimagic/DiagnosisPanel.tsx, lib/dijimagic/learningStore.ts, app/api/dijimagic/actions/record/route.ts, app/api/dijimagic/actions/outcomes/route.ts, docs/sql/dijimagic_action_outcomes.sql

---

## 2026-04-20 — DijiAlgoritma diagnosis + decision katmanları (çok değişkenli teşhis)
...
- **Çözüm:** `lib/dijimagic/meta/diagnosis.ts` — CTR+CPM+frequency+CPA/CPC+dönüşüm trendi birlikte okunarak 9 tane root cause (hook_problem, landing_page_problem, creative_fatigue, audience_mismatch, event_quality_problem, insufficient_data, budget_starvation, wrong_optimization_goal, pixel_misfire) tanımlandı; her teşhise confidence + evidence eşlik eder. `lib/dijimagic/meta/decision.ts` — her root cause'u monitor/tweak/revise/recreate/change_objective aksiyonlarına bağlar. `/api/dijimagic/diagnose` endpoint'i eklendi (read-only, hiçbir Meta kaynağına dokunmaz). Mevcut analiz akışı etkilenmedi; bu yeni katman opsiyonel olarak çağırılır.
- **Dosyalar:** lib/dijimagic/meta/diagnosis.ts, lib/dijimagic/meta/decision.ts, app/api/dijimagic/diagnose/route.ts

---

## 2026-04-20 — DijiAlgoritma Wizard: kreatif pipeline (AI görsel + Meta upload)
- **Sorun:** Preflight adımı sonrası kullanıcıya "Ads Manager'dan ekle" mesajı dönüyor, reklam (ad + creative) gerçek oluşmuyordu. DijiAlgoritma "taslak üret ve onaylat" vaadini tamamlamıyordu.
...
- **Dosyalar:** components/dijimagic/MetaCreativePanel.tsx, components/dijimagic/AdCreationWizard.tsx

---

## 2026-04-20 — DijiAlgoritma Wizard: Meta preflight UI adımı
...
- **Dosyalar:** `app/api/dijimagic/business-profile/route.ts`, `app/api/dijimagic/business-profile/scan/route.ts`
