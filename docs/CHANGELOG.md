...
## 2026-06-19 — Web Site Yöneticisi: Builder Workspace sade/premium yeniden düzen (#builder-8d)
- **Sorun:** Builder Workspace düzeni dağınıktı ("saçma sapan"): sol dar rail'e "Sayfalar" + "Kredi kullanımı" + "AI Asistan" üst üste sıkışmıştı; tuvalin altına yüzen "Bu tasarımı beğendiniz mi? Reddet/Düzenle/Onayla ve Yayınla" kartı biniyordu (gereksiz tekrar); tuval küçük/sıkışık hissediliyordu. Hedef: sakin, ferah, 3-panelli Promake/Lovable kalitesinde stüdyo editör — tüm işlevsellik korunarak yalnız yeniden düzen + görsel yükseltme.
- **Çözüm:** (1) **Topbar** inceltildi (~54px, sessiz chrome): sol = geri + site adı (+ alt adres), orta = geri/ileri al + DeviceSwitcher, sağ = sade **kredi pill'i** (yalnız spark + sayı — eski faz-çubuğu DEĞİL) + Yenile + Tam ekran + Önizleme + Yönet + Yayınla (emerald birincil). (2) **Sayfa-sekmeleri şeridi** (`PageTabsStrip`, ~40px): sayfalar yatay kompakt sekme (aktif = emerald alt-çizgi + hafif zemin), çok sayfada yatay scroll, çok-dilli sitede sağ uçta dil seçici — sol-rail "Sayfalar" bloğunun yerine. (3) **SOL rail (340–356px) = YALNIZ AI sohbet** (kredi + sayfa blokları kaldırıldı; ferah, birincil düzenleme yüzeyi). (4) **ORTA = BÜYÜK tuval (kahraman)**: yumuşak emerald-tonlu zemin + masaüstünde de hafif padding (24px) + katmanlı yumuşak gölge + 14px köşe (cihaz çerçevesi hissi); **yüzen onay kartı tamamen KALDIRILDI** — yalnız sade köşe **"Taslak" rozeti**. (5) **SAĞ = daraltılabilir Müfettiş**: varsayılan daraltılmış ince dikey rail (tuval büyük başlar), tuvalde ilk seçimde otomatik genişler; başlıkta daralt/genişlet chevron'u. (6) **Kredi adım-çizelgesi** kalıcı panel olmaktan çıktı → yalnız aktif üretim/revizyon sırasında tuvalin üstünde merkeze gelen GEÇİCİ overlay (`GenerationTimelineOverlay`), iş bitince kaybolur; detaylı geçmiş Yönet drawer'ında, kalıcı kredi bilgisi topbar pill'inde. Yüzen kartın aksiyonları zaten kapsanıyor: Yayınla = topbar; Reddet/Düzenle = sol sohbet (Düzenle/Baştan üret toggle'ı). TÜM handler'lar korundu: chat → `/patch`(seçili blok) / `/generate`(sayfa revize) + baştan üret reject; görsel-seçim postMessage → inspector; publish/unpublish; rollback; cihaz/sayfa değişimi; 402 → AccessRequiredModal; reloadKey tazeleme; C-1 fix (chat deps `activeSlugSafe`+`previewLocale`); iframe sandbox izolasyonu. Eski standalone `revise`/`approve` handler'ları (yüzen kartın) artık tekrar olduğu için kaldırıldı (reddet/düzenle = chat, yayınla = topbar Yayınla popup'ı — aynı endpoint'ler). amber/sarı yok; tr+en (`builder.collapseInspector`/`expandInspector`); yeni bağımlılık yok; lib/meta + lib/google dokunulmadı. Build 0 TS hata + `verify-website-codegen` tüm bölümler yeşil + tr/en JSON geçerli.
- **Dosyalar:** `components/website/builder/{BuilderWorkspace,BuilderTopbar,CreditBalanceIndicator,RightInspectorPanel,PreviewCanvas}.tsx`, yeni `components/website/builder/{PageTabsStrip,GenerationTimelineOverlay}.tsx`, `app/globals.css` (wsy-timeline-pop + wsy-tabs-scroll), `locales/tr.json`, `locales/en.json`

## 2026-06-19 — Web Site Yöneticisi: AI sohbet paneli + kredi zaman çizelgesi + sürüm geçmişi + Yayınla popup (#builder-8c)
- **Sorun:** Builder Workspace'te sol panel "Yakında" placeholder'ıydı; doğal-dil sohbetle düzenleme, adım-adım kredi telemetrisi, topbar kredi bakiyesi, onaylı sürüm-geri-al ve markalı Yayınla popup'ı yoktu. Plan Bölüm 8c: mevcut backend'leri (patch/generate/credit-events/versions/publish/preview-url + useCredits) workspace'e bağla.
- **Çözüm:** Beş bileşen eklendi/bağlandı. (1) **AiChatPanel** (sol): sohbet thread'i + giriş — tuvalde blok SEÇİLİYSE komut hedefli `/patch` (op `ai_rewrite`, targetId = seçili blok), seçim YOKSA sayfa-düzey `/generate` revize (revisionMode `edit`, targetSlug/targetLocale). "Baştan üret" mod toggle'ı → `/generate` revisionMode `reject` (reddet affordance korunur). Konuşma (kullanıcı mesajı + kısa "uygulanıyor/uygulandı" asistan satırı), işlem sırasında kilitli giriş, 402 → AccessRequiredModal (kredi), başarı → tuval tazelenir (reloadKey). Seçili blok etiketi sade TR/EN (ham `blockKey` ASLA). (2) **CreditUsageTimeline**: `GET /credit-events` yoklar (üretim sürerken 2sn, aksi halde tek sefer + reloadKey), tek charge'ın faz kırılımını sade TR etiketlerle gösterir (designSystem→"Tasarım sistemi", blueprint→"İçerik planı", render→"Sayfalar", images→"Görseller", translate→"Çeviri"); ham enum yok, amber yok. (3) **CreditBalanceIndicator** (topbar): `useCredits` ile anlık bakiye; owner için ∞. (4) **VersionHistoryPanel**: `GET /versions` listesi + inline ONAYLI "Geri Al" (rollback route, kredi düşmez) → tuval tazelenir; ManageDrawer'a bağlandı (eski inline liste çıkarıldı). (5) **PublishPopup**: topbar "Yayınla" → popup; markalı önizleme/canlı URL (`/preview-url` + yayınlanınca `/s/<subdomain>`, **`.vercel.app` ASLA**), kopyala/yeni-sekme, Yayınla onayı (POST publish), yayınlıysa Canlı Görüntüle + Yayından Kaldır; kendi alan adı bağlama = etiketli "Yakında" (Faz 2) stub — yarım domain mantığı kurulmadı. Yeni backend yok; tek charge yolu (`/patch` + `/generate`) korundu, çift düşüm yok. tr+en simetrik (`builder.chat/credit/versionHistory/publishPopup/managePublish`); amber/sarı yok; yeni bağımlılık yok; lib/meta + lib/google dokunulmadı. Build 0 TS hata + `verify-website-codegen` tüm bölümler yeşil + tr/en JSON geçerli.
- **Dosyalar:** `components/website/builder/{AiChatPanel,CreditUsageTimeline,CreditBalanceIndicator,VersionHistoryPanel,PublishPopup}.tsx`, `components/website/builder/{BuilderWorkspace,BuilderTopbar,ManageDrawer}.tsx`, `app/globals.css` (wsy-pop-in), `locales/tr.json`, `locales/en.json`

## 2026-06-19 — Web Site Yöneticisi: görsel düzenleme — tıkla-seç + sağ inspector + hedefli PATCH (#builder-8b)
- **Sorun:** Builder tuvalinde bir bölüme tıklayıp içeriğini düzenleme yoktu; sağ müfettiş paneli placeholder'dı. Plan Bölüm 12: tuvalde tıkla-seç → sağ inspector'da düzenle → SADECE seçili bloğun cerrahi PATCH'i (tam üretim YASAK), tuval iframe'i sandbox'lı kalmalı (allow-same-origin YOK; postMessage köprüsü).
- **Çözüm:** Tuval içi seçim katmanı postMessage ile kuruldu. `public/yoai-builder-runtime.js` (CSP 'self', YALNIZ builder modunda inline) tuval iframe'i içinde her `[data-yoai-id]` bloğa hover-highlight + tık ekler; tıkta `window.parent.postMessage({type:'yoai:select', blockId, blockKey, rect, text, order}, parentOrigin)` yollar, parent komutlarını (`yoai:highlight`/`yoai:clear`) yalnız `e.source===window.parent` ise kabul eder (vanilla, fail-open). `assembleDocument` yeni `mode:'builder'` → site runtime + builder runtime'ı birlikte inline eder; serve/preview/yeni-sekme önizleme builder runtime'ı ASLA içermez (builder-only). `app/website-preview/[id]` `?builder=1` ile builder mode'u seçer; iç iframe HÂLÂ `sandbox="allow-scripts allow-forms"` (same-origin YOK) — seçim sadece postMessage ile. Parent tarafı: `VisualEditLayer` (rect'i cihaz ölçeği + ortalama ile parent koordinatına eşleyip highlight kutusu çizer), `SelectedElementToolbar` (Metni düzenle/AI yeniden yaz/Sil/Yukarı-Aşağı taşı), yeni `RightInspectorPanel` (seçili bloğun `blockKey` → library `contentFields` → text/richtext/href editörü). `POST /api/website/[id]/patch` (yeni): `{op:'edit'|'ai_rewrite'|'delete'|'move', targetId, content?|instruction?|after?}` → edit/ai_rewrite mevcut `applyBlockPatch` (sanitize+gate yeniden geçer), delete/move deterministik `blockMap.mergeBlocks` + gate (AI yok); yalnız hedef sayfa persist (`replacePages` byte-aynı diğerleri) + `createVersion('revision')` + `website_edit_events` log (fail-soft). Ücret = revizyon politikası (ilk 3 ücretsiz, tek charge, çift düşüm yok). Op doğrulama (allowlist + `^b\d+$`) hem route'ta hem postMessage shape doğrulamasında. tr+en `visualEdit.*` simetrik; amber/sarı yok; yeni bağımlılık yok; lib/meta + lib/google dokunulmadı. Build 0 TS hata + `verify-website-codegen` tüm bölümler yeşil (yeni AB1-3 builder-mode + VE1-2 contentFields/op assertion'ları dahil) + `node --check` runtime'lar temiz.
- **Dosyalar:** `public/yoai-builder-runtime.js` (yeni), `lib/website/codegen/assembleDocument.{mjs,ts}`, `app/website-preview/[id]/page.tsx`, `app/api/website/[id]/patch/route.ts` (yeni), `lib/website/editEvents.ts` (yeni), `components/website/builder/{VisualEditLayer,SelectedElementToolbar,RightInspectorPanel,visualEditTypes}.tsx/.ts`, `components/website/builder/{PreviewCanvas,BuilderWorkspace}.tsx`, `scripts/verify-website-codegen.mjs`, `locales/tr.json`, `locales/en.json`

## 2026-06-19 — Web Site Yöneticisi: tam-ekran Builder Workspace iskeleti (#builder-8a)
- **Sorun:** Site düzenleme/önizleme, dashboard içine sıkışmış DAR bir iframe'di (Promake/Lovable seviyesinde geniş bir tuval yoktu); düzenleme, önizleme ve revize ayrı sayfalara (`/[id]` + `/[id]/onizleme`) bölünmüştü. Plan Bölüm 8: dar iframe kaldırılıp tam-ekran editör iskeleti kurulacak.
- **Çözüm:** `components/website/builder/` altında tam-ekran Builder Workspace iskeleti kuruldu: üstte `BuilderTopbar` (cihaz seçici + sayfa yolu + Yenile + Tam ekran + Önizleme[yeni sekme] + Canlı + Yönet + Yayınla; Geri/İleri Al pasif placeholder — 8c), solda sayfa gezgini + AI sohbet placeholder (`AiChatPanel` — 8c), ortada BÜYÜK `PreviewCanvas` (cihaz-boyutlu, merkez alanı dolduracak ölçekli iframe; ResizeObserver fit), sağda müfettiş placeholder (`RightInspectorPanel` — 8b). `app/tasarim` split-pane istisnası geçerli (max-w-7xl YOK, full-width). İzolasyon dikişi: dış iframe same-origin owner-preview sayfasını yükler (sandbox'sız, cookie gerekir), gerçek izolasyon o sayfanın İÇİNDEKİ sandbox'lı iframe'de; 8b görsel-seçim için `data-builder-canvas`/`data-builder-frame` işaretleri bırakıldı. Mevcut revize/yayınla/onayla/geri-al/logo/build/create-flow akışları AYNEN korundu (eski detay+onizleme sayfalarından `BuilderWorkspace`'e taşındı; API/üretim değişmedi). `ManageDrawer` Tasarım/Logo/Alan adı/Sürüm geçmişini tek noktada erişilebilir tutar. Eski `/onizleme` route workspace'e redirect. tr+en `builder.*` (27 anahtar simetrik); amber/sarı yok; yeni bağımlılık yok; lib/meta + lib/google dokunulmadı. Build 0 TS hata + tr/en JSON geçerli.
- **Dosyalar:** `components/website/builder/{BuilderWorkspace,BuilderTopbar,DeviceSwitcher,PreviewCanvas,PageNavigator,AiChatPanel,RightInspectorPanel,ManageDrawer,RevisePanel}.tsx` (yeni), `app/web-site-yoneticisi/[id]/page.tsx`, `app/web-site-yoneticisi/[id]/onizleme/page.tsx`, `app/globals.css`, `locales/tr.json`, `locales/en.json`

## 2026-06-19 — Web Site Yöneticisi: markalı yeni-sekme önizleme (#builder-7)
- **Sorun:** "Önizleme" yalnız dashboard içindeki sıkışık bir iframe'di; sahibi siteyi gerçek bir ziyaretçi gibi tam-ekran test edemiyordu. Markalı bir önizleme URL'i yoktu ve önizleme adresinin tahmin edilebilir olması güvenlik riskiydi. Ayrıca .vercel.app adresinin hiçbir yerde görünmemesi gerekiyordu.
- **Çözüm:** Çıplak (bare) tam-sayfa TASLAK önizlemesi iki erişim moduyla eklendi. (a) Owner-gated path (`/website-preview/<id>/live/[[...slug]]`) — DEV'de + her zaman; getCurrentUser + getWebsite kapısı, taslak sayfaları `assembleDocument(mode:'serve')` ile birleştirip `text/html` + SITE_CSP + noindex/no-store döner; çok-sayfalı nav opsiyonel catch-all ile aynı route'a path-tabanlı bağlanır. (b) Token-gated markalı route (`(sites)/site-preview/<token>/[[...slug]]`) — PROD, `WEBSITE_PREVIEW_DOMAIN==='1'` + `PREVIEW_ROOT_DOMAIN` ile; middleware `<token>.<root>` host'unu (custom-domain desenin aynısı, default-OFF) bu route'a rewrite eder, kapı = 160-bit tahmin-edilemez `previewToken` (theme jsonb'de, migration yok, ilk istekte üretilir). Nav host-kök tabanlı (`/`) → URL çubuğunda iç route yolu görünmez, host sabit kalır. `GET /api/website/[id]/preview-url` (owner-gated) markalı `https://<token>.<root>` (flag+root) veya owner-gated path döner — **.vercel.app ASLA**. "Önizleme" butonu (onizleme sayfası kontrol satırı, i18n `openInNewTab`) bu URL'i alıp `window.open(_blank)` ile yeni sekmede açar. Flag-OFF iken davranış bugünküyle birebir aynı (dev güvenli); /s/ yayını ve mevcut iframe önizleme bozulmaz; lib/meta + lib/google dokunulmadı; yeni bağımlılık yok. Build 0 TS hata + `verify-website-codegen` tüm bölümler yeşil + tr/en JSON geçerli.
- **Dosyalar:** `app/website-preview/[id]/live/[[...slug]]/route.ts` (yeni), `app/(sites)/site-preview/[token]/[[...slug]]/route.ts` (yeni), `app/api/website/[id]/preview-url/route.ts` (yeni), `lib/website/render/servePreviewDocument.ts` (yeni), `lib/website/store.ts`, `lib/website/types.ts`, `middleware.ts`, `app/web-site-yoneticisi/[id]/onizleme/page.tsx`, `locales/tr.json`, `locales/en.json`

## 2026-06-19 — Web Site Yöneticisi: create-modal alanları library üretimini AKTİF yönlendirir (#builder-6)
- **Sorun:** Create-modal (CreateSiteWizard) alanlarının bir kısmı kaydediliyor ama LIBRARY üretim yolunu (designSystem + blueprint) gerçekten ŞEKİLLENDİRMİYORDU. Ana açık: seçilen TARZ (siteStyle: modern/corporate/playful/luxury/minimal/vibrant) yalnız ham id olarak designSystem prompt'una geçiyor, üretilen palet/gölge/hareket tonunu yönlendirmiyordu; seçilen YAZI AİLESİ (fontPairing) yalnız theme'e yazılıyor, designSystem keyfi font icat ediyordu; referans siteler reference-priority'de ham veriliyor ama soyut tasarım-DNA'sı (renk ailesi/ritim/sektör) çıkarılıp İLHAM olarak verilmiyordu.
- **Çözüm:** Pür `styleProfile.mjs` eklendi: (1) `styleProfileFor(siteStyle)` → her tarz için zengin tasarım direktifi (palet sıcaklığı/gölge derinliği/hareket enerjisi/radius) + TERCİH EDİLEN hero (luxury→`hero.luxury`, corporate→`hero.corporate`, …); (2) `extractDesignDna(summary)` → referans taramasından SOYUT DNA (baskın renk ailesi + layout ritmi + sektör sinyali) — referansın birebir metni/URL'i/görseli ASLA kopyalanmaz (§5.4 anti-clone). `buildCodegenContext` bu sinyalleri ctx'e koyar (`styleDirective`, `preferredHero`, `fontPairing`) ve reference-priority'de DNA'yı ayrı karantinalı `<untrusted_source name="reference_design_dna">` bloğu olarak ekler (veri, talimat değil). `designSystem.ts` prompt'u artık zengin styleDirective'i ONURLANDIRIR ve fontPairing pinliyse o Google-Fonts çiftini (+ headingHref) AYNEN kullanır. `blueprintGeneratorShared.mjs` zengin direktifi + tercih edilen hero'yu prompt'a basar; deterministik fallback + validator HOME hero'yu tarza göre biaslar (havuzda varsa). Kategori gönderilmiyor (modal'da yok) ama instruction + siteType inference'a aktif girer (LG1b). Free-form yolu (`ctx.style` ham id'yi okur), #7 source-priority, #9 revize, #3b mobileMenuAnim, locales multilang KORUNDU. Yeni bağımlılık yok; lib/meta + lib/google dokunulmadı; Opus no-temp/budget_tokens. verify'a `style-profile` bölümü (STY1-STY4) + blueprint BP6 (styleDirective/preferredHero prompt'a ulaşır + fallback HOME hero biası) + LG1b (kategori-null instruction inference) eklendi → tüm bölümler yeşil.
- **Dosyalar:** `lib/website/codegen/styleProfile.mjs` (yeni), `lib/website/codegen/buildCodegenContext.ts`, `lib/website/codegen/types.ts`, `lib/website/codegen/designSystem.ts`, `lib/website/codegen/blueprintGenerator.ts`, `lib/website/codegen/blueprintGeneratorShared.mjs`, `lib/website/codegen/generateHtmlSite.ts`, `scripts/verify-website-codegen.mjs`

## 2026-06-19 — Web Site Yöneticisi: adım-adım kredi telemetrisi (website_credit_events + faz-bazlı kırılım) (#builder-5b)
- **Sorun:** Kredi tüketimi tek bir gerçek charge olarak düşülüyor (atomik defter), ama kullanıcıya işlem sırasında adım-adım (faz-bazlı) gösterilemiyordu. UI'ın (CreditUsageTimeline) tek charge'ı fazlara (tasarım sistemi, blueprint, render, görseller, çeviri) kırarak gösterebilmesi için bir telemetri akışına ihtiyaç vardı — gerçek defteri DEĞİŞTİRMEDEN ve çift düşüm YAPMADAN.
- **Çözüm:** Üç additive+idempotent tablo (`website_credit_events`, `website_publish_events`, `website_edit_events`) için RLS-nested (websites.user_id) migration eklendi (Bölüm 6). Fail-soft store (`lib/website/creditEvents.ts`): `logCreditEvent`/`logGenerationBreakdown`/`logGenerationRefund` + owner-scoped `getCreditEvents` — tablo yoksa (migration uygulanmadan) `console.warn` + return, ASLA throw etmez. Pür `creditBreakdown.mjs`: tek charge'ı çalışan fazlar üzerinde iş-ağırlıklı (render×sayfa, translate×ek dil) orantılar; artık son faza pinlenir → faz-bazlı `credit_delta` toplamı charge'a TAM eşit (çift düşüm yok, salt gösterim). Generate route başarılı üretimden sonra kırılımı, gate/persist hatasında 'refunded' olayını yazar (owner/0-maliyet → 0-delta faz olayları). Stream endpoint `GET /api/website/[id]/credit-events` owner-scoped (getCurrentUser + getWebsite → 404). Atomik defter (`credit_transactions` / `chargeFeature`) DOKUNULMADI; lib/meta + lib/google dokunulmadı; yeni bağımlılık yok. verify'a `credit-breakdown` bölümü eklendi (CB1-CB6: faz toplamı = charge invariantı, owner 0-delta, rounding remainder pinleme) → `credit-breakdown OK`.
- **Dosyalar:** `supabase/migrations/20260619100000_website_event_logs.sql` (yeni), `lib/website/creditEvents.ts` (yeni), `lib/website/creditBreakdown.mjs` (yeni), `app/api/website/[id]/credit-events/route.ts` (yeni), `app/api/website/[id]/generate/route.ts`, `scripts/verify-website-codegen.mjs`

## 2026-06-19 — Web Site Yöneticisi: library/blueprint üretim yolu canlı generate akışına bağlandı (DEFAULT mod) (#builder-5a)
- **Sorun:** builder-1/2/3 ile kütüphane (component library) + blueprint generator + composition engine + render gate hazırdı ama henüz canlı üretim akışına (`generateHtmlSite`) bağlı değildi — gerçek bir site üretmiyordu. Yeni mimariyi varsayılan yol yapmak, mevcut serbest-HTML (free-form) yolunu ve ücret/iade davranışını bozmadan gerekiyordu.
- **Çözüm:** `generateHtmlSite`'a bir üretim MODU eklendi (`website.theme.generationMode`; varsayılan `'library'`, yalnız açık `'freeform'` eski motoru seçer). `'library'` yolu: ctx → designSystem → sektör template seçimi (`inferIndustryTemplateKey` — kategori/siteType/talimat → 11 template'ten biri, eşleşme yoksa `kurumsal`) → blueprint (Opus, builder-3; hata/geçersiz → deterministik fallback validator içinde) → seed (`website.id` + sürüm sayısı; siteler farklı, tekrar-üretim varyasyon) → her sayfa için `composeBlueprint` + `renderComponent` (navbar→`<main>`→footer kova) → `resolveImagePlaceholders` → `gateSiteHtml`. Gate başarısız → TEK self-repair: aynı siteyi deterministik FALLBACK blueprint'ten (hepsi SABİT kütüphane bileşeni → garantili gate-geçer) yeniden render + re-gate; yine başarısız → `ok:false` (route iade eder). Multilang/multipage/revize/ücret KORUNDU: free-form yolu byte-byte değişmedi (yalnızca artık bir mod), `buildExtraLocalePages` aynen kullanıldı, ücret/iade route'ta kaldı (`generateHtmlSite` yine soft-fail). Pür `librarySiteShared.mjs` (template inference + seed + compose→render→concat) hem TS orchestrator hem verify script tarafından import edilir. verify'a `library-generate` bölümü eklendi (enjekte deterministik blueprint, canlı API yok): çok-sayfalı sitenin HER sayfası `gateSiteHtml`'ten geçer, blok id'leri var, multilang çağrılabilir, fallback self-repair gate-geçer → `library-generate OK`. Yeni bağımlılık yok; lib/meta + lib/google dokunulmadı; Opus çağrıları no-temp/budget_tokens; builder-1/2/3 modülleri yeniden yazılmadı.
- **Dosyalar:** `lib/website/codegen/generateHtmlSite.ts`, `lib/website/codegen/librarySiteShared.mjs` (yeni), `lib/website/types.ts`, `scripts/verify-website-codegen.mjs`

## 2026-06-19 — Web Site Yöneticisi: blueprint fallback gerçek seed-variety (11/11 anti-clone) + CO4 tüm template + relabel footer-koruması (#builder-3 CRITICAL)
- **Sorun:** Blueprint fallback yolu (`buildFallbackBlueprint`) 11 template'in 6'sı için (`restoran`, `feribot-bilet`, `klinik`, `e-ticaret`, `kurumsal`, `gayrimenkul`) 50 seed boyunca TEK bir `blueprintSignature` üretiyordu → aynı sektörden iki site fallback yolunda byte-aynı layout/bileşen DNA'sı alıyordu (anti-clone ihlali). İki kök neden: (1) hero hiç seed-varied değildi — `firstHeroFrom` her zaman havuz-sırasındaki ilk hero'yu döndürüyordu; (2) `wantBody = min(bodyCandidates.length, 4)` — bir sayfanın ≤4 body adayı olduğunda HEPSİNİ istiyordu → `pickVariedSubset` havuz sırasına geri sıralıyor → seed'den bağımsız aynı. Minor: `breakConsecutiveArchetypes` kırılamayan ardışık arketip için swap hedefi olarak FOOTER'ı (veya nav) seçebiliyordu → footer sayfa ortasına düşebiliyordu.
- **Çözüm:** Fallback gerçek seed-varied yapıldı: (a) hero artık rol havuzundaki TÜM hero anahtarlarından deterministik seed ile seçilir (`seededHeroFrom`/`pickSeededOne` — aynı seed→aynı hero, farklı seed→olası farklı hero); (b) navbar da havuzun navbar seçeneklerinden seed ile seçilir (`seededNavbarFrom`); (c) body için yer bırakılır — ≥2 aday varsa `wantBody = min(len-1, 4)` ile en az biri düşürülür (düşen seed'e göre değişir) VE `pickVariedOrdered` ile body SIRASI seed-güdümlüdür (havuz sırasına geri sıralanmaz). Determinizm korundu (yalnız seed; Date/Math.random yok); geçerlilik korundu (home-first / contact / gerçek anahtarlar / aşağı akış no-consecutive-archetype). Sonuç: 11/11 template seed span 1..16'da 16/16 farklı imza. CO4 assert'i tek `ajans` yerine 11 template'in HEPSİNİ döngüyle gezip her birinin >1 farklı imza ürettiğini iddia edecek şekilde genişletildi (bir template tek imzaya çökerse FAIL). Minor: `breakConsecutiveArchetypes`'a `isPinned` koruması eklendi — nav/footer arketipleri (ve navbar/footer bileşenleri) swap adayı olarak hariç tutulur; geçerli non-nav/footer swap yoksa relabel'a düşülür (invariant korunur) → footer son, nav ilk kalır.
- **Dosyalar:** `lib/website/codegen/blueprintGeneratorShared.mjs`, `lib/website/codegen/compositionEngine.mjs`, `scripts/verify-website-codegen.mjs`

## 2026-06-19 — Web Site Yöneticisi: Component Library variant genişletme + contentFields tamamlandı + mobil menü dış-tıkla-kapat (#builder-2)
- **Sorun:** builder-1 temeli 8 bileşenle başladı; anti-clone çeşitlilik için daha fazla variant gerekiyordu. İki taşınan kusur vardı: (M1) bazı renderer'lar `contentFields`'te BİLDİRİLMEYEN content anahtarları okuyordu (navbar `menuLabel`/`closeLabel`/`links[].label/href`; footer `navLabel`; contact-form `nameLabel`/`*Placeholder`/`errorText`…) — visual-edit/chat-patch katmanı `contentFields`'i düzenlenebilir-alan kontratı olarak kullandığı için bu alanlar aşağı akışta düzenlenemiyordu. (M2) `initMobileNav` mobil menüyü iç-link-tıklama + Escape ile kapatıyordu ama plan gereği "başka yere tıklayınca otomatik kapanmalı" davranışı (dış-tıkla/backdrop) yoktu.
- **Çözüm:** (M1) Her bileşenin `contentFields` dizisi TAMAMLANDI — `deterministicRender`'ın `content`'ten okuduğu her üst-düzey anahtar bildirildi; `list` alanlarına `item[]` alt-şekli (links/items/tiers vb.) eklendi (`index.ts`'e `ListItemField` tipi + `editable?:false` bayrağı). verify'a LIB11 eklendi: her bileşen, erişilen üst-düzey anahtarları kaydeden bir Proxy ile render edilir; erişilen ⊆ bildirilen iddiası. (M2) `initMobileNav`'a permanent + delegated `pointerdown` dinleyici eklendi: bir panel AÇIKKEN, panelin VE onu kontrol eden herhangi bir toggle'ın (hamburger/kapatma-X) DIŞINDA bir pointerdown paneli kapatır; toggle ÜZERİNDEKİ pointerdown atlanır (toggle'ın kendi click'i kapatır → close→open çift-tetik yok). fail-open, sızan global yok, reduced-motion güvenli; mevcut toggle/iç-link/Escape korundu; runtime davranış-kontrat yorumu güncellendi. (Variant) +4 hero (`hero.full-background` tam-ekran görsel+overlay, `hero.service-business` güven-odaklı, `hero.corporate` metrik istatistikli B2B, `hero.luxury` gradyan üstü premium), +2 navbar (`navbar.centered-logo`, `navbar.left-logo-right-cta` — navbar.standard'la AYNI sert kurallar: OPAK, masaüstü nowrap, runtime mobil kontratı), +3 content (`testimonials.cards`, `gallery.grid`, `pricing-table.tiers`). Hepsi var-token-only, sanitize/gate-temiz, `data-yoai-block`/`-id`, görseller `{{IMG:}}`. verify'a LIB12 (navbar variant opak/nowrap/mobil-kontrat), LIB13 (yeni hero'lar tek-`<h1>` + `{{IMG:}}` + gate), LIB14 (yeni content variant sanitize/gate-temiz, `<h1>` yok, var-token). Sanitizer genişletilmedi, yeni bağımlılık yok, serbest-HTML motoru/pipeline ve lib/meta+lib/google dokunulmadı.
- **Dosyalar:** `lib/website/codegen/library/components.mjs`, `lib/website/codegen/library/index.ts`, `public/yoai-site-runtime.js`, `scripts/verify-website-codegen.mjs`

## 2026-06-18 — Web Site Yöneticisi: Component Library temeli (ComponentDef kayıt defteri + SABİT navbar/footer + ilk variant seti) (#builder-1)
- **Sorun:** Onaylanan Promake-benzeri builder'ın "design vocabulary" katmanı (Bölüm 4) yoktu — AI her şeyi sıfırdan serbest HTML yazıyordu, kalite-garantili (SABİT) navbar/footer/iletişim formu yoktu. HİBRİT kararı: kütüphane ana iskeleti/sözlüğü versin, serbest-HTML motoru omurgada kalsın; kütüphane HTML STRING üretip MEVCUT pipeline'dan (sanitize → renderGate → assembleDocument) geçsin (React bileşeni DEĞİL).
- **Çözüm:** `lib/website/codegen/library/` eklendi. (1) `components.mjs` — saf ESM `ComponentDef` kayıt defteri; her bileşen `{ key, category, blockTag, contentFields (Zod yok — bağımlılıksız düz descriptor), promptHint, deterministicRender(content, ds, opts), requiresTier? }`. Her `deterministicRender` SADECE var-token renk sınıfları (`bg-[var(--surface)]`, `text-[var(--ink)]`, `[var(--accent)]` — designVars adları), üst elemanda `data-yoai-block`+`data-yoai-id` (opts.id), görseller `{{IMG:query}}`, `<script>`/inline handler/iframe yok, iletişim formu dışında form yok. (2) `index.ts` — ince TS sarmalayıcı (ComponentDef/ContentField tipleri + getComponent/listComponents/renderComponent; `.mjs` STATİK literal yolla import — Turbopack kuralı, renderGate.ts↔.mjs deseni). (3) Kalite-garantili: `navbar.standard` (OPAK `bg-[var(--surface)]` — asla şeffaf; masaüstü menü tek satır `whitespace-nowrap`+`flex-nowrap`; mobil hamburger MEVCUT runtime kontratı `data-yoai-nav-toggle`/`data-yoai-mobile-nav`/`data-yoai-mobile-anim` + OPAK panel + kapatma X), `footer.standard` (geçerli yıl `new Date().getFullYear()` ile SUNUCU TARAFINDA enjekte — asla sabit yıl; nav `flex-wrap` ile zarif sarmalanır; "YoAi ile üretildi" işareti korunur), `contact-form.standard` (MEVCUT `data-yoai-form` kontratı — name/email/phone/message + honeypot + `data-yoai-form-success`; action YOK, sunucu post-sanitize enjekte eder). (4) Proof seti: `hero.minimal`, `hero.split-image`, `services.grid`, `cta.band`, `faq.accordion` (`data-yoai-toggle` runtime hook). (5) verify scriptine `library` bölümü (LIB1..LIB10): her render sanitize'ı byte-temiz geçer + `data-yoai-block`/`-id` taşır; navbar opak+nowrap+mobil-menü kontratı; footer geçerli yıl; iletişim formu data-yoai-form+honeypot+action yok; birleşik mini-sayfa (navbar+hero+services+cta+contact+footer) `gateSiteHtml`'i tek `<h1>` ile geçer. Sanitizer genişletilmedi, yeni bağımlılık yok, serbest-HTML motoru/pipeline bozulmadı, lib/meta+lib/google dokunulmadı.
- **Dosyalar:** `lib/website/codegen/library/components.mjs` (yeni), `lib/website/codegen/library/index.ts` (yeni), `scripts/verify-website-codegen.mjs`

## 2026-06-18 — Web Site Yöneticisi (CRITICAL #3): iletişim formu action'ı sunucu sahiplenir — veri sızdırma açığı kapatıldı
- **Sorun:** İletişim formunun POST hedefi `data-yoai-form-action`'tan okunuyordu ve bu attribute sanitizer `form` allowlist'indeydi; `rewriteFormAction`'ın temizleme regex'i YALNIZ çift tırnaklı değeri eşliyordu (`/\sdata-yoai-form-action="[^"]*"/gi`). AI `data-yoai-form-action='https://evil'` (tek tırnak) veya `=https://evil` (tırnaksız) yazarsa: temizleme ıskalıyor, sunucu kendi güvenli action'ını İKİNCİ bir aynı-isimli attribute olarak ekliyor, sanitize-html İLKİNİ (AI'ın kötücülünü) tutuyordu → runtime ziyaretçinin ad/e-posta/telefon/mesajını saldırgan URL'sine `fetch`liyordu. Tek savunma CSP `connect-src 'self'` idi (tek nokta arıza). Ek kök neden: attribute'u sadece `form` allowlist'inden çıkarmak YETMİYOR — global `data-*` glob onu her etikette yeniden serbest bırakıyor.
- **Çözüm:** Sunucu action'ı TAM sahiplenir, AI ASLA yazamaz (kuşak+pantolon): (1) `data-yoai-form-action` `form` allowlist'inden çıkarıldı + global `*` `transformTags` handler'ına koşulsuz strip eklendi → her tırnaklama (tek/çift/tırnaksız), her etikette sanitize tarafından silinir (`data-yoai-form` işaretçisi ve diğer `data-yoai-*` hook'ları korunur). (2) `assembleDocument`'te pre-sanitize `rewriteFormAction` kaldırıldı; yerine POST-sanitize `injectFormAction(cleanHtml, actionPath)` geldi — temizlenmiş (güvenilir) gövdede cheerio ile her `[data-yoai-form]`'a sunucunun aynı-köken yolunu (ör. `/s/<sub>/lead`) tam olarak BİR kez ekler (idempotent; sıkı same-origin guard `//evil`/`/\evil`/mutlak URL reddeder); SERVE'de enjekte eder, PREVIEW'de etmez (runtime iyimser başarı, fetch yok); formsuz gövde aynen döner. (3) `renderGate` backstop: ham girişte same-origin olmayan herhangi bir `data-yoai-form-action` değeri (her tırnaklama) → `suspicious_form`; ayrıca post-sanitize'da non-`/` değer hayatta kalırsa (gelecekteki allowlist regresyonu) reddedilir. Verify'a CF-X1..X5 sızdırma iddiaları eklendi (tek tırnak/tırnaksız/duplicate hepsi nötralize; SERVE'de tek same-origin action; PREVIEW'de action yok; gate non-same-origin'i reddeder). Runtime/endpoint/prompt/CSP/Meta/Google dokunulmadı; yeni bağımlılık yok.
...
- **Çözüm:** Deny-by-default sanitizer **minimum + sıkı** genişletildi: yalnız `form` + `input` + `textarea` eklendi; `<form>`'dan `action/method/target/onsubmit/formaction/name` striplenir (yalnız `data-yoai-form` + `data-yoai-form-action` + class/id/style/aria); `<input>` için `transformTags` **type ∈ {text,email,tel}** zorunlu kılar (password/file/hidden/image/submit/checkbox/… → `text`'e coerce); honeypot görünmez `type=text` input'tur. Runtime'a (`yoai-site-runtime.js`) `data-yoai-form` submit handler eklendi: honeypot kontrolü → sessiz bot drop, basit doğrulama (ad+e-posta+mesaj zorunlu, e-posta regex), `data-yoai-form-action`'a JSON POST → 2xx'te formu gizle + `data-yoai-form-success` göster; action yoksa (önizleme) **iyimser başarı** (fetch yok); fail-open, global sızıntı yok. Yeni public lead uç noktası `app/(sites)/s/[subdomain]/lead/route.ts` (force-dynamic, JSON parse + uzunluk sınırları + honeypot sessiz-drop + IP başına 5/dk in-memory hız sınırı + subdomain→user_id→`signups.email` sahip e-postası + `SITE_CSP`; teslim durumu public'e sızmaz). `assembleDocument` serve modunda her `[data-yoai-form]`'a `data-yoai-form-action="/s/<sub>/lead"` enjekte eder (önizlemede etmez). Prompt "form yok" direktifi **çalışan güvenli form** ile değiştirildi; gate (`renderGate`) hassas input (password/file/hidden/image) veya harici `action`/`formaction` taşıyan formu **`suspicious_form`** ile reddeder (güvenli iletişim formu geçer). Verify scripti `contact-form OK` ile yeni güvenlik iddialarını doğrular. CSP değişmedi (`connect-src 'self'` + `form-action 'self'`), AI `<script>` yok, yeni bağımlılık yok. NOT: e-posta yalnız `RESEND_API_KEY` set + sahibin `signups.email`'i varsa gider; yoksa form yine başarı gösterir (graceful), e-posta no-op.
- **Dosyalar:** `lib/website/codegen/sanitizeAllowlist.mjs`, `public/yoai-site-runtime.js`, `app/(sites)/s/[subdomain]/lead/route.ts`, `lib/website/store.ts`, `lib/website/codegen/assembleDocument.mjs`, `lib/website/codegen/assembleDocument.ts`, `app/(sites)/s/[subdomain]/route.ts`, `app/(sites)/s/[subdomain]/[slug]/route.ts`, `lib/website/codegen/htmlGenerateShared.mjs`, `lib/website/codegen/renderGate.mjs`, `scripts/verify-website-codegen.mjs`
...
- **Sorun:** Tam-sayfa üreteci içerik bölümlerini tek bir `<main>` içine sarıyor (`<header data-yoai-id="b1">…</header><main><section data-yoai-id="b2">…</section>…</main><footer data-yoai-id="bN">…</footer>` — kendi prompt'unun zorunlu kıldığı yapı). Ama `extractBlocks` yalnız fragment kökünün DOĞRUDAN çocuklarını topluyordu → sadece `<header>` (b1) ve `<footer>` (bN) bulunuyordu; `<main>` içindeki TÜM `<section>`'lar GÖRÜNMEZdi. `mergeBlocks` gövdeyi bu eksik listeden YENİDEN birleştirdiğinden, geçerli bir hero/footer düzenlemesi gövdeyi yalnız `<header>…</header><footer>…</footer>`'a indiriyor → `<main>` ve TÜM içerik bölümleri sessizce siliniyordu. `renderGate` (bir h1, landmark, >40 bayt) enkazı GEÇİRİYOR, içi boşaltılmış sayfa "başarılı cerrahi düzenleme" olarak kalıcılaşıyordu. Doğrulama fixture'ı DÜZ gövde (no `<main>`) kullandığından `block-patch OK` hiçbir şey kanıtlamıyordu.
- **Çözüm:** (1) `extractBlocks` artık TÜM ağacı dolaşıp (`<main>` ve her sarmalayıcının içine inerek) `data-yoai-id` taşıyan HER öğeyi belge sırasıyla, tam `bodyHtml`'e göreli bayt aralığıyla (`start`/`end`, htmlparser2 indeksleri) toplar; her blok bayt-tam dilim (`html`). İç-içe id → yalnız EN ÜSTTEKİ blok sayılır (çift-sayma yok). (2) `mergeBlocks(originalBody, blocks, ops, newHtmlById)` ops'u liste yeniden-birleştirme yerine ORİJİNAL gövde string'i üzerinde BAYT-ARALIĞI SPLICE olarak uygular (azalan `at` sırasıyla) — `edit`/`delete`/`insert`/`move`. Op aralığı dışındaki HER ŞEY (`<main>` aç/kapa, boşluk, dokunulmamış bloklar) BAYT-İDENTİK kalır çünkü gövde hiç yeniden kurulmaz, yalnız splice edilir. (3) `applyBlockPatch`'e yapısal invaryant (savunma): birleşmeden sonra kalıcılaştırmadan önce — kaynaktaki her `data-yoai-id` (açık `delete` hedeflemedikçe) birleşmede mevcut; kaynaktaki üst-düzey sarmalayıcılar (`<main`/`<header`/`<footer`) korunmuş; `delete` yokken gövde kaynağın %50'sinden az değil. İhlal → `ok:false` → route tam-üretime düşer (zaten bağlı). (4) Doğrulama fixture'ı üretecin GERÇEK şekliyle (`<header b1><main><section b2><section b3></main><footer b4>`) yeniden yazıldı: extractBlocks 4 bloğu da (b2/b3 `<main>` içinde) bulur; hero (b1) düzenlemesi yalnız b1'i değiştirip `<main>`+b2+b3+b4'ü BAYT-İDENTİK korur; `<main>` içi b2 düzenlemesi yalnız b2'yi değiştirir; b3 silme yalnız b3'ü kaldırır; invaryant `<main>`/blok düşüren varsayımsal birleşmeyi yakalar.
...
## 2026-06-17 — Web Site Yöneticisi (Kod-Üretim): GERÇEK multipage — ayrı sayfalar + sayfa-özel içerik + nav routing (data-yoai-href)
...
- **Çözüm:** `generateHtmlSite` `siteType`'a göre dallanıyor. **landing** → mevcut tek-sayfa akışı AYNEN (regresyon yok). **multipage** → (1) **Sayfa planlama** (tek Opus çağrısı, `multipagePlan.ts` + saf `multipagePlanShared.mjs`): markaya özel 3–6 sayfalık site haritası (`{slug,title,navLabel,role,purpose}`); `validatePagePlan` ile url-safe/unique slug, home ilk + İletişim zorunlu, 6 cap, geçersiz slug coerce/dedupe, garbage/null → deterministik varsayılan plan. (2) **DesignSystem bir kez** üretilip tüm sayfalarda paylaşılıyor. (3) **Sayfa başına üretim** (`generatePageHtml`): her sayfa KENDİ amacına özel içerik + tüm sayfaları gösteren PAYLAŞILAN header/footer nav (`<a data-yoai-href="<slug>">`); tek `<h1>` = sayfa başlığı; aktif sayfa `aria-current="page"`; sayfa-içi `#anchor` yalnız o sayfanın bölümleri için. Var kontratı/spacing/mobil menü/{{IMG:}}/script-form yasağı/renk özgürlüğü AYNEN. (4) **Sayfa başına gate** + 1 self-repair; **all-or-nothing**: bir sayfa bile geçemezse tüm üretim fail (route iade) → eksik sayfaya nav linki ASLA çıkmaz; hangi sayfanın düştüğü loglanır. (5) `WebsitePageInput[]` döner; route `replacePages(user.id, id, result.pages)` ile hepsini yazar. (6) **Nav link rewrite** (`assembleDocument`): `data-yoai-href="<slug>"` → gerçek `href`; serve `linkBase=/s/<sub>` (path: home→base, slug→base/slug), preview `linkBase=/website-preview/<id>` (query: `?slug=...&locale=...` → iframe yeniden render, dashboard 404 olmaz). Yalnız doğrulanmış url-safe slug href'e girer (enjeksiyon yok); sanitize-öncesi rewrite → iç href sanitizer'dan geçer. **Custom domain:** kök base (`linkBase=''`) `resolveNavHref`'te zaten destekli (home→`/`, slug→`/x`); custom-domain servis route'u bağlanınca yalnız doğru `linkBase` geçmesi yeter — modülde TODO. verify'a multipage bölümü eklendi (`multipage OK`).
...
- **Çözüm:** Runtime'ın mobil-nav bölümü baştan yazıldı. `[data-yoai-mobile-nav]` panelleri yalnızca runtime'ın sahip olduğu **INLINE stiller** (en yüksek özgüllük → her display class'ını yener) ile gizlenip gösteriliyor: `transition: transform .32s cubic-bezier(.4,0,.2,1), opacity .32s`, kapalı durum `data-yoai-mobile-anim` yönüne göre off-screen transform (left → translateX(-100%), right → translateX(100%), top → translateY(-100%)) + `opacity:0; pointer-events:none; aria-hidden="true"`. Init'te her panel KAPALI inline duruma zorlanır (yükte güvenilir gizli). Hamburger tıklaması (delegated) toggle eder; açık = translate(0,0) + opacity:1 + pointer-events:auto + aria-hidden=false + aria-expanded=true. İçerideki bir linke tıklayınca + Escape ile kapanır. `prefers-reduced-motion`: slide atlanır, görünürlük anında değişir. `hidden` artık mobil panelde KULLANILMIYOR (eski markup için legacy class/hidden fallback korundu). Reveal / generic toggle / smooth-scroll davranışları aynen korundu; sızan global yok, eksik öğede throw yok (fail-open). Üretim prompt'u (`buildHtmlSystemPrompt`) yeni markup contract'ı emitedecek şekilde güncellendi: hamburger `data-yoai-nav-toggle="mobilenav"` + panel `<nav id="mobilenav" data-yoai-mobile-nav data-yoai-mobile-anim="left">` (default left); panel fixed/absolute overlay olarak AI tarafından stillenir, runtime aç/kapa + slide'ı yönetir; mobil paneli `hidden` ile gizleme yasaklandı. Sanitize: `data-yoai-mobile-nav` + `data-yoai-mobile-anim` (`data-*` glob) ve aria-controls/aria-expanded/aria-label/aria-hidden (aria-* allowlist) hayatta kalıyor — verify'a regresyon testi (r12) eklendi.
- **Dosyalar:** `public/yoai-site-runtime.js`, `lib/website/codegen/htmlGenerateShared.mjs`, `lib/website/codegen/sanitizeAllowlist.mjs` (yorumlar), `scripts/verify-website-codegen.mjs`
...
- **Dosyalar:** lib/yoai/watchdog/{types,rules,metaWatchdog,googleWatchdog,notify,runWatchdog}.ts, lib/yoai/featureFlag.ts, app/api/cron/erken-uyari/route.ts, app/api/yoai/watchdog/route.ts, app/yoalgoritma/page.tsx (Erken Uyarı bölümü, önerilerin altında), components/yoai/ErkenUyariSection.tsx, vercel.json, docs/superpowers/specs/2026-06-15-erken-uyari-gunluk-nobetci.md
...
- **Sorun:** Bazı sayfa URL'leri sidebar başlığıyla uyuşmuyordu: "Dönüşüm Sihirbazı" → `/marketing-kurulumu`, "SEO Plus" → `/seo`, "CRM Sistemi" → `/crm`, "İşletme Profili" → `/yoalgoritma/isletme-profili` (yanlış modül altında). Ayrıca profil menüsündeki "Yardım Merkezi" ölü linkti (`href='#'`, sayfası yoktu).
- **Çözüm:** Route'lar başlıklarla hizalandı, TR/EN slug sistemi tam uygulandı (site canlı kullanıcıya açık olmadığı için 301 yok): `/donusum-sihirbazi` (EN `/en/conversion-wizard`), `/seo-plus` (EN aynı), `/crm-sistemi` (EN `/en/crm-system`), `/isletme-profili` (EN `/en/business-profile`, yoalgoritma altından çıkarıldı + kendi layout'u). Slug haritaları (routes.ts + middleware), tabRoutes (`seo`→`seo-plus`), nav, OAuth dönüş yolları, robots.ts disallow listesi ve tüm iç linkler güncellendi. Yardım Merkezi gerçek içerikle açıldı: `/yardim-merkezi` (EN `/en/help-center`) — 7 maddelik SSS (accordion) + info@yodijital.com destek kartı, tam i18n; dropdown'daki hardcoded "İşletme Profili" etiketi de `sidebar.isletmeProfili` çevirisine taşındı. Meta/Google reklam route'larına dokunulmadı (entegrasyon koruması). Doğrulama: tsc temiz + eski slug kalıntı taraması sıfır.
