...
## 2026-06-20 — Anasayfa: imleci takip eden aurora + akan başlık gradyanı
- **Sorun:** Sahibi withgrower.com/tr anasayfasındaki "mouse hareket ettikçe renkler hareket ediyor" efektini bizim anasayfaya da istedi; ayrıca hero başlığındaki ("Yapay Zeka Destekli Hepsi Bir Arada Pazarlama Yönetimi") renklerin SABİT durmayıp kendi benzer tonlarında (emerald/teal/cyan) değişen bir animasyona kavuşmasını istedi.
- **Çözüm:** (1) Yeni client component `HeroAuroraBackground` — `pointermove` ile imleci yumuşak gecikmeyle (lerp 0.07) takip eden, marka tonunda (emerald/teal/cyan) üç radial ışık + her zaman hafifçe süzülen iki ortam blobu. Renkler mouse ile gerçekten hareket eder; `pointer-events-none` + `aria-hidden` (tıklamayı/erişilebilirliği etkilemez); `prefers-reduced-motion` açıkken hareket durur (statik temel parıltı fallback kalır, server-render'da da görünür). (2) Hero başlığının gradyanı statikten `@keyframes hero-gradient-flow` (`background-size:220%`, 7s) akan animasyona çevrildi — harfler emerald→teal→cyan→teal→emerald ailesinde sürekli kayar; reduced-motion'da durur. Masaüstü + mobil ekran görüntüsüyle (Playwright, mouse 2 konumda) aurora'nın imleci takibi + başlık gradyanı + mobilde sıkışma olmadığı doğrulandı.
- **Dosyalar:** `components/landing/HeroAuroraBackground.tsx` (yeni), `app/page.tsx`
