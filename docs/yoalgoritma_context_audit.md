# YoAlgoritma Bağlam Audit Raporu

> Tarih: 2026-05-20
> Audit eden: Claude Code (Opus 4.7)
> Kapsam: YoAlgoritma haftalık AI tarama motorunun ([lib/yoai/ai/](../lib/yoai/ai/)) [yoalgoritma_proje_amaci.md](yoalgoritma_proje_amaci.md)'ndaki iş kurallarıyla uyumu.
...
|------|--------------|----------|------|---------|
| Aktif filtre (Meta) | `effective_status:'["ACTIVE"]'` Graph API fetch'inde uygulanıyor ([metaDeepFetcher.ts:127](../lib/yoai/metaDeepFetcher.ts#L127)) | effective_status=ACTIVE | ✅ | Yok |
| Aktif filtre (Google) | GAQL `status='ENABLED'` campaign + ad_group + ad_group_ad üç düzeyde ([googleDeepFetcher.ts:161,226,274](../lib/yoai/googleDeepFetcher.ts#L161)) | status=ENABLED | ✅ | Yok |
| Platform docs entegrasyonu | System prompt'ta **yok**; RAG/vector store yok ([systemPrompt.ts](../lib/yoai/ai/systemPrompt.ts)) | Sistemde + AI okuyor | ❌ | A1 |
...
| Apify → Claude akışı | **Yok** — `lib/yoai/ai/*` içinde hiçbir competitor/apify referansı yok | Context olarak gidiyor | ❌ | A2 |
| Business profile context | Kısmi — 6 alan gidiyor; rakipler, intelligence, marka adı, website, ürünler **gitmiyor** ([scanUser.ts:152-177](../lib/yoai/ai/scanUser.ts#L152)) | Her scan'de Claude'a (tam) | ⚠️ | A3 |
| Tam ad spec çıktısı | **Üretilmiyor** — yalnızca alerts/opportunities/recommended_actions; `payload` boş ([types.ts](../lib/yoai/ai/types.ts), [persist.ts:128-147](../lib/yoai/ai/persist.ts#L128)) | Üretiliyor + saklanıyor | ❌ | A5 |
...
**Meta** ([metaDeepFetcher.ts:124-130](../lib/yoai/metaDeepFetcher.ts#L124)):
```
...
**Google** ([googleDeepFetcher.ts](../lib/yoai/googleDeepFetcher.ts)):
...
> **Not:** [accountSerializer.ts:42](../lib/yoai/ai/accountSerializer.ts#L42) ayrıca `status==='ACTIVE'||'ENABLED'` ile *sayım* yapıyor, ama asıl filtre fetch katmanında olduğu için Claude'a giden `campaignsDetail` zaten aktif-only. Kural tam karşılanıyor.

---

### 2. Platform Dokümanlarının AI Engine'de Kullanımı — ❌ EKSİK

- [systemPrompt.ts](../lib/yoai/ai/systemPrompt.ts) içinde Meta/Google resmi reklam kuralları, karakter limitleri, asset spec'leri, politika referansı **yok**.
- Vector store / RAG **yok**. Tüm context tek pass'te `buildUserBrief` ile veriliyor; bu brief yalnızca `account_overview` + `campaignsDetail` + `benchmarks` + (kısmi) `businessContext` içeriyor ([systemPrompt.ts:102-140](../lib/yoai/ai/systemPrompt.ts#L102)).
- Sistem promptu açıkça "Veri zaten elinde — başka bir kaynaktan veri çekme veya varsayım yapma" diyor ([systemPrompt.ts:25](../lib/yoai/ai/systemPrompt.ts#L25)) — yani AI'ın platform kurallarını "bildiği" varsayılıyor, dokümana dayanmıyor.
...
**Bağlama önerisi:** [resmi_dokumanlar_index.md](resmi_dokumanlar_index.md) Bölüm 3, Seçenek A (platforma göre koşullu, cache'li system bloğu). Mevcut `cache_control: ephemeral` zaten var ([agent.ts:65-70](../lib/yoai/ai/agent.ts#L65)) → doküman bloğu cache'lenir, batch maliyeti artmaz.
...
- [lib/yoai/apifyCompetitorProvider.ts](../lib/yoai/apifyCompetitorProvider.ts) — Apify REST API çağrıları (`runApifyActor`, `fetchApifyDatasetItems`), Meta/Google normalize fonksiyonları.
- [lib/yoai/competitorScanner.ts](../lib/yoai/competitorScanner.ts) — unified scan orchestrator (`runCompetitorScanForUser`, Meta+Google).
- [lib/yoai/competitorAdStore.ts](../lib/yoai/competitorAdStore.ts) → `yoai_competitor_ads` tablosu.
- [lib/yoai/competitorInsightStore.ts](../lib/yoai/competitorInsightStore.ts) → `yoai_competitor_insights` tablosu.

**Actor ID encoding doğru:** [apifyCompetitorProvider.ts:199](../lib/yoai/apifyCompetitorProvider.ts#L199) `actorId.replace(/\//g, '~')` kullanıyor — CLAUDE.md kuralına **uygun** (encodeURIComponent kullanılmıyor). `buildMetaActorInput`'taki `encodeURIComponent` yalnızca Facebook arama URL'sinin `q=` değeri için — actor ID için değil, bu doğru.
...
İki kaynak var:
- **On-demand route'lar:** [app/api/yoai/competitors/meta-ad-library/route.ts](../app/api/yoai/competitors/meta-ad-library/route.ts) ve [google-auction/route.ts](../app/api/yoai/competitors/google-auction/route.ts) — UI'dan gelen `query`/`country` ile çağrılıyor.
- **Query türetme:** [competitorScanner.ts:112](../lib/yoai/competitorScanner.ts#L112) `deriveCompetitorQueriesFromCampaigns` — kampanya keyword/domain/isimden sorgu çıkarıyor (LLM'siz).
...
- `yoai_competitor_ads` (normalize edilmiş rakip reklamları, fingerprint dedup ile) — [competitorAdStore.ts](../lib/yoai/competitorAdStore.ts).
- `yoai_competitor_insights` (snapshot/özet) — [competitorInsightStore.ts](../lib/yoai/competitorInsightStore.ts).
...
**Kesin kanıt:** `lib/yoai/ai/*` (AI motorunun tamamı) içinde `competitor`, `apify`, `ad_library`, `transparency`, `rakip` kelimelerinin **hiçbiri geçmiyor** (grep: NONE FOUND).

Akış izi:
```
cron → inngest yoalgoritmaScanUser → gatherUserScanInputs (scanUser.ts)
     → fetchMetaDeep + fetchGoogleDeep + loadBusinessContext
     → buildBatchRequestParams (agent.ts) → buildUserBrief (systemPrompt.ts)
     → Anthropic Batch API
```
`buildUserBrief` payload'ı yalnızca: `account_overview`, `campaignsDetail`, `benchmarks`, kısmi `businessContext`. **Rakip reklam context'i hiçbir adımda eklenmiyor.** `yoai_competitor_ads` / `yoai_competitor_insights` tabloları AI motoru tarafından **hiç okunmuyor**.
...
**Tanı:** Akış **kırık** — Apify çağrılsa ve veri `yoai_competitor_ads`'a yazılsa bile, o veri AI taramasına ulaşmıyor.

---

### 4. Business Profile Entegrasyonu — ⚠️ KISMİ

**Şema zengin** ([businessProfileStore.ts:25-140](../lib/yoai/businessProfileStore.ts#L25)):
...
**Ama AI motoru bunun yalnızca küçük bir kısmını okuyor** ([scanUser.ts:152-177](../lib/yoai/ai/scanUser.ts#L152)):
```
...
Ayrıca `businessContext` `buildUserBrief`'te **1500 karaktere kırpılıyor** ([systemPrompt.ts:120](../lib/yoai/ai/systemPrompt.ts#L120)).
...
**Mevcut çıktı** ([types.ts](../lib/yoai/ai/types.ts), [persist.ts](../lib/yoai/ai/persist.ts)):
...
**Yorum:** Bu motor mevcut reklamları **optimize** ediyor (pause/budget/refresh önerileri), sıfırdan **reklam spec'i üretmiyor**. Tam ad spec üretimi muhtemelen ayrı akışta ([adCreator.ts](../lib/yoai/adCreator.ts), [generate-ad route](../app/api/yoai/generate-ad/route.ts), proposalEngineOrchestrator). Proje amacı YoAlgoritma çıktısı olarak tam ad spec'i listelediğinden, bu ya (a) system prompt + types genişletilerek bu motora eklenmeli, ya da (b) proje amacı dokümanı "ad spec ayrı modülde üretiliyor" diye netleştirilmeli. **Karar Onur'a ait.**
...
| **A2** | Rakip akışını AI motoruna bağla: scan sırasında `runCompetitorScanForUser` (veya hazır `yoai_competitor_ads`/`yoai_competitor_insights`) → `buildUserBrief`'e `competitor_ads` bölümü ekle. | Yüksek — 3. ayağı tamamlar | Orta (akış + payload + system prompt) | A4 (normalize fix) |
