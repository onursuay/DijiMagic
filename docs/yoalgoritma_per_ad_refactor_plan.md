# YoAlgoritma — Per-Ad Improvement Cards Refactor (Faz 1 Audit + Plan)
...
**Kim okuyor?** **Hiç kimse.** `ai_suggestions` yalnızca yazılıyor ([lib/yoai/ai/persist.ts:145](../lib/yoai/ai/persist.ts#L145)), hiçbir API route'u veya component okumuyor (grep ile doğrulandı). `payload.ad_spec` üretiliyor ama UI'a giden yol yok.

**AdPreviewCard hangi kaynaktan besleniyor?**
- Route: `/yoai` ([app/yoai/page.tsx](../app/yoai/page.tsx)). `/yoai/yoalgoritma` alt route'u **yok**.
- Akış: [AiAdSuggestions](../components/yoai/AiAdSuggestions.tsx) → `POST /api/yoai/generate-ad` → `generateFullAutoProposals` ([lib/yoai/adCreator.ts](../lib/yoai/adCreator.ts)) → `FullAdProposal[]` → [AdPreviewCard](../components/yoai/AdPreviewCard.tsx).
- Bu pipeline `ai_suggestions` ile **ilgisiz**. Yani ekrandaki kartlar tarama motorunun çıktısı değil.
- İkinci yol: `buildDeepAnalysisFromAi` ([persist.ts:161](../lib/yoai/ai/persist.ts#L161)) `recommended_actions`'ı `DeepAction`'a çevirirken **`payload`'ı düşürüyor** ([persist.ts:204-217](../lib/yoai/ai/persist.ts#L204-L217)). `DeepAction[]` → `command_center_data` → sadece [YoAlgoritmaHeader](../components/yoai/YoAlgoritmaHeader.tsx) ticker'ı + sayaçlar. Kart yok.
...
Her iki fetcher de **yalnızca aktif/enabled** entity döndürür → "aktif reklam listesi" = fetcher çıktısı. Ad ID'leri stabil. Ad-level tip: `AdInsight` ([analysisTypes.ts:41-56](../lib/yoai/analysisTypes.ts#L41-L56)).
...
**Tetikleyiciler:** Haftalık cron ([/api/cron/yoalgoritma-scan](../app/api/cron/yoalgoritma-scan/route.ts) deseni, `USE_AI_ENGINE` flag) + on-demand. Mevcut durable iskelet [yoalgoritmaScanUser](../inngest/functions/yoalgoritmaScan.ts) (fetch→submit→poll→retrieve→persist) üzerine kurulur.
...
- Mevcut prompt ([systemPrompt.ts](../lib/yoai/ai/systemPrompt.ts)) **hesap geneli** çıktı veriyor: `critical_alerts[] + opportunities[] + recommended_actions[]`. Tek Claude isteği = tüm hesap.
...
- Çıktı validasyonu mevcut [adSpecPayload.ts](../lib/yoai/ai/adSpecPayload.ts) `validateAdSpec` ile uyumlu (yeniden kullanılır).
...
Mevcut pipeline ([app/api/yoai/business-profile/route.ts](../app/api/yoai/business-profile/route.ts) → `runProfileScansAndIntelligence`):
...
- **Apify IG/FB scraper'ları VAR:** [socialSourceScanner.ts](../lib/yoai/socialSourceScanner.ts) + [apifySocialRunner.ts](../lib/yoai/apifySocialRunner.ts) — `encodeActorId` (`/`→`~`), `waitForFinish=50`, IG/FB/LinkedIn/YouTube/TikTok aktörleri env'de tanımlı. Apify yoksa public-metadata fallback. (Yeni aktör satın almaya gerek yok — onaylanan kararla aynı.)
- **Website fetch VAR:** [businessSourceScanner.ts](../lib/yoai/businessSourceScanner.ts) — HTTP + HTML strip (cheerio/readability değil, kendi extractor'ı).
- **Intelligence yazımı VAR:** `buildBusinessIntelligenceRow` ([businessIntelligenceBuilder.ts](../lib/yoai/businessIntelligenceBuilder.ts)) → `user_business_intelligence` (zengin kolonlar: company_summary, keyword_themes[], recommended_*; confidence; missing_data[]). **Ama tamamen deterministik — Claude çağrısı YOK.**
...
**Kart grid'i:** Yeni `AdImprovementCard` (AdPreviewCard stilinde) + yeni grid component, `/yoai`'de `AiAdSuggestions`'ın yanında/yerine yeni bölüm. `GET /api/yoai/ad-improvements` (yeni) → `ai_ad_improvements (status=pending)` döner.

**Onayla / Reddet endpoint'leri (yeni):**
- `POST /api/yoai/ad-improvements/[id]/approve` → `status=approved` → publish denemesi.
- `POST /api/yoai/ad-improvements/[id]/reject` → `status=rejected` + `decision_reason`.

**Publish altyapısı — MEVCUT (Meta tam, Google kısmi):**
- **Meta: ✅ tam 3-katman create var** — [/api/meta/campaigns/create](../app/api/meta/campaigns/create/route.ts) → [/api/meta/adsets/create](../app/api/meta/adsets/create/route.ts) → [/api/meta/ads/create](../app/api/meta/ads/create/route.ts) (adcreative + ad). `AdCreationWizard` → `POST /api/yoai/create-ad` zaten bu zinciri orkestre ediyor. **Yeniden kullanılır.**
- **Google: ⚠️ native create route YOK.** `/api/yoai/create-ad` Google için `fetch('/api/integrations/google-ads/campaigns/create')` çağırıyor — bu **external/custom entegrasyon, bu audit'te doğrulanmadı**. Google approve→publish güvenilirliği belirsiz; Faz 2'de ayrıca doğrulanmalı. (Not: memory — Google/Meta entegrasyon koduna dokunma; yalnızca çağırırız.)
- **Publish güvenliği VAR:** [publishSafety.ts](../lib/yoai/publishSafety.ts) — `YOAI_DIRECT_PUBLISH_ENABLED` (default true), `YOAI_ACTIVE_PUBLISH_ENABLED` (default **false** → reklamlar PAUSED oluşturulur), `YOAI_MAX_DAILY_BUDGET_TRY` (default 1000). Yeni akış bu flag'lere uyar — onaylanan reklam **PAUSED** açılır, kullanıcı Ads Manager'dan aktive eder (mevcut davranış).

**"Brand Bilgilerini Yenile" butonu:** İşletme Profili sayfasına ([app/yoai/isletme-profili/page.tsx](../app/yoai/isletme-profili/page.tsx)) eklenir — **ama F'deki CLAUDE.md çelişkisi çözülmeden eklenmez.**
