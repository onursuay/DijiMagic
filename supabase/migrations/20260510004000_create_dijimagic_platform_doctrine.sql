-- ─────────────────────────────────────────────────────────────
-- DijiAlgoritma — Platform Doctrine (Faz 1)
--
...
-- kalır.
--
-- Migration uygulanmazsa lib/dijimagic/platformDoctrineStore.ts içindeki
-- fallback hardcoded doctrine mirror'ı devreye girer; daily-run /
-- adCreator kırılmaz.
-- ─────────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS public.dijimagic_platform_doctrine (
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_dijimagic_platform_doctrine_type_active
  ON public.dijimagic_platform_doctrine (platform, campaign_type)
  WHERE is_active = true;

CREATE INDEX IF NOT EXISTS idx_dijimagic_platform_doctrine_platform
  ON public.dijimagic_platform_doctrine (platform);

CREATE INDEX IF NOT EXISTS idx_dijimagic_platform_doctrine_campaign_type
  ON public.dijimagic_platform_doctrine (campaign_type);

CREATE INDEX IF NOT EXISTS idx_dijimagic_platform_doctrine_objective
  ON public.dijimagic_platform_doctrine (objective)
  WHERE objective IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_dijimagic_platform_doctrine_channel_type
  ON public.dijimagic_platform_doctrine (channel_type)
  WHERE channel_type IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_dijimagic_platform_doctrine_active
  ON public.dijimagic_platform_doctrine (platform, campaign_type, is_active);

ALTER TABLE public.dijimagic_platform_doctrine ENABLE ROW LEVEL SECURITY;

-- READ: tüm authenticated kullanıcılar (global knowledge base).
DROP POLICY IF EXISTS "dijimagic_platform_doctrine_select_authenticated"
  ON public.dijimagic_platform_doctrine;
CREATE POLICY "dijimagic_platform_doctrine_select_authenticated"
  ON public.dijimagic_platform_doctrine
  FOR SELECT
...
DROP POLICY IF EXISTS "dijimagic_platform_doctrine_write_service_role"
  ON public.dijimagic_platform_doctrine;
CREATE POLICY "dijimagic_platform_doctrine_write_service_role"
  ON public.dijimagic_platform_doctrine
  FOR ALL
  USING (auth.role() = 'service_role')
  WITH CHECK (auth.role() = 'service_role');

COMMENT ON TABLE public.dijimagic_platform_doctrine IS
...
INSERT INTO public.dijimagic_platform_doctrine
