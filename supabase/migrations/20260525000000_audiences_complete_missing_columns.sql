-- ─────────────────────────────────────────────────────────────
-- DijiMagic — audiences tablosu EKSİK KOLON tamamlama
--
...
--   • dijimagic_spec_json    — INSERT'te zorunlu (kitle spec'i) → en kritik
...
ALTER TABLE public.audiences
  ...
  ADD COLUMN IF NOT EXISTS dijimagic_spec_json    JSONB NOT NULL DEFAULT '{}'::jsonb,
...
DO $$
BEGIN
  RAISE NOTICE 'audiences eksik kolonlar tamamlandı: description, dijimagic_spec_json, meta_payload_json, meta_audience_id, error_code, error_message, last_synced_at.';
END $$;
