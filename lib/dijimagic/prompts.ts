...
export function buildGenerationPrompt(
  ...
  const base = `Sen DijiMagic — dijital pazarlama ve içerik üretimi konusunda uzmanlaşmış bir AI asistansın.
...
export function buildStructuredSeoArticlePrompt(input: StructuredSeoInput): string {
  ...
  return `Sen DijiMagic — SEO ve içerik pazarlaması uzmanı bir AI asistansın.
...
export const OFF_TOPIC_MESSAGE =
  'Bu konu DijiMagic\'nin uzmanlık alanı dışında kalıyor. Ben SEO makale, reklam metni, sosyal medya içeriği, e-posta pazarlama, ürün açıklaması, landing page metni ve slogan gibi dijital pazarlama konularında size yardımcı olabilirim. Nasıl bir içerik oluşturmamı istersiniz?'
