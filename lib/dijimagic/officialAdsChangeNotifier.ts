/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Official Ads Change Notifier (Alt-Proje B)
...
export function buildOfficialAdsChangeEmail(result: RefreshResult): { subject: string; html: string } {
  const subject = `[DijiMagic] Resmi reklam dökümanı güncellemesi — ${result.changedSources} değişiklik, ${result.createdDrafts} onay bekliyor`
