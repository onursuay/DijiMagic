/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Approval Store (Faz 0C)
   ...
   Persistence: Supabase tablosu `dijimagic_pending_approvals`.
   Migration:   supabase/migrations/20260510003000_create_dijimagic_pending_approvals.sql
...
import { sanitizeResponseExcerpt } from '@/lib/dijimagic/publishAuditStore'
import type { FullAdProposal } from '@/lib/dijimagic/adCreator'

const TABLE_MIGRATION_HINT =
  'supabase/migrations/20260510003000_create_dijimagic_pending_approvals.sql'
...
function logTableMissing(operation: string): void {
  console.error(
    `[ApprovalStore][AUDIT_LOSS] dijimagic_pending_approvals tablosu yok — ${operation} BAŞARISIZ. ` +
...
export async function upsertPendingApprovalFromProposal(
  ...
  const { data: existing, error: selErr } = await supabase
    .from('dijimagic_pending_approvals')
...
/* ── Approval Versions (Faz 5) ──────────────────────────────────
   dijimagic_approval_versions tablosu: öneri versiyon geçmişi.
   Tablo yoksa soft fail — çağıran flow'u kırmaz.
   ─────────────────────────────────────────────────────────────── */

const VERSIONS_TABLE = 'dijimagic_approval_versions'
const VERSIONS_MIGRATION_HINT =
  'supabase/migrations/20260510007000_create_dijimagic_approval_versions.sql'
