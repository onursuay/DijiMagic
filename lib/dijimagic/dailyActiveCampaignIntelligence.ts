/* ──────────────────────────────────────────────────────────
   ...
   önerileri (dijimagic_pending_approvals) kampanya durumuna göre
   ...
   Stale statüler → dijimagic_pending_approvals.status = 'expired'
...
export async function loadActionableApprovals(
  ...
  if (error) {
    if (error.code === '42P01' || /relation .* does not exist/i.test(error.message || '')) {
      console.warn('[DailyIntelligence] dijimagic_pending_approvals tablosu yok — graceful skip.')
      return []
    }
