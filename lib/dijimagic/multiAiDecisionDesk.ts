/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Multi-AI Decision Desk (Faz 4)
   ...
   DIJIMAGIC_MULTI_AI_ENABLED !== 'true' ise:
...
export function isMultiAiEnabled(): boolean {
  return process.env.DIJIMAGIC_MULTI_AI_ENABLED === 'true'
}
...
function buildStrategistPrompts(pkg: CampaignSynthesisPackage): { system: string; user: string } {
  const system = `Sen DijiMagic Multi-AI Decision Desk'te Strategist rolündesin.
...
function buildCreativePrompts(pkg: CampaignSynthesisPackage): { system: string; user: string } {
  const system = `Sen DijiMagic Multi-AI Decision Desk'te Creative rolündesin.
...
function buildRiskPolicyPrompts(pkg: CampaignSynthesisPackage): { system: string; user: string } {
  const system = `Sen DijiMagic Multi-AI Decision Desk'te Risk Policy rolündesin.
...
function buildTechnicalValidatorPrompts(pkg: CampaignSynthesisPackage): { system: string; user: string } {
  const system = `Sen DijiMagic Multi-AI Decision Desk'te Technical Validator rolündesin.
...
function buildJudgePrompts(
  ...
  const system = `Sen DijiMagic Multi-AI Decision Desk'te Judge/Hakem rolündesin.
...
export async function runMultiAiDecisionDesk(
  ...
  const timeoutMs = options.timeoutMs
    ?? Number(process.env.DIJIMAGIC_MULTI_AI_TIMEOUT_MS || 45_000)
  const maxCostUsd = options.maxCostUsd
    ?? (process.env.DIJIMAGIC_MULTI_AI_MAX_COST_PER_RUN_TRY
      ? Number(process.env.DIJIMAGIC_MULTI_AI_MAX_COST_PER_RUN_TRY)
      : null)
