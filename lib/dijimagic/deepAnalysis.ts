...
import { resolveDijiMagicScope, type DijiMagicScope } from './businessScope'
...
/* ── Main Orchestrator ── */
export async function runDeepAnalysis(userId?: string, explicitScope?: DijiMagicScope): Promise<DeepAnalysisResult> {
  const errors: string[] = []
  const connectedPlatforms: Platform[] = []

  // İşletme scope'u (DijiAlgoritma per-account): seçili işletme varsa yalnız onun
  ...
  // birleşik davranış (zero regresyon). Flag kapalıyken resolveDijiMagicScope hep scoped:false.
  const scope = explicitScope ?? await resolveDijiMagicScope()
