/* ──────────────────────────────────────────────────────────
   DijiMagic — Business Context Store

   Tüm üretim motorları (DijiAlgoritma, Strateji, Hedef Kitle,
...
export function buildBusinessContextPromptBlock(ctx: BusinessContext): string | null {
  ...
  const lines: string[] = []
  lines.push('## İşletme Bağlamı (DijiMagic Business Memory)')
  ...
  if (ctx.intelligenceMemory) {
    ...
    lines.push('')
    lines.push('### DijiMagic Intelligence Memory')
