/* ──────────────────────────────────────────────────────────
   DijiAlgoritma İşletme Scope Çözümleyici (server-only)

   Seçili işletmeyi (dijimagic_business_scope cookie) çözer. Flag-aware:
   DIJIMAGIC_PER_ACCOUNT_SCOPE kapalıysa scope DEVRE DIŞI → mevcut birleşik
...
export interface DijiMagicScope {
...
/**
 * Cookie'lerden aktif DijiAlgoritma scope'unu çözer.
 ...
 */
export async function resolveDijiMagicScope(): Promise<DijiMagicScope> {
  try {
