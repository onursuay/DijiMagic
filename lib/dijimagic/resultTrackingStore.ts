/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Result Tracking Store (Faz 7)
...
const RESULTS_TABLE = 'dijimagic_recommendation_results'
...
export async function recordBeforeSnapshot(
  ...
  try {
    ...
    if (error) {
      if (error.code === '42P01') {
        console.warn('[ResultTrackingStore] dijimagic_recommendation_results tablosu yok — AUDIT_LOSS.')
        return null
      }
