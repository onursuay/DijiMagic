/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Competitor Insight Store (Faz 2)

   dijimagic_competitor_ads üzerinden DETERMINISTIK kural-tabanlı özet
   içgörü üretir ve dijimagic_competitor_insights tablosuna upsert eder.
   Bu fazda LLM çağrısı YOKTUR. Multi-AI Decision Desk sonraki faz.

   Persistence: Supabase tablosu `dijimagic_competitor_insights`.
   Migration:   supabase/migrations/20260510005100_create_dijimagic_competitor_insights.sql
...
const TABLE_MIGRATION_HINT =
  'supabase/migrations/20260510005100_create_dijimagic_competitor_insights.sql'
...
function logTableMissing(op: string, hint?: Record<string, unknown>) {
  console.warn(
    `[CompetitorInsightStore][TABLE_MISSING] dijimagic_competitor_insights tablosu yok — ${op} işlemi KAYBEDİLDİ. Migration uygulayın: ${TABLE_MIGRATION_HINT}`,
    hint || {},
  )
}
...
export async function upsertCompetitorInsight(
  ...
  let selectQ: any = supabase
    .from('dijimagic_competitor_insights')
    .select('id')
    .eq('user_id', userId)
