/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Learning Store (v1: veri biriktirme)

   Amaç: DijiAlgoritma önerilerini ve kullanıcının uyguladığı
   aksiyonları izlemek. v1'de SADECE KAYIT; sonuç analizi
   (outcome evaluation) v2'ye bırakıldı.

   Persistence: Supabase tablosu `dijimagic_action_outcomes`.
   Migration: supabase/migrations/20260510000000_create_dijimagic_action_outcomes.sql
...
const TABLE_MIGRATION_HINT =
  'supabase/migrations/20260510000000_create_dijimagic_action_outcomes.sql'
...
export async function recordActionOutcome(
  ...
  const { data, error } = await supabase
    .from('dijimagic_action_outcomes')
    .insert(payload)
    .select()
    .single()
  ...
  if (error) {
    if (isTableMissingError(error)) {
      ...
      console.error(
        '[LearningStore][AUDIT_LOSS] dijimagic_action_outcomes tablosu yok — kayıt KAYBEDİLDİ. ' +
...
export async function listActionOutcomes(
  ...
  if (error) {
    if (isTableMissingError(error)) {
      console.warn(
        '[LearningStore] dijimagic_action_outcomes tablosu yok — boş liste döndü. ' +
          `Migration uygulayın: ${TABLE_MIGRATION_HINT}`,
      )
      return []
    }
