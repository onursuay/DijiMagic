/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Tek Kampanya Metrik Çekici (ID ile, durum bağımsız)
...
import type { MetricSnapshot } from '@/lib/dijimagic/resultTrackingStore'
...
async function fetchMetaCampaignMetrics(campaignId: string, userId: string): Promise<MetricSnapshot | null> {
  try {
    ...
    const { countMetaConversions } = await import('@/lib/dijimagic/metaConversions')
