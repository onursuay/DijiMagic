/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Per-Campaign Hierarchical Prompt (Faz 3)
...
import { copyQualityBlock, isExpertCopyEnabledForDijiAlgoritma } from './docs/copyQualityGuide'
...
export const PER_CAMPAIGN_SYSTEM_PROMPT = `Sen DijiAlgoritma'nın AI motorusun — bir reklam hesabını HİYERARŞİK analiz eden uzmansın: hesap → kampanya → ad set → reklam.
...
export function buildPerCampaignSystemBlocks(
  ...
  // Default-off → kapalıyken bu blok eklenmez (DijiAlgoritma prompt'u birebir aynı, sıfır regresyon).
  if (isExpertCopyEnabledForDijiAlgoritma()) {
...
export function buildPerCampaignUserBrief(ctx: PerCampaignContext): string {
  ...
  if (ctx.structuralSignals) {
    ...
    lines.push('Aşağıdaki bulgular DijiAlgoritma kural motoru tarafından üretildi (LLM tahmini değil). Kampanya türü uyumu (type_mismatch) ve yapısal önerilerinde bunları BİRİNCİL kanıt kabul et; metriklerle çelişmiyorsa bunlara uy.')
