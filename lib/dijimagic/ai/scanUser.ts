/* ──────────────────────────────────────────────────────────
   DijiAlgoritma AI Engine — Per-User Scan Helpers

   Inngest function tarafından (dijialgoritmaScanUser) kullanılır.
...
import type { DeepCampaignInsight, Platform } from '@/lib/dijimagic/analysisTypes'
 ...
 */
export async function gatherUserScanInputs(userId: string, scope?: DijiMagicScope): Promise<UserScanInputs> {
 ...
 * Migration'sız: ai_engine_runs'a platform='dijialgoritma_hier' satırı (scan.user'ın Meta/Google
...
export async function writeHierRunStatus(input: {
  ...
  if (!supabase) return
  try {
    const { getTurkeyDate } = await import('@/lib/dijimagic/dailyRunStore')
    await supabase.from('ai_engine_runs').upsert(
      {
        user_id: input.userId,
        platform: 'dijialgoritma_hier',
...
async function loadBusinessContext(
  userId: string,
  scope?: DijiMagicScope,
): Promise<{ industry?: string; businessContext?: string }> {
  try {
    const mod = await import('@/lib/dijimagic/businessContextStore')
