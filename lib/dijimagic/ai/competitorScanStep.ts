/* ──────────────────────────────────────────────────────────
   DijiAlgoritma AI Engine — Competitor Scan Step (A4)

   İki sorumluluk:
   1) READ (her zaman açık, ücretsiz): cache'lenmiş rakip reklam
      verisini (dijimagic_competitor_insights + dijimagic_competitor_ads)
   ...
   Write yolu varsayılan ÇALIŞIR; yalnız DIJIALGORITMA_SCRAPE_COMPETITORS=false
...
const SCRAPE_FLAG = 'DIJIALGORITMA_SCRAPE_COMPETITORS'
...
export async function loadCompetitorBrief(
  ...
  try {
    ...
    const { getLatestCompetitorInsight } = await import('@/lib/dijimagic/competitorInsightStore')
    const { getRecentCompetitorAds } = await import('@/lib/dijimagic/competitorAdStore')
...
export async function scrapeDeclaredCompetitors(userId: string): Promise<CompetitorScrapeSummary> {
  ...
  const { isApifyEnabled } = await import('@/lib/dijimagic/apifyCompetitorProvider')
  if (!isApifyEnabled()) {
    return { enabled: true, reason: 'no_apify_token', attempted: 0, scraped: 0, cachedSkipped: 0, errors: 0 }
  }

  const { getBusinessContextForUser } = await import('@/lib/dijimagic/businessContextStore')
  ...
  const region = 'TR'
  const { getLatestCompetitorInsight } = await import('@/lib/dijimagic/competitorInsightStore')
  const { runCompetitorScanForUser } = await import('@/lib/dijimagic/competitorScanner')
