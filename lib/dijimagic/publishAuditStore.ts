/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Publish Audit Store (Faz 0A — hazırlık)
   ...
   Persistence: Supabase tablosu `dijimagic_publish_audit_log`.
   Migration:   supabase/migrations/20260510001000_create_dijimagic_publish_audit_log.sql
...
const TABLE_MIGRATION_HINT =
  'supabase/migrations/20260510001000_create_dijimagic_publish_audit_log.sql'
...
export async function recordPublishAuditAttempt(
  ...
  const { data, error } = await supabase
    .from('dijimagic_publish_audit_log')
    .insert({
      user_id: rec.user_id,
  ...
  if (error) {
    if (isTableMissingError(error)) {
      console.error(
        '[PublishAuditStore][AUDIT_LOSS] dijimagic_publish_audit_log tablosu yok — publish audit KAYBEDİLDİ. ' +
...
export async function updatePublishAuditStatus(
  ...
  if (error) {
    if (isTableMissingError(error)) {
      console.error(
        '[PublishAuditStore][AUDIT_LOSS] dijimagic_publish_audit_log tablosu yok — update KAYBEDİLDİ. ' +
          `Migration uygulayın: ${TABLE_MIGRATION_HINT}`,
        { id, status },
      )
      return false
