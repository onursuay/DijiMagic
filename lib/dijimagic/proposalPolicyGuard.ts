/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Proposal Policy Guard (Faz B)
...
function checkMetaCapability(
  ...
  violations.push({
    ...
    severity: 'high',
    field: 'destinationType',
    message: `"${objective}" + "${destination}" DijiAlgoritma v1'de desteklenmiyor: ${cap.unsupportedReason ?? ''}`,
    userMessage:
