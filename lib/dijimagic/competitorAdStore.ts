/* ──────────────────────────────────────────────────────────
   DijiAlgoritma — Competitor Ad Store (Faz 2)
   ...
   Persistence: Supabase tablosu `dijimagic_competitor_ads`.
   Migration:   supabase/migrations/20260510005000_create_dijimagic_competitor_ads.sql
...
const TABLE_MIGRATION_HINT =
  'supabase/migrations/20260510005000_create_dijimagic_competitor_ads.sql'
...
function logTableMissing(op: string, hint?: Record<string, unknown>) {
  console.warn(
    `[CompetitorAdStore][TABLE_MISSING] dijimagic_competitor_ads tablosu yok — ${op} işlemi KAYBEDİLDİ. Migration uygulayın: ${TABLE_MIGRATION_HINT}`,
    hint || {},
  )
}
