...
import { themeToCssVars, fontHrefFor, areaCssVars, areaFontHref } from './theme'
...
function applyPreviewLinks(sections: SectionBlock[], previewId: string, locale: string | undefined): SectionBlock[] {
  ...
  return sections.map((b) => {
    ...
    if (Array.isArray(c.links)) c.links = (c.links as Record<string, unknown>[]).map((n) => ({ ...n, href: tp(n.href) }))
...
/**
 * Tek bir sayfa modelini global tema + alan bazlı (header/body/footer) override uygulayarak render eder.
 * Bölümler 3 gruba sarılır; her grubun wrapper'ı o alanın CSS değişken override'ını taşır (areaStyles).
 */
export default function SiteRenderer({ page, theme, style, previewId }: SiteRendererProps) {
  ...
  const areaStyles = theme?.areaStyles ?? {}
  const headerSec = sections.filter((b) => b.type === 'header')
  const footerSec = sections.filter((b) => b.type === 'footer')
  const bodySec = sections.filter((b) => b.type !== 'header' && b.type !== 'footer')

  // Google Fonts linkleri: global + her alanın (benzersiz, boş olmayan)
  const fontHrefs = Array.from(
    new Set([fontHrefFor(theme), areaFontHref(areaStyles.header), areaFontHref(areaStyles.body), areaFontHref(areaStyles.footer)].filter(Boolean)),
  ) as string[]
  ...
  return (
    ...
    >
      ...
      {fontHrefs.map((h, k) => (
        <link key={k} rel="stylesheet" href={h} />
      ))}
      {headerSec.length > 0 && (
        <div style={areaCssVars(areaStyles.header)}>{headerSec.map((b, i) => renderSection(b, i))}</div>
      )}
      <div style={areaCssVars(areaStyles.body)}>{bodySec.map((b, i) => renderSection(b, 100 + i))}</div>
      {footerSec.length > 0 && (
        <div style={areaCssVars(areaStyles.footer, true)}>{footerSec.map((b, i) => renderSection(b, 200 + i))}</div>
      )}
    </div>
  )
}
