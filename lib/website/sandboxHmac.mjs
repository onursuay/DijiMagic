...
/**
 * Gelen timestamp'ın pencere içinde olup olmadığını kontrol eder (replay koruması).
 * @param {number} ts          — body'den gelen Unix timestamp (saniye cinsinden)
 * @param {number} nowMs       — şimdiki zaman (ms), varsayılan Date.now()
 * @param {number} windowSec   — izin verilen sınır (saniye), varsayılan 300 (5 dk)
 * @returns {boolean}
 */
export function isTimestampFresh(ts, nowMs = Date.now(), windowSec = 300) {
  return typeof ts === 'number' && isFinite(ts) && Math.abs(nowMs / 1000 - ts) <= windowSec
}
