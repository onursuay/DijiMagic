...
// ---------------------------------------------------------------------------
// CONTACT FORM action wiring — data-yoai-form → data-yoai-form-action="<base>"
//
// The generator emits a bare `<form data-yoai-form>` (no action — the sanitizer
// strips native action/method anyway). The SERVING layer decides where the form
// POSTs by injecting `data-yoai-form-action` here, at assembly time, from a
// validated same-origin base path. The runtime reads that attribute and POSTs
// JSON to it. If NO base is supplied (preview / thumb), no action is injected →
// the runtime falls back to OPTIMISTIC success (reveal + no real send).
//
// SAFETY: the base is a server-controlled string (e.g. `/s/<sub>/lead`), never
// attacker text. It is attribute-escaped before injection. We only touch the
// OPENING <form ...> tag that already carries data-yoai-form, and any pre-existing
// data-yoai-form-action is replaced (idempotent re-rewrite is safe).
// ---------------------------------------------------------------------------

/**
 * Inject `data-yoai-form-action="<actionPath>"` on every `<form data-yoai-form>`.
 ...
 * @returns {string}
 */
export function rewriteFormAction(html, actionPath) {
  if (typeof html !== 'string' || !html) return html || ''
  if (!actionPath || typeof actionPath !== 'string') return html
  const escaped = escapeAttr(actionPath)
  // Match an opening <form ...> tag carrying data-yoai-form (in any attr position).
  return html.replace(/<form\b([^>]*\bdata-yoai-form\b[^>]*)>/gi, (full, attrs) => {
    // Drop any pre-existing data-yoai-form-action to avoid duplicates, then add ours.
    const cleaned = attrs.replace(/\sdata-yoai-form-action="[^"]*"/gi, '')
    return `<form${cleaned} data-yoai-form-action="${escaped}">`
  })
}
...
export async function assembleDocument({
  ...
  // ── 0b. CONTACT FORM action wiring (data-yoai-form → data-yoai-form-action) ──
  // Also runs BEFORE sanitize (defense-in-depth — the injected attr is validated).
  // No-op when formActionBase is absent (preview/thumb → optimistic success).
  const withForm = formActionBase
    ? rewriteFormAction(withNav, formActionBase)
    : withNav

  // ── 1. Sanitize ────────────────────────────────────────────────────────────
  const clean = sanitizeSiteHtml(withForm)
