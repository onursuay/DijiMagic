 ...
 */
export function resolveNavHref(slug, { linkBase, navMode, localeQuery = '' }) {
  ...
  const safe = !isHome && SAFE_SLUG_RE.test(s)
 ...
 *
 * @param {string} html
 * @param {object} opts  see resolveNavHref
 * @returns {string}
 */
...
export async function assembleDocument({
  ...
  // ── 1. Sanitize ────────────────────────────────────────────────────────────
  const clean = sanitizeSiteHtml(withNav)
