 ...
 *   5a. serve:   <script src="/dijimagic-site-runtime.js" defer></script>  (CSP 'self')
 ...
 *   script-src 'self'        → external /dijimagic-site-runtime.js OK, inline script BLOCKED
...
// sanitize-html (CommonJS) — STATİK default import (Next file-tracing'in lambda'ya
// dahil etmesi için; dinamik createRequire tracer'a görünmez → MODULE_NOT_FOUND → 500).
...
// ---------------------------------------------------------------------------
// MULTIPAGE nav link rewriting — data-dijimagic-href="<slug>" → real href
//
// Multipage generation emits shared header/footer nav as
//   <a data-dijimagic-href="<targetSlug>">NavLabel</a>
...
// validated slugs of pages that ACTUALLY exist, INCLUDING home). A data-dijimagic-href
 ...
 *
 * @param {string} slug      the data-dijimagic-href value (a page slug, '' or 'home' = home)
 * @param {object} opts
...
//
// The submit URL is NEVER author-able by the AI. `data-dijimagic-form-action` is OFF
...
// `data-dijimagic-form-action` that can ever be present is the server's.
//
// This replaces the old PRE-sanitize `rewriteFormAction`, whose double-quote-only
// cleanup regex (`/\sdata-dijimagic-form-action="[^"]*"/gi`) missed single-quoted /
...
//   - Precise insertion via cheerio — only `[data-dijimagic-form]` elements get the
...
/**
 * Inject the SERVER-OWNED `data-dijimagic-form-action="<actionPath>"` on every
 * `[data-dijimagic-form]` element of an ALREADY-SANITIZED body. POST-sanitize only.
...
export function injectFormAction(cleanHtml, actionPath) {
  ...
  const forms = cleanHtml.includes('data-dijimagic-form')
  ...
  const targets = $('[data-dijimagic-form]')
  if (targets.length === 0) return cleanHtml
  targets.each((_i, el) => {
    // Belt: drop any stray same-named attr (none can survive sanitize) then set ours.
    $(el).removeAttr('data-dijimagic-form-action')
    $(el).attr('data-dijimagic-form-action', actionPath)
  })
  return $.html()
}

/**
 * Rewrite every `<a data-dijimagic-href="<slug>">` in `html` so it gains a real
 * `href="<resolved>"`. Pure + deterministic. The data-dijimagic-href attribute is
 ...
 * returned unchanged — landing pages never contain data-dijimagic-href.
 *
 * `opts.knownSlugs` (when supplied) makes the rewrite slug-set-aware: a
 * data-dijimagic-href to a page that does not exist resolves to the home base.
 *
 * @param {string} html
...
export function rewriteNavLinks(html, opts) {
  ...
  // Match an <a ...> open tag carrying data-dijimagic-href="..." and inject/replace href.
  // We only touch the opening tag; the slug value is read from the attribute.
  return html.replace(/<a\b([^>]*?)\sdata-dijimagic-href="([^"]*)"([^>]*)>/gi, (full, pre, slug, post) => {
    ...
    return `<a${cleanPre} data-dijimagic-href="${escapeAttr(slug)}" href="${resolved}"${cleanPost}>`
  })
}
 ...
 * @param {string} [args.linkBase]        — MULTIPAGE: base path for data-dijimagic-href nav rewrite.
 ...
 *                                          Makes the nav rewrite slug-set-aware: a data-dijimagic-href to a
 ...
 *                                          data-dijimagic-form-action on every <form data-dijimagic-form>
 *                                          (e.g. '/s/<sub>/lead'). Set ONLY in real serve mode;
 *                                          omitted in preview/thumb → runtime optimistic success.
 * @param {boolean} [args.editMode]        — EDIT OVERLAY: when true (PREVIEW mode ONLY), the tiny
 *                                          click-select overlay (public/dijimagic-select.js) is inlined
 *                                          AFTER the runtime so the owner önizleme page can select
 *                                          a block (postMessage). NEVER injected in serve mode and
 *                                          NEVER when false/absent → the published site + normal
 *                                          preview stay byte-clean of the overlay.
...
export async function assembleDocument({
  ...
  // ── 0. MULTIPAGE nav rewrite (data-dijimagic-href → real href) ──────────────────
  ...
  // NEVER author data-dijimagic-form-action: it is off the allowlist, so sanitize
  ...
  // This guarantees the ONLY data-dijimagic-form-action present is the server's path.
  ...
  const rawTitle = seo?.title || 'DijiMagic Site'
...
${css}
  ...
  if (mode === 'serve') {
    ...
    runtimeHtml = '<script src="/dijimagic-site-runtime.js" defer></script>'
  } else {
    // preview mode: srcdoc cannot fetch external files, so inline the runtime.
    const runtimePath = path.join(process.cwd(), 'public', 'dijimagic-site-runtime.js')
    const runtimeSource = fs.readFileSync(runtimePath, 'utf-8')
    runtimeHtml = `<script>${runtimeSource}</script>`
  }

  // ── 5b. EDIT OVERLAY (click-select) — PREVIEW + editMode ONLY ───────────────
  // The tiny selection overlay is inlined ONLY when the document is rendered in
  // PREVIEW mode AND editMode===true (the owner önizleme page's "Düzenle" toggle,
  // via ?edit=1). It is NEVER added in serve mode and NEVER when editMode is
  // absent/false — so the published /s/ output and the normal (non-edit) preview
  // are byte-clean of it. postMessage-only; the iframe keeps its sandbox (no
  // allow-same-origin). Inline (no external fetch) because srcdoc has no same-origin.
  let editOverlayHtml = ''
  if (mode !== 'serve' && editMode === true) {
    const selectPath = path.join(process.cwd(), 'public', 'dijimagic-select.js')
    const selectSource = fs.readFileSync(selectPath, 'utf-8')
    editOverlayHtml = `\n<script>${selectSource}</script>`
  }
