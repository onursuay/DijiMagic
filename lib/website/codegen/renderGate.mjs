 ...
 *   suspicious_form   — a sensitive input (type password/file/hidden/image) or a
 *                       form with an external action/formaction in the RAW input.
 *                       The safe contact form (text/email/tel + textarea +
 *                       data-yoai-form, no action) PASSES. Defense-in-depth: the
 *                       sanitizer already coerces/strips these, but a page that
 *                       TRIED to collect credentials / smuggle a hidden payload /
 *                       POST cross-origin is rejected outright rather than silently
 *                       cleaned and published.
 */
