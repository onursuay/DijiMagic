...
export const SAFE_TAGS = [
  ...
  // form submitted DECLARATIVELY by the runtime (data-yoai-form), NEVER via a
...
export const SAFE_ATTRS = {
  ...
  '*': [
    ...
    // (incl. the mobile-nav hooks data-yoai-mobile-nav + data-yoai-mobile-anim).
    'data-*',
    // Tabindex
    'tabindex',
    ...
    // generic [data-yoai-toggle] hook (the runtime toggles the 'hidden' attribute on
    // its target). NOTE: the MOBILE nav no longer relies on 'hidden' — the runtime
    // hides/shows [data-yoai-mobile-nav] panels via INLINE styles (transform/opacity)
  ...
  // submits declaratively via data-yoai-form (the safe marker) + the trusted
  // data-yoai-form-action that the SERVER injects AFTER sanitize.
  //
  // SECURITY (CRITICAL #3 — exfiltration defense-in-depth): `data-yoai-form-action`
  ...
  // a same-origin path. Any AI-authored `data-yoai-form-action` — single-quoted,
  // double-quoted OR unquoted — is therefore STRIPPED here by sanitize, so the AI
  // can never point the form at an attacker URL. Only `data-yoai-form` (the inert
  ...
  form: ['class', 'id', 'style', 'data-yoai-form',
...
export function buildSanitizeOptions() {
  return {
    ...
    // 'hidden' is kept here for the generic [data-yoai-toggle] hook (runtime toggles
    ...
    // 'data-yoai-form' is the runtime form hook, also valueless.
    // 'data-yoai-gradient-anim' is the VALUELESS kinetic-gradient hook (the runtime
    // drifts the element's gradient background-position over time). VALUED hooks
    // (data-yoai-text-rotate / data-yoai-count-up / data-yoai-reveal …) already pass
    ...
    allowedEmptyAttributes: ['alt', 'hidden', 'required', 'data-yoai-form', 'data-yoai-gradient-anim'],
    ...
    transformTags: {
      ...
      '*'(tagName, attribs) {
        let attrs = attribs
        // CRITICAL #3 — the AI can NEVER author data-yoai-form-action (any quoting,
        ...
        if ('data-yoai-form-action' in attrs) {
          const { 'data-yoai-form-action': _removedAction, ...rest } = attrs
          attrs = rest
        }
        if (attrs['style']) {
