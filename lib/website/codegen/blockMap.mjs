 ...
 *                                        element carrying data-dijimagic-id, anywhere in
 ...
 *   The generator marks each top-level marketing section with data-dijimagic-block="<role>"
 *   + data-dijimagic-id="b1","b2",… (htmlGenerateShared prompt) and wraps the content
 ...
 *   string. extractBlocks descends the WHOLE tree to find every data-dijimagic-id block
...
// ---------------------------------------------------------------------------
// extractBlocks — EVERY data-dijimagic-id element (whole tree), byte-exact slices.
// ---------------------------------------------------------------------------

/**
 * Parse the body HTML and return every element that carries a data-dijimagic-id,
 ...
 *
 * TOP-MOST only: a data-dijimagic-id element nested inside ANOTHER data-dijimagic-id element
...
export function extractBlocks(bodyHtml) {
  ...
  // data-dijimagic-id we record it as a block and DO NOT descend into it (top-most wins,
  // so an id nested inside a block can never split it). Any element without a
  // data-dijimagic-id — including the <main>/<header>/<footer> wrappers — is descended
  ...
  const walk = (nodes) => {
    ...
    for (const node of nodes) {
      ...
      const rawId = typeof attribs['data-dijimagic-id'] === 'string' ? attribs['data-dijimagic-id'].trim() : ''
      const hasIndices =
        typeof node.startIndex === 'number' && typeof node.endIndex === 'number'

      if (rawId && hasIndices && !seen.has(rawId)) {
        const role =
          typeof attribs['data-dijimagic-block'] === 'string'
            ? attribs['data-dijimagic-block'].trim()
            : ''
...
function swapImgSrc(tag, url) {
  ...
  const safe = escapeAttrValue(url)
 ...
 * Exported so the caller's "did the src survive the gate?" check can compare against
 * the SAME escaped form we actually write into the tag (sanitize-html entity-encodes
 * `&` → `&amp;`, so a query-string url lives as `&amp;` in the gated html, not `&`).
 */
