...
import { pickVariedSubset, pickVariedOrdered, pickSeededOne } from './compositionEngine.mjs'
...
/** ALL hero keys present in a pool (registry-validated, pool order). */
function heroKeysFrom(keys, registry) {
  const out = []
  for (const k of keys || []) {
    if (keyExists(registry, k) && registry[k].category === 'hero') out.push(k)
  }
  return out
}

/** ALL navbar keys present in a pool (registry-validated, pool order). */
function navbarKeysFrom(keys, registry) {
  const out = []
  for (const k of keys || []) {
    if (keyExists(registry, k) && registry[k].category === 'navigation' && registry[k].blockTag === 'header') out.push(k)
  }
  return out
}

/**
 * SEED-PICK a hero for a page from ALL hero keys in its role pool (deterministic
 * by seed; same seed → same hero, different seed → potentially a different hero).
 * Falls back to the registry's first hero so a page is never hero-less.
 *
 * #builder-6 — STYLE BIAS: when `preferredHero` (from the wizard's siteStyle, e.g.
 * 'hero.luxury' for luxury) is a real hero present in this page's pool, it WINS
 * (the chosen tarz deterministically shapes the home hero). When it is not in the
 * pool we fall back to the seeded pick (the style still steers tone/tokens via the
 * design system + the prompt). This keeps anti-clone variety for everything else.
 */
function seededHeroFrom(rolePool, registry, seed, preferredHero) {
  const heroes = heroKeysFrom(rolePool, registry)
  if (
    typeof preferredHero === 'string' &&
    preferredHero &&
    heroes.includes(preferredHero)
  ) {
    return preferredHero
  }
  if (heroes.length > 0) return pickSeededOne(heroes, seed)
  return firstHeroFrom(rolePool, registry)
}

/**
 * SEED-PICK a navbar for a page from the navbar keys in its role pool (so two
 * same-industry sites can differ on the header too). Falls back to the registry's
 * first navbar so a page always has a header.
 */
function seededNavbarFrom(rolePool, registry, fallbackNavbar, seed) {
  const navs = navbarKeysFrom(rolePool, registry)
  if (navs.length > 0) return pickSeededOne(navs, seed)
 ...
 * @param {{ preferredHero?: string }} [opts]  #builder-6 — siteStyle → hero bias
 * @returns {import('./types').SiteBlueprint}
 */
export function buildFallbackBlueprint(ds, template, registry, locale, seed, opts) {
  const reg = registry || {}
  const lc = isStr(locale) ? locale : 'tr'
  const baseSeed = seed == null ? 'fallback' : seed
  // #builder-6 — the wizard's siteStyle maps to a preferred hero; the HOME hero
  // honours it when present in the home pool (other pages keep seeded variety).
  const preferredHero = opts && typeof opts.preferredHero === 'string' ? opts.preferredHero : ''
  ...
  const pages = planned.map((p, pageIdx) => {
    ...
    // SEED-PICK the hero from ALL hero keys in the role pool (not pool-order first)
    // → same seed → same hero, different seed → potentially different hero. The HOME
    // hero additionally honours the siteStyle's preferred hero when it is in the pool.
    const heroKey = seededHeroFrom(
      rolePool,
      reg,
      `${baseSeed}::${p.slug}::hero`,
      p.role === 'home' ? preferredHero : '',
    )
    // SEED-PICK the navbar from the pool's navbar options (header can vary too).
    const navKey = seededNavbarFrom(rolePool, reg, navbar, `${baseSeed}::${p.slug}::nav`)

    // Seed-driven varied body subset AND order. When there's room (≥2 candidates)
    // LEAVE room — drop at least one so the dropped block varies by seed — AND keep
    // a SEEDED ORDER (pickVariedOrdered does NOT re-sort to pool order), so even a
    // page that requests all-but-one of its few candidates still differs by seed.
    const wantBody =
      bodyCandidates.length <= 1
        ? bodyCandidates.length
        : Math.max(MIN_BODY_BLOCKS, Math.min(bodyCandidates.length - 1, 4))
    const chosenBody = pickVariedOrdered(bodyCandidates, wantBody, `${baseSeed}::${p.slug}::body`)

    const blocks = []
    let n = 1
    if (navKey) blocks.push(makeBlock(`b${n++}`, navKey, lc))
    else if (navbar) blocks.push(makeBlock(`b${n++}`, navbar, lc))
 ...
 * @param {{ locale?: string, industryTemplateKey?: string|null, seed?: string|number, preferredHero?: string }} [opts]
 * @returns {import('./types').SiteBlueprint}
 */
export function validateBlueprint(raw, ds, registry, templates, opts) {
  ...
  // #builder-6 — siteStyle → preferred hero (honoured on the HOME page scaffold).
  const preferredHero = isStr(o.preferredHero) ? o.preferredHero : ''
  ...
  pages = pages.map((p, pageIdx) => {
    // #builder-6 — only the HOME page scaffold honours the preferred hero (other
    // pages keep seeded variety). Passed through ensureScaffold's hero fill.
    let blocks = ensureScaffold(
      p, reg, navbar, footer, contactForm, pool, locale, seed,
      p.pageRole === 'home' ? preferredHero : '',
    )
  ...
  if (!isUsableBlueprint(candidate, reg)) {
    return buildFallbackBlueprint(ds, template, reg, locale, seed, { preferredHero })
  }
  return candidate
}
 ...
 */
function ensureScaffold(page, registry, navbar, footer, contactForm, pool, locale, seed, preferredHero) {
  const reg = registry
  ...
  // Ensure a hero. #builder-6 — prefer the siteStyle's hero when it is a real hero in
  // this page's pool (HOME only — the caller passes '' for other pages); else the
  // role pool's first hero, else any registry hero. Keeps every page hero-bearing.
  if (!hero) {
    const rolePool = Array.isArray(pool[page.pageRole]) ? pool[page.pageRole] : []
    const poolHeroes = heroKeysFrom(rolePool, reg)
    const hk =
      (typeof preferredHero === 'string' && preferredHero && poolHeroes.includes(preferredHero))
        ? preferredHero
        : firstHeroFrom(rolePool, reg)
...
export function buildBlueprintUserMessage(ctx, template, ds, availableKeys) {
  ...
  // #builder-6 — STYLE choice: prefer the rich directive (styleProfile) over the raw
  // id so the chosen tarz shapes the section mix + tone; raw id is the fallback.
  ...
  // #builder-6 — preferred hero: the style maps to a hero archetype (luxury →
  // hero.luxury, corporate → hero.corporate, …). Tell the model to PREFER it for the
  // home hero when present in the pool (the deterministic fallback honours it too).
  if (ctx.preferredHero) {
    lines.push(
      `Preferred hero component (use this for the HOME hero when it fits the brand): ${ctx.preferredHero}.`,
    )
  }
  lines.push('')
