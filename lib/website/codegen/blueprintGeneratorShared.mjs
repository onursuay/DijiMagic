 ...
 */
function seededHeroFrom(rolePool, registry, seed) {
...
export function buildFallbackBlueprint(ds, template, registry, locale, seed) {
  ...
  const pages = planned.map((p, pageIdx) => {
    ...
    // → same seed → same hero, different seed → potentially different hero.
    const heroKey = seededHeroFrom(rolePool, reg, `${baseSeed}::${p.slug}::hero`)
