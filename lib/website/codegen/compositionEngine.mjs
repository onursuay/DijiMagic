...
function breakConsecutiveArchetypes(blocks) {
  ...
  // out[i] with the nearest later block whose archetype differs from BOTH
  // out[i-1] and (if present) out[i+1]. Pinning is implicit — nav is at 0 and
  // footer at n-1, and a swap that would move them is naturally rejected because
  // their archetypes ('nav'/'footer') never collide with body archetypes.
  for (let i = 1; i < n; i++) {
