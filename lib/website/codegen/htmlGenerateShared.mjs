...
export function buildHtmlSystemPrompt(ctx) {
  ...
  // by the SERVER-owned runtime (data-yoai-*) and is prefers-reduced-motion-guarded.
...
- SCROLL REVEAL EVERYWHERE: put data-yoai-reveal on the hero, every section, and the cards inside them, and STAGGER siblings with data-yoai-delay (e.g. 0 / 90 / 180 / 270ms) so content cascades in as the visitor scrolls. Optionally tune data-yoai-duration (≈500–700).
- KINETIC HEADLINE: make the hero headline come alive — give a key word/phrase a rotating set via data-yoai-text-rotate="Söz1|Söz2|Söz3" (the runtime fades/slides between them on a loop). Use 2–4 short, on-brand words. This is the "hareket eden yazı" the brand wants.
- ANIMATED GRADIENT BACKGROUND: give the hero (and ideally the CTA band) an animated, slowly-shifting gradient — set a gradient background via style="background-image:var(--gradient-brand)" (or var(--gradient-glow)) AND add the valueless data-yoai-gradient-anim attribute on that SAME element so the runtime drifts the gradient over time ("değişen renkler"). Keep it tasteful and slow.
- COUNT-UP STATS: for any number/metric (stats, proof), put the target on data-yoai-count-up="1240" (optionally data-yoai-count-suffix="+") so it animates 0→target when scrolled into view. Write the final number as the element's text too (fallback).
...
- Use ONLY these data-yoai-* hooks (invent no others — unknown hooks do nothing):
  · data-yoai-reveal  (+ optional data-yoai-delay="120" data-yoai-duration="600" data-yoai-threshold="0.15") — scroll fade/slide-in.
  · data-yoai-text-rotate="Word1|Word2|Word3"  (+ optional data-yoai-rotate-interval="2200" ms) — rotates/animates the words inside the element on a loop (kinetic headline word).
  · data-yoai-gradient-anim  (VALUELESS — just the attribute) — slowly animates the element's gradient background-position over time; the element MUST already have a gradient bg (e.g. style="background-image:var(--gradient-brand)").
  · data-yoai-count-up="1240"  (+ optional data-yoai-count-suffix="+") — counts the number from 0 to the target when scrolled into view (write the final value as text too).
  · data-yoai-smooth (on in-page #anchor links), data-yoai-nav-toggle / data-yoai-mobile-nav / data-yoai-mobile-anim (mobile nav), data-yoai-form + its success/error markers (contact form), data-yoai-block / data-yoai-id (section roles).
...
${navLines}
  Reproduce these links VERBATIM (same data-yoai-href slug values, same labels) in BOTH the header nav and the footer nav. Keep the nav label text EXACTLY as given with full, correct orthography (e.g. Turkish "Hakkımızda", "İletişim" — never the ascii "Hakkimizda"/"Iletisim"; the slug is the only ascii part). The runtime resolves data-yoai-href to the real URL — do NOT add your own href; just emit data-yoai-href="<slug>". Mark the CURRENT page's link with aria-current="page" (already shown above).
