...
- Mobile nav (use EXACTLY this markup contract — the runtime handles open/close + the slide animation):
  ...
  · Menu panel: <nav id="mobilenav" data-yoai-mobile-nav data-yoai-mobile-anim="left">…the nav links…</nav>. Use data-yoai-mobile-anim="left" (the slide-in direction; "left" is the default for now).
  · Style the panel as a FIXED or ABSOLUTE overlay you design yourself (e.g. full-height side sheet anchored to the left, with a bg-[var(--surface)] background, padding, and the links stacked) — give it a high z-index so it sits above the page. You MAY put display/layout classes (flex, grid, block) on it; the runtime hides/shows it via INLINE styles that override those classes.
