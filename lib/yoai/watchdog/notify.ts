...
function buildDigestHtml(findings: WatchdogFinding[]): { subject: string; html: string } {
  ...
  const html = `
    <div style="font-family:'Inter',Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px 24px;background:#ffffff;color:#111827;">
      ...
      ${cards}
      <a href="${APP_URL}/yoalgoritma" style="display:inline-block;margin-top:8px;background:#10b981;color:#ffffff;font-weight:600;font-size:14px;padding:10px 20px;border-radius:8px;text-decoration:none;">YoAi'de İncele</a>
      <p style="margin-top:24px;font-size:11px;color:#9ca3af;">YoAi · Erken Uyarı · Bu otomatik tarama reklamlara dokunmaz, yalnız bilgilendirir.</p>
    </div>`
  return { subject, html }
}
