/* ──────────────────────────────────────────────────────────
   YoAlgoritma — Publish Safety Flags (Faz 6)
 ...
 * YOAI_DIRECT_PUBLISH_ENABLED=false → devre dışı.
 * Tanımlı değilse ya da 'true' → aktif.
 */
export function isDirectPublishEnabled(): boolean {
  const val = process.env.YOAI_DIRECT_PUBLISH_ENABLED
 ...
 * Sadece YOAI_ACTIVE_PUBLISH_ENABLED=true olduğunda true döner.
 * Default: false (güvenli default).
 */
export function isActivePublishEnabled(): boolean {
  return process.env.YOAI_ACTIVE_PUBLISH_ENABLED?.toLowerCase() === 'true'
}

/**
 * Aktif yayın izninin kapalı olduğunu doğrular.
 * YOAI_ACTIVE_PUBLISH_ENABLED=true ise hata fırlatır — kampanyalar PAUSED kalmalı.
 */
export function assertPausedOnly(): void {
  if (isActivePublishEnabled()) {
    throw new Error(
      '[PublishSafety] YOAI_ACTIVE_PUBLISH_ENABLED=true — bu sürümde aktif yayın desteklenmiyor. ' +
 ...
 * YOAI_DIRECT_PUBLISH_ENABLED=false ise { ok:false, code:'DIRECT_PUBLISH_DISABLED' } döner.
 */
  ...
  if (!isDirectPublishEnabled()) {
    return {
      ...
      message:
        ...
        'Aktifleştirmek için YOAI_DIRECT_PUBLISH_ENABLED=true olarak ayarlayın.',
    }
  }
  return { ok: true }
}
