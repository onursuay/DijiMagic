/* ──────────────────────────────────────────────────────────
   YoAlgoritma — Platform Doctrine Store (Faz 1)

   yoai_platform_doctrine tablosunu okur; tablo/migration yok ise
...
const TABLE_MIGRATION_HINT =
  'supabase/migrations/20260510004000_create_yoai_platform_doctrine.sql'
  ...
  if (error) {
    if (isTableMissingError(error)) {
      if (!warnedTableMissing) {
        console.warn(
          `[PlatformDoctrineStore] yoai_platform_doctrine tablosu yok — fallback hardcoded doctrine kullanılacak. ` +
