...
import { isAiEngineEnabled, isPerAccountScopeEnabled } from '@/lib/dijimagic/featureFlag'
 ...
 */
async function resolveUserScopesForCron(userId: string): Promise<(DijiMagicScope | undefined)[]> {
  ...
  try {
    ...
    console.error('[Cron][dijialgoritma-scan] resolveUserScopes hata, legacy fallback:', e)
    return [undefined]
  }
}
...
export async function GET(request: Request) {
  ...
  if (!cronSecret && isProduction) {
    console.error('[Cron][dijialgoritma-scan] CRON_SECRET tanımlı değil — prod\'da reddedildi')
  ...
  if (!isAiEngineEnabled() && !adminOverride) {
    return NextResponse.json({
      ok: true,
      skipped: true,
      reason: 'USE_AI_ENGINE=false — eski rule-engine flow çalışıyor (/api/dijimagic/daily-run)',
    })
  }
  ...
  type ScanData = { userId: string; scope?: DijiMagicScope }
  const events: Array<{ name: 'dijialgoritma/scan.user' | 'dijialgoritma/campaign-improvements.user'; data: ScanData }> = []
  for (const userId of ids) {
    const scopes = await resolveUserScopesForCron(userId)
    for (const scope of scopes) {
      const data: ScanData = scope ? { userId, scope } : { userId }
      events.push({ name: 'dijialgoritma/scan.user', data })
      events.push({ name: 'dijialgoritma/campaign-improvements.user', data })
    }
  }
  ...
  if (signingWarn) console.error(`[Cron][dijialgoritma-scan] ${signingWarn}`)
...
async function sendEventsChunked(
  ...
  for (let i = 0; i < events.length; i += chunkSize) {
    ...
    try {
      ...
      failedChunks++
      console.error(`[Cron][dijialgoritma-scan] send chunk ${i / chunkSize} başarısız (${chunk.length} event):`, e instanceof Error ? e.message : e)
    }
  }
...
export async function POST() {
  ...
  const { resolveDijiMagicScope } = await import('@/lib/dijimagic/businessScope')
  const scope = await resolveDijiMagicScope()
  const data: { userId: string; scope?: DijiMagicScope } = scope.scoped ? { userId, scope } : { userId }

  try {
    await inngest.send([
      { name: 'dijialgoritma/scan.user', data },
      { name: 'dijialgoritma/campaign-improvements.user', data },
    ])
  } catch (e) {
    console.error('[Cron][dijialgoritma-scan][POST] send başarısız:', e instanceof Error ? e.message : e)
