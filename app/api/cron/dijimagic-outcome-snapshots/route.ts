/* ──────────────────────────────────────────────────────────
   GET /api/cron/dijimagic-outcome-snapshots

   DijiAlgoritma öğrenen beyin — outcome ölçüm döngüsünün AFTER ucu.
...
import { listDueBeforeRecorded, recordAfterSnapshot, type RecommendationResultRow } from '@/lib/dijimagic/resultTrackingStore'
import { fetchCampaignMetricsById } from '@/lib/dijimagic/ai/campaignMetricsById'
...
export async function GET(request: Request) {
  ...
  if (!cronSecret) {
    console.error('[Cron][dijimagic-outcome-snapshots] CRON_SECRET tanımlı değil — reddedildi')
