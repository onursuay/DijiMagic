/* POST /api/tiktok/optimization/magic-scan
   Optimizasyon — TikTok kanadı. Bir TikTok kampanyasının sorunlarından
   öneri üretir (Google recommender platformdan bağımsız — yeniden kullanılır).
   AI istenirse Meta/Google ile AYNI sunucu-otoriter günlük kotayı tüketir. */
...
export async function POST(request: Request) {
  try {
    ...
    if (!campaign?.id || !campaign?.metrics) {
      return NextResponse.json({ ok: false, error: 'invalid_payload', message: 'Kampanya verisi gerekli' }, { status: 400 })
    }
    ...
    if (useAI && isAnthropicReady()) {
      ...
      if (!consume.allowed) {
        return NextResponse.json(
          { ok: false, error: 'ai_scan_limit', code: 'AI_SCAN_LIMIT', message: 'Günlük AI tarama limitiniz doldu ve yeterli krediniz yok.', balance: consume.balance },
          { status: 402 },
        )
      }
    }
    ...
    console.error('[TikTok Magic Scan] Error:', error)
    return NextResponse.json(
