...
// GET /api/dijimagic/articles — List all articles for current user
export async function GET() {
  ...
  if (error) {
    console.error('[dijimagic/articles/GET]', error)
...
// POST /api/dijimagic/articles — Create new article
export async function POST(request: Request) {
  ...
  if (error) {
    console.error('[dijimagic/articles/POST]', error)
    return NextResponse.json({ ok: false, error: 'db_error', message: error.message }, { status: 500 })
  }
