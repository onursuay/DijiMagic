...
import { improvementToProposal } from '@/lib/dijimagic/ai/improvementToProposal'
...
/**
 * POST /api/dijimagic/improvements/hierarchy/decision
    ...
    if (action === 'approve') {
      ...
      if (level === 'ad') {
        ...
        if (proposal && !proposal.finalUrl) {
          try {
            const { getProfileByUserId } = await import('@/lib/dijimagic/businessProfileStore')
    ...
    if (action === 'applied') {
      ...
      if (level === 'ad' && body.publishAuditId) {
        try {
          const row = (await getImprovementRow('ad', userId, id)) as AdImprovementRow | null
          if (row?.campaign_id) {
            const { fetchCampaignMetricsById } = await import('@/lib/dijimagic/ai/campaignMetricsById')
            const { recordBeforeSnapshot } = await import('@/lib/dijimagic/resultTrackingStore')
