...
import type { RootCauseId } from '@/lib/dijimagic/meta/diagnosis'

export const dynamic = 'force-dynamic'
export const maxDuration = 15

/* ────────────────────────────────────────────────────────────
   POST /api/dijimagic/actions/record
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      ...
      note: rec
        ? null
        : 'Kayıt yazılamadı (tablo yok olabilir). docs/sql/dijimagic_action_outcomes.sql şemasını çalıştırın.',
    })
  } catch (error) {
    console.error('[DijiMagic Actions Record] Error:', error)
    return NextResponse.json(
