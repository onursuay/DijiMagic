...
import { runDeepAnalysis } from '@/lib/dijimagic/deepAnalysis'
import { generateFullAutoProposals } from '@/lib/dijimagic/adCreator'
import { runFullCompetitorAnalysis } from '@/lib/dijimagic/competitorAnalyzer'
...
import {
  ...
  buildAccountScope,
  type DailyRun,
} from '@/lib/dijimagic/dailyRunStore'
...
export async function GET(request: Request) {
  ...
  // Aynı işi /api/cron/dijialgoritma-scan yapıyor — çakışmayı önle.
  if (isAiEngineEnabled() && isAnthropicReady()) {
    return NextResponse.json({
      ok: true,
      skipped: true,
      reason: 'USE_AI_ENGINE=true — AI engine handles via /api/cron/dijialgoritma-scan',
    })
  }
...
export async function POST(request: Request) {
  try {
    ...
    if (isAiEngineEnabled() && isAnthropicReady()) {
      ...
      await inngest.send({ name: 'dijialgoritma/scan.user', data: { userId } })
