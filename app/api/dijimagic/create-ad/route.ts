...
} from '@/lib/dijimagic/meta/orchestrator'
...
/* ────────────────────────────────────────────────────────────
   POST /api/dijimagic/create-ad

   Google path: değişmedi (backward compat korunuyor).

   Meta path:
   - Eğer body.creative varsa → DijiAlgoritma orchestrator kullanılır:
  ...
  try {
    ...
    if (proposal.platform === 'Google') {
      ...
      const res = await fetch(`${baseUrl}/api/integrations/google-ads/campaigns/create`, {
        ...
        body: JSON.stringify({
          campaignName: proposal.campaignName || `DijiMagic — ${proposal.headlines[0]}`,
          ...
          adGroupName: proposal.adsetName || `DijiMagic Ad Group`,
          ...
          // DijiAlgoritma "Yayınla" → kullanıcı onayıyla CANLI yayın (ENABLED, PAUSED değil).
          status: 'ENABLED',
        }),
      })
    ...
    if (proposal.platform === 'Meta') {
      ...
      if (creative) {
        const result = await orchestrateMetaCreate({
          ...
          campaignName: proposal.campaignName || `DijiMagic — ${proposal.headline}`,
          ...
          // DijiAlgoritma "Yayınla" → kullanıcı onayıyla CANLI yayın (PAUSED değil).
      ...
      const campaignRes = await fetch(`${baseUrl}/api/meta/campaigns/create`, {
        ...
        headers: { 'Content-Type': 'application/json', Cookie: cookieHeader },
        body: JSON.stringify({
          name: proposal.campaignName || `DijiMagic — ${proposal.headline}`,
          objective,
          status: 'PAUSED',
        }),
      })
      ...
      try {
        ...
        const { resolvePage } = await import('@/lib/dijimagic/meta/pageResolver')
      ...
      const adsetRes = await fetch(`${baseUrl}/api/meta/adsets/create`, {
        ...
        body: JSON.stringify({
          campaignId,
          name: proposal.adsetName || `DijiMagic Reklam Seti`,
