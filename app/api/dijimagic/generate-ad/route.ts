...
import { generateFullAutoProposals } from '@/lib/dijimagic/adCreator'
import { runFullCompetitorAnalysis, compareWithCompetitors } from '@/lib/dijimagic/competitorAnalyzer'
import type { CompetitorAd } from '@/lib/dijimagic/competitorAnalyzer'
import { listCompetitorAds } from '@/lib/dijimagic/competitorAdStore'
import { runStructuralAnalysis } from '@/lib/dijimagic/platformKnowledge'
import { fetchMetaDeep } from '@/lib/dijimagic/metaDeepFetcher'
import { fetchGoogleDeep } from '@/lib/dijimagic/googleDeepFetcher'
import { getBestAvailableRun, upsertDailyRun, getTurkeyDate } from '@/lib/dijimagic/dailyRunStore'
import type { Platform } from '@/lib/dijimagic/analysisTypes'
import { diagnoseCampaigns } from '@/lib/dijimagic/meta/diagnosis'
import { decideForDiagnoses } from '@/lib/dijimagic/meta/decision'
import type { FullAdProposal } from '@/lib/dijimagic/adCreator'
import { bulkInsertPendingApprovalsIfMissing, listApprovals } from '@/lib/dijimagic/approvalStore'
import { buildCompetitorContextForPrompt } from '@/lib/dijimagic/competitorInsightStore'
import { sanitizeProposalForDisplay } from '@/lib/dijimagic/competitorDisplay'
import { filterVisibleDijiMagicProposals } from '@/lib/dijimagic/proposalVisibilityFilter'
import { normalizeCampaignType } from '@/lib/dijimagic/campaignTypeIntelligence'
import { buildSynthesisPackagesForCampaigns } from '@/lib/dijimagic/synthesisEngine'
import type { CampaignSynthesisPackage } from '@/lib/dijimagic/synthesisTypes'
import { runMultiAiDecisionDesk, shouldUseDecisionDesk } from '@/lib/dijimagic/multiAiDecisionDesk'
import type { MultiAiDecisionDeskResult } from '@/lib/dijimagic/multiAiTypes'
import { buildProposalEngineContext } from '@/lib/dijimagic/proposalEngineOrchestrator'
import type { EngineContextDiagnostics } from '@/lib/dijimagic/proposalEngineOrchestrator'
import type { CompetitorQueryPlan } from '@/lib/dijimagic/competitorQueryExpander'
...
/* ────────────────────────────────────────────────────────────
   POST /api/dijimagic/generate-ad
            ...
            for (const [campaignId, pkg] of Object.entries(synthesisPackagesByCampaignId)) {
              try {
                const deskResult = await runMultiAiDecisionDesk(
                  ...
                  { timeoutMs: Number(process.env.DIJIMAGIC_MULTI_AI_TIMEOUT_MS || 45_000) },
                )
    ...
    if (!forceGenerate && userId) {
      ...
      if (run?.ad_proposals_data?.proposals) {
        ...
        if (persistedProposals.length > 0) {
          ...
          // filterVisibleDijiMagicProposals ile UI ve API aynı kuralı uygular.
          ...
          try {
            ...
            const expiredIds = expiredApprovals.length > 0
              ...
              : undefined
            persistedProposals = filterVisibleDijiMagicProposals(persistedProposals as any, { expiredIds }) as any[]
