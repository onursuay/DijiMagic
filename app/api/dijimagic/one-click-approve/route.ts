...
import { getCapability } from '@/lib/dijimagic/meta/capabilityMatrix'
import { resolvePage } from '@/lib/dijimagic/meta/pageResolver'
import { runPreflight } from '@/lib/dijimagic/meta/preflight'
import { orchestrateMetaCreate } from '@/lib/dijimagic/meta/orchestrator'
import { recordActionOutcome } from '@/lib/dijimagic/learningStore'
import {
  recordPublishAuditAttempt,
  updatePublishAuditStatus,
  hashPayload,
  sanitizeResponseExcerpt,
} from '@/lib/dijimagic/publishAuditStore'
import {
  markApprovalPublished,
  recordPublishAttemptOnApproval,
} from '@/lib/dijimagic/approvalStore'
import { validatePublishFeatureFlags, assertPausedOnly } from '@/lib/dijimagic/publishSafety'
import { validatePublishPayload } from '@/lib/dijimagic/publishPayloadValidator'
import { checkPolicyViolations } from '@/lib/dijimagic/policyGuard'

export const dynamic = 'force-dynamic'
export const maxDuration = 120

/* ────────────────────────────────────────────────────────────
   POST /api/dijimagic/one-click-approve

   "Tek Tıkla Onayla" — DijiAlgoritma'nın ürettiği proposal için:
...
export async function POST(request: Request) {
  ...
  // Helper: terminal-olmayan publish blokları için dijimagic_pending_approvals.metadata.last_publish_attempt yazımı.
  ...
  try {
    ...
    try {
      ...
      fd.append(
        'file',
        new File([blob], `dijimagic-oneclick-${Date.now()}.png`, {
    ...
    const result = await orchestrateMetaCreate({
      ...
      destination,
      optimizationGoal: proposal.optimizationGoal,
      campaignName: proposal.campaignName || `DijiMagic — ${proposal.headline || 'Auto'}`,
      adsetName: proposal.adsetName || `DijiMagic Reklam Seti`,
      adName: proposal.adName || proposal.campaignName || `DijiMagic Reklam`,
    ...
    console.error('[DijiMagic OneClick] Error:', error)
