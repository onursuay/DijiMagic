...
import { getBestAvailableRun, buildAccountScope } from '@/lib/dijimagic/dailyRunStore'
...
/* ────────────────────────────────────────────────────────────
   GET /api/dijimagic/command-center
  ...
  try {
    ...
    if (run && run.command_center_data) {
      ...
      if (isPerAccountScopeEnabled()) {
        ...
        // Scope, seçili işletme (dijimagic_business_scope) varsa ondan, yoksa global seçimden.
        const sc = await resolveDijiMagicScope()
      ...
      try {
        const { getHierarchyCounts } = await import('@/lib/dijimagic/ai/hierarchicalStore')
