 ...
 *     - ai_scan_usage (kullanıcı + gün sayacı) tablosu
 *     - consume_ai_scan atomik RPC (günlük kota + overage kredi düşme)
 *
 * ⚠️  ÖNEMLİ: Bu migration CANONICAL Supabase projesine (omddq) uygulanmalı.
 *     /api/meta/optimization/magic-scan artık bu RPC'ye bağımlı — kod deploy
 *     olmadan ÖNCE migration uygulanmazsa "AI ile Tara" 500 döner.
 *
 * Kullanım:
 *   node scripts/apply-ai-scan-usage-migration.mjs
 *
...
async function main() {
  ...
  try {
    ...
    console.log("     SELECT proname FROM pg_proc WHERE proname = 'consume_ai_scan';")
    console.log("     SELECT to_regclass('public.ai_scan_usage');\n")
