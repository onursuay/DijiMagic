 ...
 *     - credit_transactions (kredi defteri / ledger) tablosu
 *     - add_credits / spend_credits / refund_credits atomik RPC'leri
 *
 * ⚠️  ÖNEMLİ: Bu migration CANONICAL Supabase projesine (omddq) uygulanmalı.
 *     lib/billing/db.ts artık bu RPC'lere bağımlı — kod deploy olmadan ÖNCE
 *     migration uygulanmazsa kredi işlemleri (harcama/ekleme/iade) kırılır.
 *
 * Kullanım:
 *   node scripts/apply-billing-atomic-migration.mjs
 *
 ...
 * çalıştır — migration idempotenttir (CREATE TABLE IF NOT EXISTS / CREATE OR
 * REPLACE FUNCTION), tekrar çalıştırmak güvenlidir.
 */
...
async function main() {
  ...
  try {
    ...
    console.log("     SELECT proname FROM pg_proc WHERE proname IN ('add_credits','spend_credits','refund_credits');")
    console.log("     SELECT to_regclass('public.credit_transactions');\n")
    console.log('   Bunlar mevcutsa kodu deploy edebilirsin (push → Vercel).\n')
  } catch (err) {
