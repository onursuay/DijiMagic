/**
 * DijiAlgoritma Proposal Card Layout — Unit Tests
 *
 ...
 *
 * Çalıştırma:
 *   npx tsx src/tests/dijialgoritmaProposalCardLayout.test.ts
 */
...
console.log('🧪 DijiAlgoritma Proposal Card Layout Tests\n')

const root = join(__dirname, '..', '..')
const aiAdSuggestions = readFileSync(join(root, 'components/dijimagic/AiAdSuggestions.tsx'), 'utf-8')
const adPreviewCard = readFileSync(join(root, 'components/dijimagic/AdPreviewCard.tsx'), 'utf-8')
...
test('Grid container test id\'si eklendi', () => {
  assert.ok(aiAdSuggestions.includes('data-testid="dijimagic-proposal-grid"'),
    'data-testid="dijimagic-proposal-grid" not found')
})
...
import('../../components/dijimagic/AdPreviewCard').then(() => {
